...
export default class Routes {
  ...
  static nonGroupInvestorDashboard: string =
    '/groups/invest-west/student-showcase/dashboard/investor';
  ...
  public static isIssuerDashboardRoute = (route: string) => {
    return (
      route === Routes.nonGroupIssuerDashboard ||
      route === Routes.groupIssuerDashboard ||
  ...
  public static isInvestorDashboardRoute = (route: string) => {
    return (
      route === Routes.nonGroupInvestorDashboard ||
      route === Routes.groupInvestorDashboard ||
   ...
   * @param route
   */
  public static constructPublicRoute = (
    route:
      | 'privacyPolicy'
      | 'riskWarning'
      | 'termsOfUse'
      | 'createPitchTermsAndConditions'
   ...
   */
  public static constructHomeRoute = (
    routeParams: any,
    ...
    const getCourseRouteForUser = (): string => {
      ...
      if (
        ...
        AuthenticationState.groupsOfMembership.some(
          (membership) => membership.group.groupUserName === routeParams.groupUserName
        )
      ) {
      ...
      const investWestGroup = AuthenticationState.groupsOfMembership.find(
        (membership) => membership.group.groupUserName === 'invest-west'
      );
      if (investWestGroup) {
        return Routes.courseFront
          .replace(':groupUserName', 'invest-west')
      ...
      const homeGroup: GroupOfMembership | null = getHomeGroup(
   ...
   */
  public static constructSignUpRoute = (
   ...
   * @param AuthenticationState
   */
  public static constructExploreOffersRoute = (
    routeParams: any,
    ManageGroupUrlState: ManageGroupUrlState,
    AuthenticationState: AuthenticationState
  ) => {
    ...
    if (ManageGroupUrlState.groupNameFromUrl) {
      return Routes.groupExploreFront.replace(
        ':groupUserName',
    ...
    if (
      ManageGroupUrlState.routePath === Routes.nonGroupSignUp ||
      ManageGroupUrlState.routePath === Routes.nonGroupSignIn
    ) {
      ...
      if (currentAdmin) {
        if (currentAdmin.superAdmin) {
          ...
          if (AuthenticationState.groupsOfMembership.length >= 1) {
            ...
            return Routes.groupExploreFront.replace(
              ':groupUserName',
              adminGroup.group.groupUserName
            );
          }
        ...
        if (AuthenticationState.groupsOfMembership.length >= 1) {
          return Routes.groupExploreFront.replace(
            ':groupUserName',
            AuthenticationState.groupsOfMembership[0].group.groupUserName
          );
        }
      }
    }
   ...
   * @param AuthenticationState
   */
  public static constructDashboardRoute = (
    routeParams: any,
    ManageGroupUrlState: ManageGroupUrlState,
    AuthenticationState: AuthenticationState
  ) => {
    ...
    const getUniversityAndCourseForUser = (): {
      ...
      if (routeParams.groupUserName && routeParams.courseUserName) {
        ...
        const hasUniversityMembership = AuthenticationState.groupsOfMembership.some(
          (m) => m.group.groupUserName === routeParams.groupUserName && isUniversity(m.group)
        );
        const hasCourseMembership = AuthenticationState.groupsOfMembership.some(
          (m) => m.group.groupUserName === routeParams.courseUserName && isCourse(m.group)
        );
      ...
      const userCourseField = !currentAdmin
        ? (AuthenticationState.currentUser as User).course
        : undefined;
      ...
      if (!currentAdmin && userCourseField && userCourseField !== '-1') {
        ...
        const courseMembership = AuthenticationState.groupsOfMembership.find(
          (m) =>
            isCourse(m.group) &&
        ...
        if (courseMembership) {
          const parentUniversity = AuthenticationState.groupsOfMembership.find(
      ...
      const courseMemberships = AuthenticationState.groupsOfMembership.filter((m) =>
        isCourse(m.group)
      );
    ...
    if (currentAdmin) {
      ...
      // Group admin → course admin dashboard
        ...
        route = Routes.courseAdminDashboard
          ...
          .replace(':courseUserName', courseUserName);
        console.log(
          '[DASHBOARD DEBUG] Constructed admin dashboard route:',
          route,
    ...
    // Handle regular users (investors/issuers)
      ...
      if (isInvestor(AuthenticationState.currentUser as User)) {
        route = Routes.courseInvestorDashboard
          ...
          .replace(':courseUserName', courseUserName);
        console.log(
          '[DASHBOARD DEBUG] Constructed investor dashboard route:',
          route,
          `(university: ${universityUserName}, course: ${courseUserName})`
        );
      } else {
        ...
        route = Routes.courseIssuerDashboard
          ...
          .replace(':courseUserName', courseUserName);
        console.log(
          '[DASHBOARD DEBUG] Constructed issuer dashboard route:',
          route,
          `(university: ${universityUserName}, course: ${courseUserName})`
        );
      }
   ...
   * @param projectID
   */
  public static constructProjectDetailRoute = (
   ...
   * @param params
   */
  public static constructCreateProjectRoute = (
    groupUserName: string | null,
    courseUserName?: string | null,
    params?: CreateProjectRouteParams
  ) => {
    let route;
    if (groupUserName) {
      if (courseUserName) {
   ...
   */
  public static constructGroupDetailRoute = (
    groupUserName: string | null,
    courseUserName: string | null,
   ...
   * @param resourceName
   */
  public static constructViewResourceDetailRoute = (
    groupUserName: string | null,
    courseUserName: string | null,
    resourceName: string
  ) => {
