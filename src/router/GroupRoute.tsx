...
> {
    ...
    const getUniversityAndCourseForUser = (): {
      ...
      if (!currentAdmin && userCourseField && userCourseField !== '-1') {
        ...
        if (courseMembership) {
          ...
          console.log(
            '[GET UNIVERSITY AND COURSE] ✅ Found matching course in groupsOfMembership:',
            {
        ...
        console.log(
          '[GET UNIVERSITY AND COURSE] ⚠️ Course not found in groupsOfMembership, using as-is:',
          {
      ...
      if (courseMemberships.length > 0) {
        ...
        if (parentUniversity) {
          console.log(
            '[GET UNIVERSITY AND COURSE] ✅ Found user assigned course with parent university:',
            {
        ...
        if (firstCourse.parentGroup) {
          console.log(
            '[GET UNIVERSITY AND COURSE] ✅ Found user assigned course with parentGroup property:',
            {
              courseUserName: firstCourse.groupUserName,
      ...
      if (universityMemberships.length > 0) {
        ...
        console.log(
          '[GET UNIVERSITY AND COURSE] ⚠️ User has university membership but NO course - using DEFAULT:',
          {
    ...
    if (isInvestor(currentUser as User)) {
      const investorRoute =
        Routes.courseInvestorDashboard
          ...
          .replace(':courseUserName', courseUserName) + '?tab=Home';
      console.log(
        '[POST-LOGIN DEBUG] Investor route:',
        investorRoute,
        `(university: ${universityUserName}, course: ${courseUserName})`
      );
  ...
  componentDidUpdate(
    ...
    if (justAuthenticated) {
      const now = Date.now();
      console.log(
        `[COURSE ADMIN AUTH] ✅ Authentication completed successfully! Setting grace period timestamp: ${now}`
      );
    ...
    if (
      ...
      // Don't redirect super admin routes to 404 - let auth handle it
      console.log('[ROUTING DEBUG] ❌ TRIGGERING 404 REDIRECT - Validation check failed', {
        successfullyValidatedGroupUrl: successfullyValidatedGroupUrl(
          this.props.ManageGroupUrlState
        ),
        navigatingToError: this.state.navigatingToError,
        navigatingFromSignInOrSignUpToDashboard: this.state.navigatingFromSignInOrSignUpToDashboard,
      ...
      (isInvestWestRoute || isProtectedAdminRoute || isSuperAdminRoute) &&
      !successfullyValidatedGroupUrl(this.props.ManageGroupUrlState)
    ) {
      console.log(
        '[ROUTING DEBUG] invest-west, admin, or super admin route detected but validation pending, allowing through'
      );
    }
      ...
      if (currentUser) {
        ...
        if (Routes.isRouteReservedForSuperAdmin(this.routePath)) {
          ...
          if (!isInvestor(currentUser)) {
            ...
            if (membershipCount === 0) {
              console.log(
                '[ROUTING DEBUG] User is not a member of group:',
        ...
        if (shouldRedirectToError && !this.state.navigatingToError) {
          console.log('[ROUTING DEBUG] ⛔ REDIRECTING TO 404 PAGE - Permission check failed ⛔', {
            ...
            reasonForRedirect: Routes.isRouteReservedForSuperAdmin(this.routePath)
              ? 'Not super admin'
              : Routes.isGroupAdminRoute(this.routePath)
                ? 'Admin permission check failed'
                : Routes.isIssuerDashboardRoute(this.routePath)
                  ? 'Issuer permission check failed'
                  : Routes.isInvestorDashboardRoute(this.routePath)
                    ? 'Investor permission check failed'
                    : Routes.isCreateOfferRoute(this.routePath)
                      ? 'Create offer permission check failed'
                      : 'Unknown reason',
          });
          this.setState({
            navigatingToError: true,
  ...
  render() {
    ...
    const isAdminRoute =
      Routes.isGroupAdminRoute(this.routePath) ||
      Routes.isRouteReservedForSuperAdmin(this.routePath);
  ...
  validateRouteAndAuthentication = () => {
    ...
    if (
      this.routePath === Routes.courseCreateOffer ||
      this.routePath === Routes.groupCreateOffer ||
      this.routePath === Routes.nonGroupCreateOffer
    ) {
  ...
  attachAuthListener = () => {
    ...
    if (
      ...
      this.authListener = firebase.auth().onAuthStateChanged((firebaseUser) => {
        ...
        if (firebaseUser) {
          console.log(
            `[COURSE ADMIN AUTH] 🔥 Auth listener at ${timestamp}: Firebase user detected, calling signIn`,
            {
          ...
          console.log(
            `[COURSE ADMIN AUTH] 🔥 Auth listener at ${timestamp}: No Firebase user detected`,
            {
              isCurrentlyAuthenticating,
              isAlreadyAuthenticated,
              isWithinGracePeriod,
          ...
          if (willSignOut) {
            ...
            console.log(
              `[COURSE ADMIN AUTH] 🔥 Auth listener at ${timestamp}: Skipping signOut (user is authenticating, authenticated, within grace period, or initial check)`
            );
          }
        }
      });
