...
export default class GroupRepository {
  ...
  public async fetchCoursesByParentGroup(parentGroupId: string): Promise<GroupProperties[]> {
    try {
      ...
      console.log(
        `[GroupRepository] Fetched ${courses.length} courses for parent group: ${parentGroupId}`
      );
      return courses;
    } catch (error) {
  ...
  public async getCourseByParentAndSlug(
    ...
    try {
      ...
      if (matchingCourse) {
        console.log(
          `[GroupRepository] Found course by parent+slug: ${courseSlug} for parent ${parentGroupId}`,
          matchingCourse
        );
      } else {
        console.log(
          `[GroupRepository] Course not found: ${courseSlug} for parent ${parentGroupId}`
        );
      }
  ...
  public async fetchGroups(params?: FetchGroupsParams) {
    return await new Api().request('get', ApiRoutes.listGroups, {
      requestBody: null,
  ...
  public async fetchGroupMembers(groupID: string | 'system') {
    return await new Api().request('get', ApiRoutes.listGroupMembers.replace(':group', groupID));
  }
