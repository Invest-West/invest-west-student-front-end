    ...
    realtimeDBUtils
      ...
      .then((angelNetworks) => {
        ...
        if (shouldFilter) {
          ...
          console.log(
            '%c✅ NO FILTERING: Super user mode (superAdmin OR superGroupAdmin) - showing ALL universities',
            'color: green; font-weight: bold'
          );
        }

        console.table(
          filteredAngelNetworks.map((n) => ({
            name: n.displayName,
            anid: n.anid,
          }))
        );
...
export const ANGEL_NETWORKS_TABLE_ROWS_PER_PAGE_CHANGED =
  'ANGEL_NETWORKS_TABLE_ROWS_PER_PAGE_CHANGED';
...
export const handleAngelNetworksTableInputChanged = (event) => {
  return (dispatch, getState) => {
    ...
    const matchedAngelNetworks = angelNetworks.filter((angelNetwork) =>
      angelNetwork.displayName.toLowerCase().includes(searchText.trim().toLowerCase())
    );
...
export const TOGGLE_SEARCH_MODE_IN_ANGEL_NETWORKS_TABLE =
  'TOGGLE_SEARCH_MODE_IN_ANGEL_NETWORKS_TABLE';
...
export const startListeningForAngelNetworksChanged = () => {
  return (dispatch, getState) => {
    if (!angelNetworksListener) {
      ...
      angelNetworksListener = firebase.database().ref(DB_CONST.GROUP_PROPERTIES_CHILD);

      angelNetworksListener.on('child_added', (snapshot) => {
        ...
        const angelNetworks = [...getState().manageAngelNetworks.angelNetworks];
        const angelNetworkIndex = angelNetworks.findIndex(
          (existingAN) => existingAN.anid === angelNetwork.anid
        );
      ...
      angelNetworksListener.on('child_changed', (snapshot) => {
        const angelNetwork = snapshot.val();
      ...
      angelNetworksListener.on('child_removed', (snapshot) => {
        const removedAngelNetwork = snapshot.val();

        const angelNetworks = [...getState().manageAngelNetworks.angelNetworks];
        const filteredAngelNetworks = angelNetworks.filter(
