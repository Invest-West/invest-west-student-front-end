...
export const loadData = async (params) => {
  return (dispatch, getState) => {
    ...
    const { projectEditedLoaded, projectIDToBeLoadedAfterSavingFirstTime } =
      ...
      realtimeDBUtils
        ...
        .then((project) => {
          ...
          if (
            ...
            currentUser.anid === project.anid &&
            groupProperties &&
            groupProperties.anid === project.anid
          ) {
            dispatch({
    ...
    // in create mode
      ...
      if (
        currentUser.type === DB_CONST.TYPE_ADMIN &&
        groupProperties &&
        groupProperties.anid === currentUser.anid
      ) {
        dispatch({
          type: selectProjectVisibilityActions.SELECT_PROJECT_VISIBILITY_SET_PROJECT,
          project: null,
...
export const navigateToTheSamePageWithActiveStepSaved = (activeStep, projectID) => {
  return (dispatch, getState) => {
    const { groupUserName } = getState().manageGroupFromParams;
...
export const handleMoveToNextStep = (params) => {
  return (dispatch, getState) => {
    const { projectEdited, progressBeingSaved } = getState().manageCreateProject;
    ...
    switch (activeStep) {
      ...
      case STEP_PITCH_GENERAL_INFORMATION:
        ...
        if (pitchExpiredDate === null) {
          finalExpiredDate = utils.getDateWithDaysFurtherThanToday(365);
        }
        ...
        if (pitchSector === '-' || pitchCourse === '-' || pitchProjectName.trim().length === 0) {
          this.setState({
            createNewPitch: {
        ...
        if (
          isNaN(finalExpiredDate) ||
          (finalExpiredDate && finalExpiredDate < utils.getDateWithDaysFurtherThanToday(0))
        ) {
          this.setState({
            createNewPitch: {
              ...this.state.createNewPitch,
  ...
  return (dispatch, getState) => {
    ...
    if (utils.isDraftProject(projectEdited)) {
      if (!draftProjectListener) {
        ...
        draftProjectListener.on('child_changed', (snapshot) => {
