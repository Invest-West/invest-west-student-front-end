...
export const startListeningForLegalDocumentsChanged = () => {
  return (dispatch, getState) => {
    ...
    if (!legalDocumentsListener) {
      ...
      legalDocumentsListener.once('value', (snapshot) => {
        ...
        legalDocumentsListener.on('child_added', (snapshot) => {
          ...
          if (updatedLegalDocuments.length === 0) {
            ...
            const index = updatedLegalDocuments.findIndex(
              (doc) => doc.storageID === snapshot.val().storageID
            );
            if (index === -1) {
        ...
        legalDocumentsListener.on('child_changed', (snapshot) => {
          const updatedLegalDocuments = [...getState().legalDocuments.legalDocuments];
