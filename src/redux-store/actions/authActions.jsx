...
export const TOGGLE_PREVENT_VALIDATING_USER_WHEN_SIGNING_UP =
  'TOGGLE_PREVENT_VALIDATING_USER_WHEN_SIGNING_UP';
...
export const getUserProfileAndValidateUser = (uid) => {
  return (dispatch, getState) => {
    ...
    const { groupUserName, groupProperties } = getState().manageGroupFromParams;

    const { signingIn, user, userLoaded, preventValidatingUserWhenSigningUp } = getState().auth;
                      ...
                      authStatus:
                        groupFound.userInGroupStatus === DB_CONST.INVITED_USER_DECLINED_TO_REGISTER
                          ...
                          : groupFound.userInGroupStatus === DB_CONST.INVITED_USER_STATUS_LEFT
                            ? INVALID_AUTH_USER_LEFT
                            : groupFound.userInGroupStatus ===
                                DB_CONST.INVITED_USER_STATUS_KICKED_OUT
                              ? INVALID_AUTH_USER_KICKED_OUT
                              : INVALID_AUTH_USER_NOT_EXIST,
                    });
...
export const startListeningForUserProfileChanges = () => {
  return (dispatch, getState) => {
    if (!userListener) {
      ...
      userListener = firebase.database().ref(DB_CONST.USERS_CHILD).child(user.id);

      userListener.on('value', (snapshot) => {
...
export const startListeningForGroupsUserIsIn = () => {
  return (dispatch, getState) => {
    ...
    if (!groupsUserIsInListener) {
      ...
      groupsUserIsInListener.on('child_added', (snapshot) => {
        ...
        const index = groupsUserIsIn.findIndex(
      ...
      groupsUserIsInListener.on('child_changed', (snapshot) => {
        const invitedUser = snapshot.val();
        const groupsUserIsIn = getState().auth.groupsUserIsIn;
