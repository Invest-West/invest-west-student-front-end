...
export const ACTIVITIES_TABLE_FINISHED_LOADING_ACTIVITIES =
  'ACTIVITIES_TABLE_FINISHED_LOADING_ACTIVITIES';
...
export const startListeningForActivitiesChanged = () => {
  return (dispatch, getState) => {
    ...
    // table user is a groupProperties object
      ...
      activitiesListener = firebase.database().ref(DB_CONST.ACTIVITIES_LOG_CHILD);
    }

    activitiesListener.on('child_added', (snapshot) => {
      const activity = snapshot.val();
      const activities = [...getState().manageActivitiesTable.activities];
      ...
      if (activityIndex === -1) {
        ...
        // table user is a groupProperties object
          ...
          realtimeDBUtils.loadGroupAdminsBasedOnGroupID(tableUser.anid).then((groupAdmins) => {
            const adminIndex = groupAdmins.findIndex(
              (groupAdmin) => groupAdmin.id === activity.userID
            );
            if (adminIndex !== -1) {
