...
export const createNewForum = () => {
  return (dispatch, getState) => {
    const forumBeforeEditing = getState().manageForums.forumEdited;
      ...
      realtimeDBUtils
        ...
        .then((forumID) => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_CREATED_A_FORUM.replace(
              '%forum%',
              forum.name
            ),
          });
    ...
    // editing a forum
      ...
      if (!currentUser || (currentUser && !currentUser.superAdmin)) {
        return;
      }

      const forumAfterEditing = JSON.parse(JSON.stringify(forumBeforeEditing));
      ...
      realtimeDBUtils
        ...
        .then(() => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_A_FORUM.replace(
              '%forumName%',
              forumAfterEditing.name
            ),
          });
...
export const deleteForum = (forumID) => {
  return (dispatch, getState) => {
    ...
    const forums = [...getState().manageForums.forums];
    const forumBeforeChanging = JSON.parse(
      JSON.stringify(forums[forums.findIndex((forum) => forum.id === forumID)])
    );
    ...
    realtimeDBUtils
      ...
      .then(() => {
        ...
        trackActivity({
          ...
          activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_DELETED_A_FORUM.replace(
            '%forumName%',
            forumBeforeChanging.name
          ),
          value: {
...
export const createNewThread = () => {
  return (dispatch, getState) => {
    const threadBeforeEditing = getState().manageForums.threadEdited;
    const forumSelected = getState().manageForums.forumSelected;
    ...
    if (createThreadName.trim().length === 0 || createThreadBriefDesc.trim().length === 0) {
      dispatch({
      ...
      realtimeDBUtils
        ...
        .then((threadID) => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_CREATED_A_THREAD.replace(
              '%thread%',
              thread.name
    ...
    // editing a thread
      ...
      const threadAfterEditing = JSON.parse(JSON.stringify(threadBeforeEditing));
      ...
      realtimeDBUtils
        ...
        .then(() => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_A_THREAD.replace(
              '%threadName%',
              threadAfterEditing.name
...
export const deleteThread = (threadID) => {
  return (dispatch, getState) => {
    ...
    const threads = [...getState().manageForums.forumThreads];
    const threadBeforeChanging = JSON.parse(
      JSON.stringify(threads[threads.findIndex((thread) => thread.id === threadID)])
    );
    ...
    realtimeDBUtils
      ...
      .then(() => {
        ...
        trackActivity({
          ...
          activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_DELETED_A_THREAD.replace(
            '%threadName%',
            threadBeforeChanging.name
          ).replace('%forumName%', forumSelected.name),
          value: {
...
export const deleteThreadReply = (threadReplyID) => {
  return (dispatch, getState) => {
    ...
    const threadReplies = [...getState().manageForums.forumThreadReplies];
    const threadReplyBeforeChanging = JSON.parse(
      JSON.stringify(threadReplies[threadReplies.findIndex((reply) => reply.id === threadReplyID)])
    );
    ...
    realtimeDBUtils
      ...
      .then(() => {
        ...
        trackActivity({
          ...
          activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_DELETED_A_THREAD_REPLY.replace(
            '%threadName%',
            forumThreadSelected.name
          ).replace('%forumName%', forumSelected.name),
          value: {
...
export const submitThreadReply = () => {
  return (dispatch, getState) => {
    ...
    const threadReplyBeforeEditing = getState().manageForums.threadReplyEdited;
      ...
      realtimeDBUtils
        ...
        .then((replyID) => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_REPLIED_TO_A_THREAD.replace(
              '%thread%',
              forumThreadSelected.name
            ).replace('%forum%', forumSelected.name),
          });
    ...
    // editing an existing thread reply
      ...
      const threadReplyAfterEditing = JSON.parse(JSON.stringify(threadReplyBeforeEditing));
      ...
      realtimeDBUtils
        ...
        .then(() => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary:
              realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_A_THREAD_REPLY.replace(
                '%threadName%',
                forumThreadSelected.name
              ).replace('%forumName%', forumSelected.name),
          });
...
export const MANAGE_FORUMS_TABLE_ROWS_PER_PAGE_CHANGED =
  'MANAGE_FORUMS_TABLE_ROWS_PER_PAGE_CHANGED';
...
export const startListeningForForumsChanged = () => {
  return (dispatch, getState) => {
    if (!forumsListener) {
      forumsListener = firebase.database().ref(DB_CONST.FORUMS_CHILD);

      // a new forum is added
      forumsListener.on('child_added', (snapshot) => {
        ...
        const existingForumIndex = forums.findIndex(
          (existingForum) => existingForum.id === forum.id
        );
      ...
      // a forum has changed
      forumsListener.on('child_changed', (snapshot) => {
        const forum = snapshot.val();
      ...
      // a forums is removed
      forumsListener.on('child_removed', (snapshot) => {
        const forums = [...getState().manageForums.forums];

        const existingForumIndex = forums.findIndex(
          (existingForum) => existingForum.id === snapshot.key
        );
        forums.splice(existingForumIndex, 1);
...
export const startListeningForThreadsChanged = () => {
  return (dispatch, getState) => {
    ...
    if (!forumThreadsListener) {
      ...
      forumThreadsListener.on('child_added', (snapshot) => {
      ...
      forumThreadsListener.on('child_changed', (snapshot) => {
        const thread = snapshot.val();
      ...
      // a thread is removed
      forumThreadsListener.on('child_removed', (snapshot) => {
        const threads = [...getState().manageForums.forumThreads];

        const existingThreadIndex = threads.findIndex(
          (existingThread) => existingThread.id === snapshot.key
        );
        threads.splice(existingThreadIndex, 1);
...
export const startListeningForThreadRepliesChanged = () => {
  return (dispatch, getState) => {
    ...
    if (!forumThreadRepliesListener) {
      ...
      forumThreadRepliesListener.on('child_added', (snapshot) => {
        const reply = snapshot.val();

        const replies = [...getState().manageForums.forumThreadReplies];
      ...
      // a reply has changed
      forumThreadRepliesListener.on('child_changed', (snapshot) => {
        const reply = snapshot.val();
      ...
      // a reply is removed
      forumThreadRepliesListener.on('child_removed', (snapshot) => {
        const replies = [...getState().manageForums.forumThreadReplies];

        const existingReplyIndex = replies.findIndex(
          (existingReply) => existingReply.id === snapshot.key
        );
