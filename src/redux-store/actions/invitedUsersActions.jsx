    ...
    // enter search mode
      ...
      if (invitedUserSearchText.trim().length > 0) {
        matchedInvitedUsers = prevMatchedInvitedUsers.filter(
          (invitedUser) =>
            invitedUser.email.toLowerCase() === invitedUserSearchText.trim().toLowerCase()
        );
      } else {
...
const getUpdatedInvitedUsers = ({
  ...
  sourceInvitedUsers.forEach((invitedUser) => {
    ...
    switch (filterMembers) {
      ...
      case FILTER_HOME_MEMBERS:
        if (currentGroupProperties) {
          if (
            currentGroupProperties.anid === invitedUser.invitedBy &&
    ...
    if (
      satisfiedRegistrationStatusFilter &&
  ...
  if (searchText !== null) {
    matchedInvitedUsers = matchedInvitedUsers.filter((invitedUser) =>
      invitedUser.email.toLowerCase().includes(searchText.trim().toLowerCase())
    );
  }
    ...
    emailUtils
      .sendEmail({
        ...
        data: {
          ...
          userType: invitedUser.type === DB_CONST.TYPE_ISSUER ? 'Issuer' : 'Investor',
          signupURL: groupUserName
      ...
      .then(() => {
        ...
        realtimeDBUtils.trackActivity({
          ...
          activitySummary:
            realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_RESENT_A_USER_INVITATION.replace(
              '%userName%',
              ...
              .replace(
                '%userType%',
                invitedUser.type === DB_CONST.TYPE_ISSUER ? 'Issuer' : 'Investor'
              ),
          action: ROUTES.USER_PROFILE_INVEST_WEST_SUPER.replace(':userID', invitedUser.id),
          value: {
            ...invitedUser,
            Invitor: null,
          },
...
export const INVITED_USERS_ADDING_MEMBERS_FROM_ONE_GROUP_TO_ANOTHER_GROUP_STATE_CHANGED =
  'INVITED_USERS_ADDING_MEMBERS_FROM_ONE_GROUP_TO_ANOTHER_GROUP_STATE_CHANGED';
export const addMembersFromOneGroupToAnotherGroup = (fromGroup, toGroup) => {
  return async (dispatch, getState) => {
    const currentUser = getState().auth.user;
    const addingMembersFromOneGroupToAnotherGroup =
      getState().invitedUsers.addingMembersFromOneGroupToAnotherGroup;
    ...
    try {
      ...
      await new Api().request('put', ApiRoutes.addMembersToGroup.replace(':group', toGroup), {
...
export const INVITED_USERS_REQUESTING_CSV_STATE_CHANGED =
  'INVITED_USERS_REQUESTING_CSV_STATE_CHANGED';
export const exportToCsv = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      const response = await new Api().request('get', ApiRoutes.exportUsersCsvRoute);
...
export const startListeningForInvitedUsersChanged = () => {
  return (dispatch, getState) => {
    if (!invitedUsersListener) {
      ...
      invitedUsersListener = firebase.database().ref(DB_CONST.INVITED_USERS_CHILD);

      invitedUsersListener.on('child_added', async (snapshot) => {
        ...
        const invitedUsers = [...getState().invitedUsers.invitedUsers];
        const invitedUserIndex = invitedUsers.findIndex(
        ...
        if (invitedUserIndex === -1) {
          ...
          const matchedInvitedUsers = getUpdatedInvitedUsers({
      ...
      invitedUsersListener.on('child_changed', async (snapshot) => {
        const invitedUser = snapshot.val();
