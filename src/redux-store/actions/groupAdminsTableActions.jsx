...
export const GROUP_ADMINS_TABLE_FINISHED_LOADING_GROUP_ADMINS =
  'GROUP_ADMINS_TABLE_FINISHED_LOADING_GROUP_ADMINS';
...
export const GROUP_ADMINS_TABLE_TOGGLE_ADD_NEW_GROUP_ADMIN_DIALOG =
  'GROUP_ADMINS_TABLE_TOGGLE_ADD_NEW_GROUP_ADMIN_DIALOG';
...
export const GROUP_ADMINS_TABLE_ADD_NEW_GROUP_STATUS_CHANGED =
  'GROUP_ADMINS_TABLE_ADD_NEW_GROUP_STATUS_CHANGED';
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (checkResult.userExists) {
        ...
        const response = await new Api().request('post', ApiRoutes.addGroupAdminRoute, {
        ...
        trackActivity({
          ...
          activitySummary:
            realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_ADDED_A_NEW_GROUP_ADMIN.replace(
              '%groupAdminEmail%',
              newGroupAdminEmail
            ),
          value: newGroupAdmin,
...
export const startListeningForGroupAdminsChanged = () => {
  return (dispatch, getState) => {
    ...
    groupAdminsListener = firebase.database().ref(DB_CONST.ADMINISTRATORS_CHILD);

    groupAdminsListener.on('child_added', (snapshot) => {
      const groupAdmin = snapshot.val();
      const groupAdmins = [...getState().manageGroupAdminsTable.groupAdmins];

      const index = groupAdmins.findIndex(
        (existingGroupAdmin) => existingGroupAdmin.id === groupAdmin.id
      );
      if (index === -1) {
    ...
    groupAdminsListener.on('child_changed', (snapshot) => {
      const groupAdmin = snapshot.val();
      const groupAdmins = [...getState().manageGroupAdminsTable.groupAdmins];
    ...
    groupAdminsListener.on('child_removed', (snapshot) => {
      const groupAdmin = snapshot.val();
      const groupAdmins = [...getState().manageGroupAdminsTable.groupAdmins];
