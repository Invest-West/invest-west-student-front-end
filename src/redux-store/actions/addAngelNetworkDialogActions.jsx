...
export const addNewAngelNetwork = () => {
  return async (dispatch, getState) => {
    ...
    // generate group id
    const anid = firebase.database().ref(DB_CONST.GROUP_PROPERTIES_CHILD).push().key;
    ...
    try {
      ...
      const uploadedLogoWithText = await uploadLogo(
        dispatch,
        anid,
        UPLOAD_LOGO_WITH_TEXT,
        logoWithText
      ...
      const newGroupProperties = {
        ...
        logoWithText: !uploadedLogoWithText
          ? null
          : [
              {
        ...
        settings: {
          ...
          secondaryColor:
            secondaryColor.trim().length > 0 ? secondaryColor.toUpperCase() : '#E91E63',
        },
      };
      ...
      await new Api().request('post', ApiRoutes.addGroupAdminRoute, {
        requestBody: {
          adder: currentUser,
      ...
      await realtimeDBUtils.sendNotification({
        title: 'Change your password',
        message:
          'Welcome to Invest West. It is important to change your password as soon as possible. Please do it by clicking on the Password tab in your dashboard.',
        userID: anid,
      ...
      await firebase.database().ref(DB_CONST.GROUP_PROPERTIES_CHILD).child(anid).remove();
...
const uploadLogo = async (dispatch, anid, mode, blob) => {
  return new Promise((resolve, reject) => {
    ...
    const storageID = myUtils.getCurrentDate();
    const storageLocation = `${mode === UPLOAD_PLAIN_LOGO ? DB_CONST.PLAIN_LOGOS_CHILD : DB_CONST.LOGOS_WITH_TEXT_CHILD}/${storageID}`;
...
export const handleImageFilesChanged = (mode, files) => {
  return (dispatch, getState) => {
    if (
      getState().manageAddAngelNetworkDialog.imgUploadError.trim().length > 0 ||
      getState().manageAddAngelNetworkDialog.imgUploadErrorSnackbarOpen
    ) {
      return null;
    }
...
export const ADD_ANGEL_NETWORK_CLOSE_IMAGE_FILE_ERROR_SNACKBAR =
  'ADD_ANGEL_NETWORK_CLOSE_IMAGE_FILE_ERROR_SNACKBAR';
...
export const ADD_ANGEL_NETWORK_RESET_IMAGE_FILE_ERROR_MESSAGE_WHEN_SNACKBAR_EXITED =
  'ADD_ANGEL_NETWORK_RESET_IMAGE_FILE_ERROR_MESSAGE_WHEN_SNACKBAR_EXITED';
