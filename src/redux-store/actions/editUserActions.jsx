...
export const setOriginalUserAndEditedUser = (user) => {
  return (dispatch, getState) => {
    ...
    if (
      user.hasOwnProperty('groupsUserIsIn') &&
      currentUser.type === DB_CONST.TYPE_ADMIN &&
      !currentUser.superAdmin
    ) {
      const userHomeGroup = utils.getUserHomeGroup(user.groupsUserIsIn);
...
export const EDIT_ORDINARY_BUSINESS_PROFILE_INFORMATION =
  'EDIT_ORDINARY_BUSINESS_PROFILE_INFORMATION';
...
export const commitUserProfileChanges = (type) => {
  return (dispatch, getState) => {
    ...
    const userRef = firebase.database().ref(DB_CONST.USERS_CHILD).child(userEdited.id);
    ...
    if (type === COMMIT_PERSONAL_INFORMATION_CHANGES) {
        ...
        .update({
          ...
          linkedin:
            userEdited.linkedin && userEdited.linkedin.trim().length > 0
              ...
              : null,
          company:
            userEdited.company && userEdited.company.trim().length > 0 ? userEdited.company : null,
          companyName:
            ...
            activitySummary:
              currentUser.id === userEdited.id
                ...
                : // admin is editing user's details
                  realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_ADMIN_UPDATED_USER_PERSONAL_DETAILS.replace(
                    '%user%',
                    userEdited.firstName + ' ' + userEdited.lastName
                  ),
            action:
              currentUser.id === userEdited.id
                ? // user is editing their own details
      ...
      const businessProfile = JSON.parse(JSON.stringify(userEdited.BusinessProfile));
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary:
              currentUser.id === userEdited.id
                ...
                : // admin is editing user's details
                  realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_ADMIN_UPDATED_USER_BUSINESS_PROFILE.replace(
                    '%user%',
                    userEdited.firstName + ' ' + userEdited.lastName
                  ),
            action:
...
export const startOriginalUserChangedListener = () => {
  return (dispatch, getState) => {
    ...
    if (!originalUserListener) {
      originalUserListener = firebase.database().ref(DB_CONST.USERS_CHILD).child(originalUser.id);

      originalUserListener.on('value', (snapshot) => {
