...
export const loadGroups = () => {
  return async (dispatch, getState) => {
    const { loadingGroups, groupsLoaded } = getState().manageSystemGroups;
    ...
    if (!loadingGroups && !groupsLoaded) {
      ...
      try {
        ...
        const universities = await realtimeDBUtils.loadAngelNetworks(
          {},
          realtimeDBUtils.SEARCH_ANGEL_NETWORKS_NONE
        );
...
export const startListeningForSystemGroupsChanged = () => {
  return (dispatch, getState) => {
    ...
    if (!groupsListener) {
      groupsListener = firebase.database().ref(DB_CONST.GROUP_PROPERTIES_CHILD);

      groupsListener.on('child_added', (snapshot) => {
        const group = snapshot.val();

        const groups = [...getState().manageSystemGroups.systemGroups];
      ...
      groupsListener.on('child_changed', (snapshot) => {
        const group = snapshot.val();
      ...
      groupsListener.on('child_removed', (snapshot) => {
    ...
    if (!coursesListener) {
      coursesListener = firebase.database().ref(DB_CONST.COURSES_CHILD);

      coursesListener.on('child_added', (snapshot) => {
      ...
      coursesListener.on('child_changed', (snapshot) => {
        const course = snapshot.val();
      ...
      coursesListener.on('child_removed', (snapshot) => {
        const removedCourse = snapshot.val();

        const groups = [...getState().manageSystemGroups.systemGroups];
        const filteredGroups = groups.filter(
          (existingGroup) => existingGroup.anid !== removedCourse.anid
        );
