...
export const MANAGE_PLEDGES_TABLE_FINISHED_LOADING_PLEDGES =
  'MANAGE_PLEDGES_TABLE_FINISHED_LOADING_PLEDGES';
export const loadPledges = () => {
  return (dispatch, getState) => {
    const { project } = getState().managePledgesTable;
        ...
        Promise.all(
          pledges.map((pledge) => {
            return new Promise((resolve, reject) => {
              realtimeDBUtils.loadGroupsUserIsIn(pledge.investorID).then((groupsInvestorIsIn) => {
...
export const toggleSearchMode = () => {
  return (dispatch, getState) => {
    ...
    if (inSearchMode) {
      ...
      const matchedPledges = pledges.filter(
        (pledge) =>
          pledge.investor.firstName.toLowerCase().includes(searchText.trim().toLowerCase()) ||
...
export const MANAGE_PLEDGES_TABLE_ROWS_PER_PAGE_CHANGED =
...
export const startListeningForPledgesChanged = () => {
  return (dispatch, getState) => {
    if (!pledgesListener) {
      ...
      pledgesListener.on('child_added', (snapshot) => {
        const pledge = snapshot.val();

        const pledges = [...getState().managePledgesTable.pledges];
        const pledgeIndex = pledges.findIndex((existingPledge) => existingPledge.id === pledge.id);
        if (pledgeIndex === -1) {
      ...
      pledgesListener.on('child_changed', (snapshot) => {
        const pledge = snapshot.val();
      ...
      pledgesListener.on('child_removed', (snapshot) => {
