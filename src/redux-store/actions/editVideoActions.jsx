...
export const handleSaveVideo = () => {
  return (dispatch, getState) => {
    ...
    uploadVideo(
      ...
      getState().auth.user,
      getState().editVideo,
      getState().editVideo.videoChosen
    );
  };
};
...
export const uploadVideo = async (dispatch, user, editVideoState, video) => {
  const { mode, videoPreIndex } = editVideoState;
  ...
  return new Promise((resolve, reject) => {
    ...
    const videoUploadTask = storageRef.put(
      downloadedPreviousVideo ? downloadedPreviousVideo : video
    );
    videoUploadTask.on(
      'state_changed',
      (snapshot) => {
 ...
 * @returns {Promise<unknown>}
 */
const putUploadedVideoToFirebaseRealtime = async (
  dispatch,
  user,
  videoUploadedObj,
  videoPreIndex
