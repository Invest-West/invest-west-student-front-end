          ...
          if (user.type === DB_CONST.TYPE_ISSUER) {
            if (user.LegalDocuments) {
              if (
                ...
                DB_CONST.MAX_LEGAL_DOCUMENTS_UPLOAD_FOR_ISSUER -
                  user.LegalDocuments.filter((document) => !document.hasOwnProperty('removed'))
                    .length
              ) {
                dispatch({
...
export const RESET_ERROR_MESSAGE_WHEN_FILE_UPLOAD_ERROR_SNACKBAR_EXITED =
  'RESET_ERROR_MESSAGE_WHEN_FILE_UPLOAD_ERROR_SNACKBAR_EXITED';
...
export const DELETE_UPLOADED_PITCH_PRESENTATION_DOCUMENT =
  'DELETED_UPLOADED_PITCH_PRESENTATION_DOCUMENT';
export const DELETE_UPLOADED_PITCH_SUPPORTING_DOCUMENT =
  ...
  return (dispatch, getState) => {
    switch (mode) {
      case DELETE_UPLOADED_LEGAL_DOCUMENT:
        ...
        const updateLegalDocuments = [...getState().legalDocuments.legalDocuments];
...
export const DELETE_TO_BE_UPLOADED_PITCH_PRESENTATION_DOCUMENT =
  'DELETE_TO_BE_UPLOADED_PITCH_PRESENTATION_DOCUMENT';
export const DELETE_TO_BE_UPLOADED_PITCH_SUPPORTING_DOCUMENT =
  ...
  return (dispatch, getState) => {
    ...
    switch (mode) {
      case DELETE_TO_BE_UPLOADED_LEGAL_DOCUMENT:
        updatedFilesToBeUploaded = JSON.parse(
          JSON.stringify(getState().uploadFiles.filesToBeUploaded.legalDocuments)
        );
        break;
      case DELETE_TO_BE_UPLOADED_PITCH_COVER:
        updatedFilesToBeUploaded = JSON.parse(
          JSON.stringify(getState().uploadFiles.filesToBeUploaded.pitchCover)
        );
        break;
      case DELETE_TO_BE_UPLOADED_PITCH_PRESENTATION_DOCUMENT:
        updatedFilesToBeUploaded = JSON.parse(
          JSON.stringify(getState().uploadFiles.filesToBeUploaded.pitchPresentationDocument)
        );
        break;
      case DELETE_TO_BE_UPLOADED_PITCH_SUPPORTING_DOCUMENT:
        updatedFilesToBeUploaded = JSON.parse(
          JSON.stringify(getState().uploadFiles.filesToBeUploaded.pitchSupportingDocuments)
        );
        break;
      default:
        return;
...
export const FINISHED_UPLOADING_A_PITCH_PRESENTATION_DOCUMENT =
  'FINISHED_UPLOADING_A_PITCH_PRESENTATION_DOCUMENT';
export const FINISHED_UPLOADING_A_PITCH_SUPPORTING_DOCUMENT =
  'FINISHED_UPLOADING_A_PITCH_SUPPORTING_DOCUMENT';
...
const putFilesReferencesToDatabase = async (
  dispatch,
  getState,
  mode,
  user = null,
