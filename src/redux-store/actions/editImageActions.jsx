...
export const handleImageFilesChanged = (files) => {
  return (dispatch, getState) => {
    if (
      getState().manageFeedbackSnackbar.message.trim().length > 0 ||
      getState().manageFeedbackSnackbar.open
    ) {
      return;
    }
...
export const viewEditedImagePreview = () => {
  return (dispatch, getState) => {
    ...
    getEditedImage(imgEditor).then((blob) => {
      dispatch({
...
export const CREATE_BUSINESS_PROFILE_SAVE_EDITED_IMAGE =
  'CREATE_BUSINESS_PROFILE_SAVE_EDITED_IMAGE';
export const handleSaveEditedImage = () => {
  return (dispatch, getState) => {
    if (getState().editImage.mode === UPLOAD_LOGO_FIRST_TIME_MODE) {
      getEditedImage(getState().editImage.imgEditor).then((blob) => {
        dispatch({
...
export const uploadImage = async (dispatch, user, editImageState, blob) => {
  const { mode, imgPreIndex } = editImageState;
  ...
  if (
    ...
    mode !== UPLOAD_LOGO_FIRST_TIME_MODE &&
    mode !== UPLOAD_LOGO_MODE
  ) {
    return;
  }
    ...
    uploadTask.on(
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((imgDownloadURL) => {
          ...
          let prevNodeImgs = [];
          let firebaseRef = firebaseDB.ref(DB_CONST.USERS_CHILD).child(user.id);
          ...
          // upload profile picture
            ...
            firebaseRef = firebaseRef.child(DB_CONST.PROFILE_PICTURE_CHILD);
          }
