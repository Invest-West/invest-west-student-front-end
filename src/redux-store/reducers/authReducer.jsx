...
import { INVALID_AUTH_NONE, AUTH_SUCCESS } from '../../pages/signin/Signin';
  ...
  switch (action.type) {
    ...
    case authActions.FINISHED_AUTHENTICATING:
      return {
        ...
        user:
          action.authStatus !== AUTH_SUCCESS && action.authStatus !== INVALID_AUTH_NONE
            ? null
            : state.user,
        userLoaded:
          action.authStatus !== AUTH_SUCCESS && action.authStatus !== INVALID_AUTH_NONE
        ...
        userBeingLoaded:
          action.authStatus !== AUTH_SUCCESS && action.authStatus !== INVALID_AUTH_NONE
            ? false
            : state.userBeingLoaded,

        groupsUserIsIn:
          action.authStatus !== AUTH_SUCCESS && action.authStatus !== INVALID_AUTH_NONE
            ? null
            : state.groupsUserIsIn,
        loadingGroupsUserIsIn:
          action.authStatus !== AUTH_SUCCESS && action.authStatus !== INVALID_AUTH_NONE
            ? false
            : state.loadingGroupsUserIsIn,
        groupsUserIsInLoaded:
          action.authStatus !== AUTH_SUCCESS && action.authStatus !== INVALID_AUTH_NONE
            ? false
            : state.groupsUserIsInLoaded,
      };
