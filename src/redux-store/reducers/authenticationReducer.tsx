...
const authenticationReducer = (state = initialState, action: AuthenticationAction) => {
  ...
  switch (action.type) {
    ...
    case AuthenticationEvents.CompleteAuthentication:
      const completeAuthenticationAction: CompleteAuthenticationAction =
        action as CompleteAuthenticationAction;
      ...
      const newAuthState = {
        ...
        currentUser: completeAuthenticationAction.currentUser
          ? JSON.parse(JSON.stringify(completeAuthenticationAction.currentUser))
          : state.currentUser,
        groupsOfMembership: JSON.parse(
          JSON.stringify(completeAuthenticationAction.groupsOfMembership)
        ),
        error: completeAuthenticationAction.error,
      };

      console.log(
        `[REDUX AUTH] [${timestamp}] New Redux auth state after CompleteAuthentication:`,
        {
