...
const groupAdminSettingsReducer = (state = initState, action) => {
  switch (action.type) {
    ...
    case groupAdminSettingsActions.GROUP_ADMIN_SETTINGS_INITIALIZE_GROUP_ATTRIBUTES_EDITED:
      const groupSettings = JSON.parse(JSON.stringify(action.group.settings));
    ...
    case manageANIDFromParamsActions.ANGEL_NETWORK_PROPERTIES_CHANGED:
      if (action.key === 'settings') {
        return {
          ...
          expandedPledgeFAQ: !action.value.hasOwnProperty(DB_CONST.PLEDGE_FAQS_CHILD)
            ? null
            : !state.expandedPledgeFAQ
              ? null
              : action.value[DB_CONST.PLEDGE_FAQS_CHILD].findIndex(
                    (pledge) => pledge.id === state.expandedPledgeFAQ.id
                  ) !== -1
                ? action.value[DB_CONST.PLEDGE_FAQS_CHILD][
                    action.value[DB_CONST.PLEDGE_FAQS_CHILD].findIndex(
                      (pledge) => pledge.id === state.expandedPledgeFAQ.id
                    )
                  ]
