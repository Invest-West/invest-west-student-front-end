...
import { UPLOAD_VIDEO_MODE } from '../../shared-components/uploading-dialog/UploadingDialog';
  ...
  switch (action.type) {
    ...
    case editVideoActions.TOGGLE_EDIT_VIDEO_DIALOG:
      return {
        ...state,
        editVideoDialogOpen:
          action.hasOwnProperty('waitForUploading') && !action.waitForUploading
            ? false
            : !state.editVideoDialogOpen,
        ...
        previousVideos:
          action.mode === UPLOAD_VIDEO_MODE
            ? !action.hasOwnProperty('user') ||
              (action.hasOwnProperty('user') && !action.user.BusinessProfile.video)
              ? []
