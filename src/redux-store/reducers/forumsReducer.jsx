...
const forumsReducer = (state = initState, action) => {
  switch (action.type) {
    ...
    case forumsActions.TOGGLE_CREATE_NEW_FORUM:
      return {
        ...state,
        modeForums: action.open ? MODE_CREATE_FORUM : MODE_FORUMS_MAIN,
        forumEdited: action.forumEdited ? JSON.parse(JSON.stringify(action.forumEdited)) : null,
        createForumName: action.forumEdited ? action.forumEdited.name : '',
        createForumDesc: action.forumEdited ? action.forumEdited.description : '',
        createForumError: false,
        createForumAuthor: null,
      };
    ...
    case forumsActions.TOGGLE_CREATE_NEW_THREAD:
      return {
        ...state,
        modeForums: action.open ? MODE_CREATE_THREAD : MODE_VIEW_FORUM,
        threadEdited: action.threadEdited ? JSON.parse(JSON.stringify(action.threadEdited)) : null,
        createThreadName: action.threadEdited ? action.threadEdited.name : '',
        createThreadBriefDesc: action.threadEdited ? action.threadEdited.description : '',
        createThreadMessage: action.threadEdited ? action.threadEdited.message : { ops: [] },
        createThreadError: false,
      };
    ...
    case forumsActions.FORUMS_QUILL_CHANGED:
      return {
        ...state,
        createThreadMessage:
          action.field === forumsActions.THREAD_CONTENT_QUILL
            ? action.quill
            : state.createThreadMessage,
        threadReplyMessage:
          action.field === forumsActions.THREAD_REPLY_CONTENT_QUILL
            ? action.quill
            : state.threadReplyMessage,
      };
    ...
    case forumsActions.MANAGE_FORUMS_TABLE_ROWS_PER_PAGE_CHANGED:
      return {
        ...
        forumsTableRowsPerPage:
          action.table === 'forums' ? action.value : state.forumsTableRowsPerPage,
        threadsTableRowsPerPage:
          action.table === 'threads' ? action.value : state.threadsTableRowsPerPage,
      };
    default:
      return state;
  }
