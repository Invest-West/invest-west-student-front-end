...
export const fetchActivitiesBy = async ({
  ...
  return new Promise((resolve, reject) => {
    ...
    let activitiesRef = firebase.database().ref(DB_CONST.ACTIVITIES_LOG_CHILD);
...
export const sendNotification = async ({ title = '', message = '', userID = '', action = '' }) => {
  return new Promise((resolve, reject) => {
    ...
    db.ref(DB_CONST.NOTIFICATIONS_CHILD)
      .child(id)
      .set({
        id,
        title,
  ...
  return new Promise((resolve, reject) => {
    ...
    firebase
      ...
      .once('value', (snapshots) => {
        ...
        Promise.all(
          groupsUserIsIn.map((groupUserIsIn) => {
            return new Promise((resolve, reject) => {
              firebase
                .database()
          ...
          firebase
            ...
            .once('value', (courseSnapshot) => {
              ...
              if (courseData.parentGroupId) {
                console.log(
                  '[REALTIME-DB DEBUG] Course found, loading parent university:',
                  courseData.parentGroupId
                );
                firebase
                  .database()
                  .ref(DB_CONST.GROUP_PROPERTIES_CHILD)
                  ...
                  .once('value', (parentSnapshot) => {
                    if (!parentSnapshot || !parentSnapshot.exists()) {
                      console.warn(
                        '[REALTIME-DB DEBUG] Parent university not found for course:',
                        courseData.parentGroupId
                      );
...
export const loadAngelNetworks = async ({ name = null, email = null }, mode) => {
  return new Promise((resolve, reject) => {
    ...
    angelNetworksRef.once('value', (snapshots) => {
      if (
        !snapshots ||
        !snapshots.exists() ||
        (snapshots && (!snapshots.val() || snapshots.numChildren() === 0))
...
export const loadAngelNetworksInvitedAUser = async (email) => {
  const db = firebase.database();
  const angelNetworksInvitedUser = [];

  return new Promise((resolve, reject) => {
    db.ref(DB_CONST.INVITED_USERS_CHILD)
      .orderByChild('email')
      .equalTo(email.toLowerCase())
      .once('value', (snapshots) => {
        ...
        Promise.all(
          angelNetworksInvitedUser.map((angelNetworkInvitedUser) => {
            return new Promise((resolve, reject) => {
              db.ref(DB_CONST.GROUP_PROPERTIES_CHILD)
...
export const loadForums = async (mode) => {
  return new Promise((resolve, reject) => {
    ...
    db.ref(DB_CONST.FORUMS_CHILD).once('value', (snapshots) => {
      // no forums
      if (
        !snapshots ||
        !snapshots.exists() ||
...
export const createThread = async (threadObj) => {
  return new Promise((resolve, reject) => {
    ...
    db.ref(DB_CONST.FORUM_THREADS_CHILD)
      .child(threadObj.id)
      .set(threadObj)
      .then(() => {
...
export const createThreadReply = async (replyObj) => {
  return new Promise((resolve, reject) => {
    ...
    replyObj.id = replyID;
    db.ref(DB_CONST.FORUM_THREAD_REPLIES_CHILD)
      .child(replyObj.id)
      .set(replyObj)
      .then(() => {
...
export const loadClubAttributes = async () => {
  const db = firebase.database();
  return new Promise((resolve, reject) => {
    db.ref(DB_CONST.CLUB_ATTRIBUTES_CHILD).once('value', (snapshot) => {
      ...
      .once('value', (snapshots) => {
        ...
        Promise.all(
          invitedUsers.map((invitedUser) => {
            return new Promise((resolve, reject) => {
              loadAngelNetworkBasedOnANID(invitedUser.invitedBy).then((angelNetwork) => {
                invitedUser.Invitor = {
                  anid: angelNetwork.anid,
                  displayName: angelNetwork.displayName,
                  logo: angelNetwork.logo,
                ...
                if (invitedUser.hasOwnProperty('officialUserID')) {
                  getUserBasedOnID(invitedUser.officialUserID).then((officialUser) => {
...
export const loadRequestsToJoin = async ({ userID = null, anid = null }) => {
  return new Promise((resolve, reject) => {
    let requestsRef = firebase.database().ref(DB_CONST.REQUESTS_TO_JOIN_CHILD);
    ...
    requestsRef.once('value', (snapshots) => {
      if (
        !snapshots ||
        !snapshots.exists() ||
        (snapshots && (!snapshots.val() || snapshots.numChildren() === 0))
...
export const fetchProjectsBy = async (
  ...
  return new Promise((resolve, reject) => {
    ...
    projectsRef.limitToLast(limit).once('value', (snapshots) => {
      if (
        !snapshots ||
        !snapshots.exists() ||
        (snapshots && (!snapshots.val() || snapshots.numChildren() === 0))
  ...
  return new Promise((resolve, reject) => {
    db.ref(DB_CONST.PROJECTS_CHILD)
      ...
      .once('value', (snapshot) => {
        ...
        const isFirebaseKey =
          project.anid && project.anid.startsWith('-') && project.anid.length > 10;
        ...
        if (!isFirebaseKey && project.anid) {
          ...
          loadAngelNetworkBasedOnANID(project.anid)
            ...
            .catch((error) => {
              console.log(
                '[REALTIME-DB DEBUG] ANID lookup failed, trying fallback with invest-west:',
                {
                  originalError: error,
                  projectAnid: project.anid,
                  projectID: projectID,
              ...
              fallbackPromise
                ...
                .catch((fallbackError) => {
                  ...
                  console.error(
                    '[REALTIME-DB ERROR] Complete failure in group loading for project',
                    projectID
                  );
...
export const loadProjectRejectFeedbacks = async (projectID) => {
  const db = firebase.database();
  const rejectFeedbacks = [];

  return new Promise((resolve, reject) => {
    db.ref(DB_CONST.PROJECT_REJECT_FEEDBACKS_CHILD)
  ...
  return new Promise((resolve, reject) => {
    ...
    firebase
      ...
      .then(() => {
        ...
        // load votes
        loadVotes(project.id, null, LOAD_VOTES_ORDER_BY_PROJECT).then((votes) => {
          ...
          // load pledges
          loadPledges(project.id, null, LOAD_PLEDGES_ORDER_BY_PROJECT).then((pledges) => {
...
export const loadComments = async (projectID) => {
  ...
  return new Promise((resolve, reject) => {
    db.ref(DB_CONST.COMMENTS_CHILD)
      .orderByChild('projectID')
      .equalTo(projectID)
      .once('value', (snapshots) => {
        if (
...
export const loadPledges = async (projectID = null, investorID = null, mode) => {
  return new Promise((resolve, reject) => {
    ...
    pledgesRef.once('value', (snapshots) => {
      if (
        !snapshots ||
        !snapshots.exists() ||
        (snapshots && (!snapshots.val() || snapshots.numChildren() === 0))
...
export const postCommentReply = (commentReply) => {
  return new Promise((resolve, reject) => {
    ...
    db.ref(DB_CONST.COMMENT_REPLIES_CHILD)
      .child(replyID)
      .set({
        ...commentReply,
        id: replyID,
...
export const loadMarketingPreferences = (userID) => {
  return new Promise((resolve, reject) => {
    ...
    marketingPreferencesRef.once('value', (snapshots) => {
      if (
        !snapshots ||
        !snapshots.exists() ||
        (snapshots && (!snapshots.val() || snapshots.numChildren() === 0))
      ...
      snapshots.forEach((snapshot) => {
        const marketingPreference = snapshot.val();
 ...
 * @returns {Promise<unknown>}
 */
export const toggleMarketingPreferences = async ({ userID, value }) => {
  return new Promise((resolve, reject) => {
    if (!userID || !value) {
 ...
 * @returns {Promise<unknown>}
 */
export const updateMarketingPreferencesSetting = async ({ userID, field, value }) => {
  return new Promise((resolve, reject) => {
    loadMarketingPreferences(userID)
      .then((marketingPreferences) => {
        // user has not registered for Marketing preferences before
 ...
 * @returns {Promise<unknown>}
 */
export const updateGroupPropertiesSetting = async ({ anid, field, value }) => {
  return new Promise((resolve, reject) => {
    firebase
      .database()
      .ref(DB_CONST.GROUP_PROPERTIES_CHILD)
