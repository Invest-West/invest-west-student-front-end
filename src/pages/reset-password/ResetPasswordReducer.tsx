...
export const successfullyVerifyingCode = (state: ResetPasswordState) => {
  return (
    ...
    state.codeVerified &&
    state.email !== null &&
    state.errorVerifyingCode === undefined
  );
};
...
export const successfullyResettingPassword = (state: ResetPasswordState) => {
  return (
    !state.resettingPassword && state.passwordReset && state.errorResettingPassword === undefined
  );
};
...
const resetPasswordReducer = (state = initialState, action: ResetPasswordAction) => {
  switch (action.type) {
    ...
    case ResetPasswordEvents.CompleteVerifyingCode:
      const completeVerifyingCodeAction: CompleteVerifyingCodeAction =
        action as CompleteVerifyingCodeAction;
      return {
        ...state,
        verifyingCode: false,
        codeVerified: true,
        ...
        errorVerifyingCode:
          completeVerifyingCodeAction.error !== undefined
            ? { detail: completeVerifyingCodeAction.error }
            : state.errorVerifyingCode,
      };
    ...
    case ResetPasswordEvents.CompleteConfirmingResetPassword:
      const completeConfirmingResetPasswordAction: CompleteConfirmingResetPasswordAction =
        action as CompleteConfirmingResetPasswordAction;
      return {
        ...state,
        resettingPassword: false,
        passwordReset: completeConfirmingResetPasswordAction.error === undefined,
        errorResettingPassword: completeConfirmingResetPasswordAction.error
          ? { detail: completeConfirmingResetPasswordAction.error }
          : state.errorResettingPassword,
      };
    default:
      return state;
  }
