...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateDocumentDescription: (mode, index, description) =>
      dispatch(uploadFilesActions.updateDocumentDescription(mode, index, description)),
    deleteToBeUploadedFile: (mode, index) =>
      dispatch(uploadFilesActions.deleteToBeUploadedFile(mode, index)),
  };
};
...
class CreatePitchPageMain extends Component {
  ...
  componentDidMount() {
    const { requestToLoadData } = this.state;
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        this.setState(
          {
            ...
            createProject: {
              ...
              requestToLoadData: !previousState.requestToLoadData
                ? true
                : previousState.requestToLoadData,
            },
          },
    ...
    if (prevProps.AuthenticationState.currentUser !== this.props.AuthenticationState.currentUser) {
      console.log(
        'Authentication state changed. Current user:',
        this.props.AuthenticationState.currentUser
      );
    }
  }
  ...
  loadData = () => {
    ...
    const { ManageGroupUrlState, AuthenticationState, selectProjectVisibility_setProject } =
      this.props;

    const { projectEditedLoaded, projectIDToBeLoadedAfterSavingFirstTime } = this.state;
    ...
    if (
      ...
      AuthenticationState.currentUser.type === DB_CONST.TYPE_INVESTOR
    ) {
      ...
      if (params.edit || projectIDToBeLoadedAfterSavingFirstTime) {
        ...
        if (!projectEditedLoaded) {
          console.log(
            '[CREATE-PROJECT DEBUG] Setting groupIssuerCreateOfferFor from ManageGroupUrlState:',
            {
        ...
        .then((project) => {
          ...
          if (project.status === DB_CONST.PROJECT_STATUS_DRAFT) {
            if (!this.projectEditedSaveModeRefListener) {
              ...
              this.projectEditedSaveModeRefListener.on('child_changed', (snapshot) => {
          ...
          this.setState({
            ...
            createProject: {
              ...this.state.createProject,
              groupIssuerCreateOfferFor: !isDraftProjectNotSubmitted(project)
                ? 'undefined'
                : project.hasOwnProperty('anid')
                  ? project.anid
                  : 'undefined',
              // project sector
              pitchSector: project.hasOwnProperty('sector') ? project.sector : '',
              pitchCourse: project.hasOwnProperty('course') ? project.course : '',
              // project project name
              pitchProjectName: project.hasOwnProperty('projectName') ? project.projectName : '',
              pitchProjectDescription: project.hasOwnProperty('description')
                ? project.description
                : '',
              pitchExpiryDate: project.Pitch.hasOwnProperty('expiredDate')
              ...
              //         ''
              // ,
              detailsAboutEarlierFundraisingRounds: project.Pitch.hasOwnProperty(
                'detailsAboutEarlierFundraisingRounds'
              )
              ...
              //         ''
              // ,
              pitchPresentationText: !project.Pitch.presentationText
                ? { ops: [] }
                : project.Pitch.presentationText,
              // this field is only available for QIB
              qibSpecialNews: project.Pitch.hasOwnProperty('qibSpecialNews')
  ...
  handleNextStepClick = () => {
    ...
    const { projectEdited, progressBeingSaved, projectEditedLoaded } = this.state;
    ...
    switch (activeStep) {
        ...
        if (params.edit && projectEdited) {
          if (
            (!projectEdited.Pitch.cover ||
              (projectEdited.Pitch.cover &&
                projectEdited.Pitch.cover.filter(
                  (coverItem) => !coverItem.hasOwnProperty('removed')
                ).length === 0)) &&
            (!pitchCoverTypeSelected ||
              (pitchCoverTypeSelected === PITCH_COVER_FILE_TYPE_SELECTED &&
                pitchCover.length === 0) ||
              (pitchCoverTypeSelected === PITCH_COVER_VIDEO_URL_TYPE_SELECTED &&
                pitchCoverVideoURL.trim().length === 0))
          ) {
            this.setState({
              createProject: {
                ...this.state.createProject,
      ...
      case STEP_PITCH_DECK:
        ...
        if (ManageGroupUrlState.groupNameFromUrl === 'qib') {
          ...
          if (params.edit && projectEdited) {
            if (
              pitchPresentationDocument.length === 0 &&
              (!projectEdited.Pitch.presentationDocument ||
                (projectEdited.Pitch.presentationDocument &&
                  projectEdited.Pitch.presentationDocument.filter(
  ...
  handleBackClick = () => {
    ...
    this.navigateToTheSamePageWithActiveStepSaved(
      activeStep - 1,
      projectEdited ? projectEdited.id : null
    );
  };
  ...
  handleSelectPitchCoverTypeClick = (type) => {
    const { pitchCoverTypeSelected, pitchCover, pitchCoverVideoURL } = this.state.createProject;
    ...
    if (pitchCoverTypeSelected === type) {
      this.setState({
        createProject: {
          ...
          pitchCoverVideoURL:
            type === PITCH_COVER_VIDEO_URL_TYPE_SELECTED ? '' : pitchCoverVideoURL,
        },
      });
    } else {
      this.setState({
  ...
  handleFilesChanged = (mode, files) => {
    ...
    const { projectEdited } = this.state;

    const { pitchSupportingDocuments } = this.state.createProject;
          ...
          // for other groups, the user can upload multiple files for supporting documents
            ...
            if (projectEdited) {
              if (!projectEdited.Pitch.supportingDocuments) {
                if (
                  pitchSupportingDocuments.length >=
                  DB_CONST.MAX_FILES_FOR_PITCH_SUPPORTING_DOCUMENTS
                ) {
                  setFeedbackSnackbarContent('Max file count reached.', 'error', 'bottom');
                  break;
                }
              } else {
                if (
                  pitchSupportingDocuments.length >=
                  DB_CONST.MAX_FILES_FOR_PITCH_SUPPORTING_DOCUMENTS -
                    projectEdited.Pitch.supportingDocuments.filter(
                      (document) => !document.hasOwnProperty('removed')
                    ).length
                ) {
                  setFeedbackSnackbarContent('Max file count reached.', 'error', 'bottom');
              ...
              if (
                pitchSupportingDocuments.length >= DB_CONST.MAX_FILES_FOR_PITCH_SUPPORTING_DOCUMENTS
              ) {
                setFeedbackSnackbarContent('Max file count reached.', 'error', 'bottom');
                break;
              }
            ...
            this.setState((prevState) => ({
              createProject: {
                ...prevState.createProject,
                pitchSupportingDocuments: [
                  ...prevState.createProject.pitchSupportingDocuments,
                  fileRead,
                ],
              },
            }));
  ...
  handleFileError = (error, file) => {
    ...
    setFeedbackSnackbarContent(errorMessage, 'error', 'bottom');
  };
  ...
  handleCreatePitchInputChanged = (event) => {
    ...
    if (name === 'groupIssuerCreateOfferFor') {
      ...
      const index = AuthenticationState.groupsOfMembership.findIndex(
        (groupOfMembership) => groupOfMembership.group.anid === value
      );
      if (index === -1) {
        return;
      }
  ...
  uploadProject = () => {
    ...
    if (params.edit && projectEdited) {
      ...
      const projectRef = this.firebaseDB.ref(DB_CONST.PROJECTS_CHILD);
      ...
      this.uploadMultipleFiles(UPLOAD_PITCH_COVER_MODE, projectStorageRef).then((successful) => {
        this.setState({
          createProject: {
            ...this.state.createProject,
        ...
        this.uploadMultipleFiles(UPLOAD_PITCH_SUPPORTING_DOCUMENTS_MODE, projectStorageRef).then(
          (successful) => {
            this.setState({
              createProject: {
                ...this.state.createProject,
            ...
            this.uploadMultipleFiles(
              ...
              const updatedProject = {
                ...
                issuer: null,
                group: null,
                anid: isDraftProjectNotSubmitted(projectEdited)
                  ? groupIssuerCreateOfferFor !== 'undefined'
                    ? groupIssuerCreateOfferFor
                    : projectEdited.anid
                  : projectEdited.anid,
                projectName: saveProgress
                  ? pitchProjectName.trim().length === 0
                    ? null
                    : pitchProjectName
                  : pitchProjectName,
                ...
                edited: saveProgress ? null : utils.getCurrentDate(),
                PrimaryOffer: projectEdited.PrimaryOffer ? projectEdited.PrimaryOffer : null,
                Pitch: {
                  // financialRound:
                  //     saveProgress
                  //         ?
                  ...
                  //         pitchInvestorsCommitted
                  // ,
                  supportingDocuments: projectEdited.Pitch.supportingDocuments
                    ? [
                    ...
                    : this.pitchSupportingDocumentsRealtimeDB,
                  presentationText:
                    pitchPresentationText.ops.length === 0 ? null : pitchPresentationText,
                  presentationPlainText:
                    pitchPresentationText.ops.length === 0 ? null : pitchPresentationPlainText,
              ...
              projectRef
                ...
                .then(() => {
                  ...
                  if (!saveProgress) {
                    ...
                    if (projectEdited.status !== DB_CONST.PROJECT_STATUS_DRAFT) {
                      ...
                      realtimeDBUtils.trackActivity({
                        ...
                        activitySummary:
                          realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_PROJECT.replace(
                            '%project%',
                            projectEdited.projectName
                          ),
                        action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(
                          ':projectID',
                          projectEdited.id
                        ),
                        value: {
                      ...
                      realtimeDBUtils
                        .loadVotes(
                          projectEdited.id,
                          null,
                          realtimeDBUtils.LOAD_VOTES_ORDER_BY_PROJECT
                        )
                        .then((votes) => {
                          const listOfInvestorsToBeNotified = [];
                          votes.forEach((vote) => {
                          ...
                          realtimeDBUtils
                            .loadPledges(
                              projectEdited.id,
                              null,
                              realtimeDBUtils.LOAD_PLEDGES_ORDER_BY_PROJECT
                            )
                            .then((pledges) => {
                              pledges.forEach((pledge) => {
                                if (
                                  listOfInvestorsToBeNotified.findIndex(
                                    (investorID) => investorID === pledge.investorID
                                  ) === -1
                                ) {
                    ...
                    // a draft offer is published
                      ...
                      new Api()
                        .request('post', ApiRoutes.notifyAdminsOfResubmissionRoute, {
                      ...
                      this.firebaseDB
                        ...
                        .then(async () => {
                          ...
                          realtimeDBUtils.trackActivity({
                            ...
                            interactedObjectID: projectEdited.id,
                            activitySummary:
                              realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_CREATED_PROJECT.replace(
                                '%project%',
                  ...
                  // save button is clicked
                    ...
                    this.navigateToTheSamePageWithActiveStepSaved(activeStep, projectEdited.id);
                  }
                })
                .catch((error) => {
                  // handle error
                ...
                issuerID:
                  ...
                  params.admin && params.issuer
                    ...
                    : // in this case, it can be the issuer creating their own offer
                      ...
                      // main dashboard.
                      AuthenticationState.currentUser.id, // this can the id of the issuer or the group admin
                createdByGroupAdmin:
                  // a group admin is creating an offer on behalf of a known issuer
                  params.admin && params.issuer
                ...
                status: saveProgress
                  ...
                  : // if a group admin creates the offer
                    ...
                    : // if an issuer creates the offer
                      ...
                      DB_CONST.PROJECT_STATUS_BEING_CHECKED,
                Pitch: {
                  detailsAboutEarlierFundraisingRounds: saveProgress
                    ? detailsAboutEarlierFundraisingRounds.trim().length === 0
                      ? null
                      : detailsAboutEarlierFundraisingRounds
                    : detailsAboutEarlierFundraisingRounds,
              ...
              projectRef
                ...
                .then(() => {
                  ...
                  if (saveProgress) {
                    ...
                    this.navigateToTheSamePageWithActiveStepSaved(activeStep, projectID);
                  }
                  // publish button is clicked
                  else {
                    // Notify admins via notification bell of new project submission
    ...
    return new Promise((resolve, reject) => {
      ...
      uploadTask.on(
        ...
        (snapshot) => {
          ...
          if (
            mode === UPLOAD_PITCH_COVER_MODE ||
            mode === UPLOAD_PITCH_PRESENTATION_DOCUMENT_MODE
          ) {
            this.setState({
              createProject: {
                ...this.state.createProject,
            ...
            switch (mode) {
              case UPLOAD_PITCH_COVER_MODE:
                ...
                const formattedCover = {
                  url: fileDownloadURL,
                  fileType:
                    document.file.type === 'video/mp4'
                      ? DB_CONST.FILE_TYPE_VIDEO
                      : DB_CONST.FILE_TYPE_IMAGE,
              ...
              case UPLOAD_PITCH_SUPPORTING_DOCUMENTS_MODE:
                ...
                this.setState((prevState) => ({
                  createProject: {
                    ...prevState.createProject,
                    uploadFileProgress:
                      prevState.createProject.uploadFileProgress + percentDone.toFixed(0),
                  },
                }));
  ...
  handleDeleteDocument = (mode, index) => {
    switch (mode) {
      case PITCH_SUPPORTING_DOCUMENTS_FILES_CHANGED:
        const supportingDocuments = this.state.createProject.pitchSupportingDocuments;
  ...
  handleDescriptionChange = (mode, index, description) => {
    switch (mode) {
      case PITCH_SUPPORTING_DOCUMENTS_FILES_CHANGED:
        ...
        break;
      case PITCH_PRESENTATION_FILES_CHANGED:
        const presentationDocument = [...this.state.createProject.pitchPresentationDocument];
  ...
  handleDownloadDocumentDeleteClick = (index) => {
    ...
    const updatedProjectEdited = JSON.parse(JSON.stringify(projectEdited));
  ...
  handleDateChanged = (date) => {
    ...
    this.setState({
      createProject: {
        ...this.state.createProject,
        pitchExpiryDate: !date ? null : date.getTime(),
      },
    });
  };
  ...
  handleSaveProgressClick = () => {
    ...
    if (activeStep === STEP_PITCH_GENERAL_INFORMATION) {
      ...
      if (
        ...
        pitchExpiryDate === null &&
        //&& financialRound.trim().length === 0
        ManageGroupUrlState.groupNameFromUrl === 'qib' &&
        qibSpecialNews.trim().length === 0
      ) {
        setFeedbackSnackbarContent('You have nothing to save.', 'error', 'bottom');
        return;
      }

      // check if the entered date is in a valid format or the entered date is less than the minimum date
      if (
        isNaN(pitchExpiryDate) ||
        (pitchExpiryDate && pitchExpiryDate < utils.getDateWithDaysFurtherThanToday(0))
      ) {
        setFeedbackSnackbarContent('Please enter a valid date.', 'error', 'bottom');
        return;
      }
    }
  ...
  handleDeleteDraftClick = () => {
    ...
    this.deleteMultiplesOnStorage(UPLOAD_PITCH_COVER_MODE).then(() => {
      // delete project summary
      this.deleteMultiplesOnStorage(UPLOAD_PITCH_PRESENTATION_DOCUMENT_MODE).then(() => {
        // delete project supporting documents
        this.deleteMultiplesOnStorage(UPLOAD_PITCH_SUPPORTING_DOCUMENTS_MODE).then(() => {
          this.firebaseDB
            .ref(DB_CONST.PROJECTS_CHILD)
            .child(projectEdited.id)
            .remove()
  ...
  deleteMultiplesOnStorage = async (mode) => {
    ...
    switch (mode) {
      case UPLOAD_PITCH_COVER_MODE:
        ...
        storageRef = storageRef.child(DB_CONST.PROJECT_COVER_CHILD);
        return Promise.all(
      ...
      case UPLOAD_PITCH_PRESENTATION_DOCUMENT_MODE:
        ...
        storageRef = storageRef.child(DB_CONST.PROJECT_PRESENTATION_DOCUMENT_CHILD);
        return Promise.all(
      ...
      case UPLOAD_PITCH_SUPPORTING_DOCUMENTS_MODE:
        ...
        storageRef = storageRef.child(DB_CONST.PROJECT_SUPPORTING_DOCUMENTS_CHILD);
        return Promise.all(
  ...
  render() {
    ...
    const { ManageSystemAttributesState, ManageGroupUrlState, AuthenticationState, isMobile } =
      this.props;
    ...
    if (params.edit) {
      ...
      const currentGroupUserName = ManageGroupUrlState.group
        ? ManageGroupUrlState.group.groupUserName
        : null;
      const currentGroupAnid = ManageGroupUrlState.group ? ManageGroupUrlState.group.anid : null;
      const projectBelongsToCurrentGroup =
        projectEdited.anid === currentGroupUserName ||
      ...
      if (
        ...
        return (
          <FlexView marginTop={50} hAlignContent="center">
            <Typography variant="h4" align="center">
...
class CreateProject extends Component {
  ...
  renderPitchPublishErrorMessage = () => {
    const { groupUserName } = this.props;
    ...
    switch (this.props.createProjectState.pitchPublishCheck) {
      ...
      case PITCH_PUBLISH_FALSE_MISSING_PITCH_PRESENTATION:
        ...
        // for other groups, project summary is still project summary
          ...
          msg =
            'Please upload a file or use our provided text editor to make your project presentation.';
        }
        break;
      default:
        return null;
      ...
      <Container fluid style={{ padding: 0 }}>
        ...
        <Row noGutters style={{ marginTop: 40, padding: 20 }}>
          <Col xs={12} sm={12} md={{ span: 8, offset: 2 }} lg={{ span: 8, offset: 2 }}>
            <FlexView hAlignContent="center" vAlignContent="center">
              <Stepper
                activeStep={createProjectState.activeStep}
                alternativeLabel
                style={{ width: isMobile ? 'auto' : '100%', padding: 0 }}
              >
                <Step key={0}>
                ...
                {params.edit && projectEdited ? (
                  projectEdited.status !== DB_CONST.PROJECT_STATUS_DRAFT ? null : (
                    <Step key={4}>
          ...
          {createProjectState.activeStep === STEP_PITCH_GENERAL_INFORMATION ? (
            ...
            >
              ...
              <FlexView column>
                ...
                'undefined' ? null : params.edit &&
                  projectEdited &&
                  !isDraftProjectNotSubmitted(projectEdited) ? null : !isIssuer(
                    AuthenticationState.currentUser
                  ) &&
                  !isInvestor(
                    AuthenticationState.currentUser
                  ...
                  <FlexView marginTop={30}>
                    <FormControl fullWidth required>
                      ...
                      >
                        {AuthenticationState.groupsOfMembership.map((groupOfMembership) => (
                          <MenuItem
                            key={groupOfMembership.group.anid}
                            value={groupOfMembership.group.anid}
                          >
                            {groupOfMembership.group.displayName}
                          </MenuItem>
                        ))}
                      </Select>
                    </FormControl>
                  ...
                  >
                    ...
                    <Select
                      ...
                      value={
                        ...
                        (clubAttributes.Sectors.includes(createProjectState.pitchSector) ||
                          createProjectState.pitchSector === '-')
                          ? createProjectState.pitchSector
                          : '-'
                      }
                      ...
                      <Col xs={12} sm={12} md={8} lg={6}>
                        {!params.edit || !projectEdited ? (
                          ...
                          <FlexView column>
                            ...
                            {currentUser.type !==
                            DB_CONST.TYPE_ADMIN ? null : projectEdited.status ===
            ...
            >
              <FlexView column width="100%">
                <FlexView vAlignContent="center">
                  <Typography color="primary" variant="h6" align="left" style={{ marginRight: 15 }}>
                    Step 2: Upload project cover
                  </Typography>
                  <InfoOverlay
                    message="project cover is an image or video that will appear on your project summary tile."
                    placement="right"
                  />
                </FlexView>

                {!params.edit || !projectEdited ? null : !projectEdited.Pitch.hasOwnProperty(
                    'cover'
                  ) ? null : (
                    ...
                    {projectEdited.Pitch.cover.map((coverItem, index) =>
                      coverItem.hasOwnProperty('removed') ? null : coverItem.fileType ===
                        ...
                        <ReactPlayer
                          ...
                          url={coverItem.url}
                          width="100%"
                          height={coverItem.storageID === '' ? 280 : 'auto'}
                          playing={false}
                          controls={true}
                        />
                      ) : (
                ...
                {groupUserName !== 'qib' ? null : (
                  <Typography
                    ...
                    style={{ marginTop: 8 }}
                  >
                    Please upload your project logo here.
                  </Typography>
                )}
                ...
                <FlexView column marginTop={32}>
                  <FlexView>
                    <Button
                      ...
                      color="primary"
                      onClick={() =>
                        this.onSelectPitchCoverTypeClick(PITCH_COVER_FILE_TYPE_SELECTED)
                      }
                    >
                    ...
                    <FlexView marginLeft={20} vAlignContent="center">
                      ...
                      <InfoOverlay
                        message="If your company has a short explainer or introductory video, we suggest you embed it here."
                        placement="right"
                      />
                    </FlexView>
                  </FlexView>
                  ...
                  <FlexView column marginTop={20}>
                    {!createProjectState.pitchCoverTypeSelected ? null : createProjectState.pitchCoverTypeSelected ===
                      PITCH_COVER_FILE_TYPE_SELECTED ? (
                      <FlexView column marginTop={10}>
                        <Files
                          className={css(styles.file_drop_zone)}
                          onChange={this.onFilesChanged(PITCH_COVER_FILES_CHANGED)}
                        ...
                        <Typography
                          ...
                          style={{ marginTop: 11 }}
                        >
                          <u>Note:</u> project cover must not exceed{' '}
                          {DB_CONST.MAX_VIDEO_OR_IMAGE_SIZE_IN_MB}MB.
                        </Typography>
                      </FlexView>
                    ) : (
                      <FlexView column>
                        <TextField
                          name="pitchCoverVideoURL"
                          value={createProjectState.pitchCoverVideoURL}
                    ...
                    createProjectState.pitchCoverVideoURL.trim().length > 0 ? (
                      <FlexView column marginTop={35} height={280}>
                        <ReactPlayer
                          url={createProjectState.pitchCoverVideoURL}
                          playsInline
                          width="100%"
                          height="100%"
                          playing={false}
                ...
                <FlexView column>
                  ...
                  <FlexView column marginTop={30} marginBottom={30}>
                    ...
                    <Typography
                      ...
                      style={{ marginTop: 11 }}
                    >
                      <u>Note:</u> The one-pager must not exceed{' '}
                      {DB_CONST.MAX_VIDEO_OR_IMAGE_SIZE_IN_MB}MB. Files in formats .pdf, .doc,
                      .docx, .ppt, and .pptx are accepted only.
                    </Typography>
                    ...
                    <UploadDocuments
                      ...
                      onDescriptionChange={this.onDescriptionChange(
                        PITCH_PRESENTATION_FILES_CHANGED
                      )}
                    />
                  </FlexView>
                  ...
                  <FlexView column marginBottom={60}>
                    <Typography variant="body1" align="left" component="span">
                      We recommend the following headings for your one-page, one-sided summary:
                      <ul>
                        <li>Problem</li>
                        <li>Solution</li>
                ...
                <FlexView column>
                  ...
                  <FlexView column marginTop={30} marginBottom={60}>
                    ...
                    <Typography
                      ...
                      style={{ marginTop: 11 }}
                    >
                      <u>Note:</u> Project summary must not exceed{' '}
                      {DB_CONST.MAX_VIDEO_OR_IMAGE_SIZE_IN_MB}MB. Files in formats .pdf, .doc,
                      .docx, .ppt, and .pptx are accepted only.
                    </Typography>
                  ...
                  <FlexView column marginTop={20}>
                    <Typography variant="body1" paragraph align="left">
                      Please add a more detailed explanation of your project
                    </Typography>
                ...
                <FlexView column>
                    ...
                    <FlexView column marginTop={25} marginBottom={25}>
                      ...
                      projectEdited.Pitch.supportingDocuments.filter(
                        ...
                        <FlexView column>
                          ...
                          <DocumentsDownload
                            documents={projectEdited.Pitch.supportingDocuments}
                            deleteEnable
                            onDeleteDocument={this.onDownloadDocumentDeleteClick}
                          />
                        </FlexView>
                      ) : (
                        <Typography variant="body1" color="textSecondary" align="left">
                  ...
                  <FlexView column marginTop={25}>
                    ...
                    <Typography
                      ...
                      style={{ marginTop: 11 }}
                    >
                      <u>Note:</u> Each document must not exceed {DB_CONST.MAX_FILE_SIZE_IN_MB}MB.
                      Files in formats .pdf, .doc, .docx, .ppt, .pptx, .xls, and .xlsx are accepted
                      only.
                    </Typography>
                    ...
                    <UploadDocuments
                      ...
                      onDeleteDocument={this.onDeleteDocument(
                        PITCH_SUPPORTING_DOCUMENTS_FILES_CHANGED
                      )}
                      onDescriptionChange={this.onDescriptionChange(
                        PITCH_SUPPORTING_DOCUMENTS_FILES_CHANGED
                      )}
                  ...
                  {!params.edit || !projectEdited ? null : utils.isDraftProject(projectEdited) &&
                    ...
                    <FlexView column marginTop={25} marginBottom={25}>
                      ...
                      projectEdited.Pitch.supportingDocuments.filter(
                        ...
                        <Typography variant="body1" color="textSecondary" align="left">
                          You have no supporting documents.
                        </Typography>
                      )}
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        ...
                        value={createProjectState.acceptedTermsAndConditions}
                        checked={createProjectState.acceptedTermsAndConditions}
        ...
        <Row noGutters>
          <Col
            ...
            md={{ span: 10, offset: 1 }}
            lg={{ span: 6, offset: 3 }}
            style={{ marginTop: 20, marginBottom: 120, padding: 18 }}
          >
            {params.edit && projectEdited ? (
              projectEdited.status === DB_CONST.PROJECT_STATUS_DRAFT ? (
                createProjectState.activeStep === STEP_ACCEPT_TERMS_AND_CONDITIONS ? (
                  !createProjectState.acceptedTermsAndConditions ? null : (
                    <Typography
                      color="error"
                      variant="body1"
                      align="right"
                    ...
                    >
                      Please check everything carefully again before you submit.
                    </Typography>
                  )
                ) : null
              ) : createProjectState.activeStep === STEP_PITCH_SUPPORTING_DOCUMENTS ? (
                <Typography
                  ...
                  style={{ marginBottom: 25 }}
                >
                  Please check everything carefully again before publishing.
                </Typography>
              ) : null
            ) : createProjectState.activeStep === STEP_ACCEPT_TERMS_AND_CONDITIONS ? (
              !createProjectState.acceptedTermsAndConditions ? null : (
            ...
            <FlexView width="100%" hAlignContent="right">
              <Button
                ...
                disabled={createProjectState.activeStep === STEP_PITCH_GENERAL_INFORMATION}
                size="large"
                className={css(sharedStyles.no_text_transform)}
              >
                Back
              ...
              <Button
                ref={this.attachNextStepButtonRef}
                variant="outlined"
                color="primary"
              ...
              <Overlay
                show={createProjectState.pitchPublishErrorMessageShowed}
                target={createProjectState.pitchNextStepButtonTarget}
                placement="bottom"
              >
            ...
            <FlexView column>
              <div>
                <OverlayTrigger
                  ...
                  placement="left"
                  flip
                  overlay={<Tooltip id={`tooltip-left`}>Save draft</Tooltip>}
                >
                  <Fab
                    color="primary"
                    size={isMobile ? 'medium' : 'large'}
                    onClick={this.onSaveProgressClick}
                    disabled={saveProgress && progressBeingSaved}
              ...
              projectEdited.status === DB_CONST.PROJECT_STATUS_DRAFT ? (
                <div style={{ marginTop: isMobile ? 62 : 72 }}>
                  <OverlayTrigger
                    ...
                    placement="left"
                    flip
                    overlay={<Tooltip id={`tooltip-left`}>Delete this draft</Tooltip>}
                  >
                    <Fab
                      size={isMobile ? 'medium' : 'large'}
                      onClick={this.onDeleteDraftClick}
                      disabled={draftBeingDeleted}
                      style={{ position: 'absolute', zIndex: 1 }}
                    >
                  ...
                  {draftBeingDeleted ? (
                    <CircularProgress
                      ...
                      thickness={2.7}
                      size={isMobile ? 60 : 68}
                      style={{ position: 'absolute', top: isMobile ? 56 : 66, left: -6 }}
                    />
                  ) : null}
                </div>
              ) : null}
...
class UploadingDialog extends Component {
        ...
        <DialogContent>
          <FlexView column>
            ...
            <FlexView column marginBottom={20}>
              ...
              {uploadFileMode === UPLOAD_DONE_MODE ? null : (
                <ProgressBar
                  style={{ marginTop: 25 }}
                  striped
                  variant="success"
                  now={uploadFileProgress}
                  label={`${parseInt(uploadFileProgress)}%`}
                />
              )}
            </FlexView>
          </FlexView>
  ...
  renderUploadFileMode = () => {
    const { groupUserName, uploadFileMode, params, projectEdited, currentUser } = this.props;

    switch (uploadFileMode) {
      case UPLOAD_PITCH_COVER_MODE:
        return (
          <Typography variant="body2" color="textSecondary" align="left">
            Uploading project cover
          </Typography>
        );
      case UPLOAD_PITCH_SUPPORTING_DOCUMENTS_MODE:
        return (
          <Typography variant="body2" color="textSecondary" align="left">
            Uploading project supporting documents
          </Typography>
        );
      case UPLOAD_PITCH_PRESENTATION_DOCUMENT_MODE:
        return (
          <Typography variant="body2" color="textSecondary" align="left">
            Uploading project presentation document
          </Typography>
        );
          ...
          if (projectEdited.status !== DB_CONST.PROJECT_STATUS_DRAFT) {
            ...
            return (
              <FlexView column>
                ...
                <br />
                <Typography variant="body1" color="textSecondary" align="left">
                  Invite others to upload their projects using following link:
                </Typography>
                <br />
                <Box border={1} bgcolor={colors.gray_100} padding="6px">
                  <CustomLink
                    url={Routes.constructSignUpRoute(groupUserName)}
                    target="_blank"
                    color="none"
                    activeColor="none"
