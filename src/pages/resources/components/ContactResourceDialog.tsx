...
class ContactResourceDialog extends Component<ContactResourceDialogProps, any> {
  render() {
    const { resourceName, resourceEmail, ResourcesLocalState, toggle, sendContactEmail } =
      this.props;

    return (
      <Dialog open={ResourcesLocalState.detailedPageContactDialogOpen} onClose={() => toggle()}>
        <DialogContent>
        ...
        <DialogActions>
          {isDetailedPageSendingContactEmail(ResourcesLocalState) ? null : (
            <Button onClick={() => toggle()} className={css(sharedStyles.no_text_transform)}>
              Cancel
            </Button>
          )}
          <Button
