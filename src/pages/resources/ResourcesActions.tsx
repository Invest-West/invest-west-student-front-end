...
export const sendContactEmail: ActionCreator<any> = (receiver: string | string[]) => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { detailedPageSendingContactEmail } = getState().ResourcesLocalState;
    ...
    try {
      const emailInfo: ContactResourceEmailData = {
        ...
        userCompanyName: `${
          admin
            ? 'Not specified'
            : hasBusinessProfile(currentUser as User)
              ? (currentUser as User).BusinessProfile?.companyName
              : 'Not specified.'
        }`,
      };
