...
const groupDetailsReducer = (state = initialState, action: GroupDetailsAction) => {
  switch (action.type) {
    ...
    case GroupDetailsEvents.CompleteLoadingData:
      const completeLoadingDataAction: CompleteLoadingDataAction =
        action as CompleteLoadingDataAction;
      return {
        ...state,
        loadingData: false,
        dataLoaded: true,
        group:
          completeLoadingDataAction.group !== undefined
            ? JSON.parse(JSON.stringify(completeLoadingDataAction.group))
            : state.group,
        members:
          completeLoadingDataAction.members !== undefined
            ? [...completeLoadingDataAction.members]
            : state.members,
        offers:
          completeLoadingDataAction.offers !== undefined
            ? [...completeLoadingDataAction.offers]
            : state.offers,
        admins:
          completeLoadingDataAction.admins !== undefined
            ? [...completeLoadingDataAction.admins]
            : state.admins,
        accessRequestsInstances:
          completeLoadingDataAction.accessRequestInstances !== undefined
            ? [...completeLoadingDataAction.accessRequestInstances]
            : state.accessRequestsInstances,
        error:
          completeLoadingDataAction.error !== undefined
