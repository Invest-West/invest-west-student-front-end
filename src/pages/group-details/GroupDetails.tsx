...
import {
  validateGroupUrl,
  resetGroupUrlState,
} from '../../redux-store/actions/manageGroupUrlActions';
...
const mapDispatchToProps = (dispatch: ThunkDispatch<any, any, AnyAction>) => {
  return {
    ...
    openFeedbackSnackbar: (type: FeedbackSnackbarTypes, message: string) =>
...
> {
  ...
  isCourse = () => {
    ...
    return (
      GroupDetailsLocalState.group?.parentGroupId != null ||
  ...
  render() {
    ...
    // error
    if (
      hasErrorLoadingData(GroupDetailsLocalState) ||
      !successfullyLoadedData(GroupDetailsLocalState)
    ) {
      return (
        <Box display="flex" justifyContent="center" alignItems="center" marginTop="50px">
          <Typography variant="h5" color="error" align="center">
            Error loading page. Please retry.
          </Typography>
        </Box>
      );
    }
    ...
    if (hasAccessRequestsBeenSatisfied(GroupDetailsLocalState)) {
      hasRequestedToAccessGroup =
        GroupDetailsLocalState.accessRequestsInstances?.findIndex(
          (accessRequestInstance) =>
            accessRequestInstance.group.anid === GroupDetailsLocalState.group?.anid
        ) !== -1;
    }
    ...
    // successfully loaded
    return (
      <Box paddingY={MediaQueryState.isMobile ? '15px' : '40px'}>
        {/** Header section */}
        <Row noGutters>
                    ...
                    >
                      ...
                      >
                        <Typography align="center" variant="h4">
                          {GroupDetailsLocalState.group?.displayName}
                        </Typography>

                        {/** Home/platform member + joined date (available for investor and issuer) */}
                        {currentAdmin ? null : !groupMember ? (
                          <Box marginTop="25px">
                            {!hasRequestedToAccessGroup ? (
                              <Button
                                variant="outlined"
                                ...
                                disabled={isSendingAccessRequest(GroupDetailsLocalState)}
                              >
                              ...
                              <Button
                                variant="outlined"
                                className={css(sharedStyles.no_text_transform)}
                                onClick={() => removeAccessRequest()}
                                disabled={isRemovingAccessRequest(GroupDetailsLocalState)}
                              >
                          ...
                          <Box marginTop="25px">
                            <Box display="flex" flexDirection="row">
                              ...
                              <Typography variant="body1" align="center" color="textSecondary">
                                {getHomeGroup(AuthenticationState.groupsOfMembership)?.group
                                  .anid === groupMember.group.anid
                                  ? 'Home member'
                                  : 'Platform member'}
                              </Typography>
                            </Box>
                            ...
                            <Box marginTop="5px">
                              <Typography variant="body1" align="center" color="textSecondary">
                                Joined on: {dateInReadableFormat(groupMember.joinedDate)}
                              </Typography>
                            </Box>
                          </Box>
                        )}
                ...
                <Box padding="20px">
                  ...
                  <Box marginTop="18px" whiteSpace="pre-line">
                    <Typography variant="body1" align="left">
                      {GroupDetailsLocalState.group?.description}
                    </Typography>
                  </Box>
                  ...
                  <Box marginTop="18px">
                    <Typography variant="body1" align="left">
                      ...
                      <CustomLink
                        ...
                        component="a"
                        childComponent={GroupDetailsLocalState.group?.website ?? 'unknown'}
                      />
                    </Typography>
                  </Box>
                </Box>
                    ...
                    <Row>
                      <Col xs={12} sm={12} md={6} lg={6}>
                        <Box padding="18px">
                          <Typography variant="h4" align="left">
                            {GroupDetailsLocalState.members?.length ?? 0}
                          </Typography>
                          <Box height="2px" />
                          <Typography variant="body1" align="left">
                            Students
                      ...
                      <Col xs={12} sm={12} md={6} lg={6}>
                        <Box display="flex" flexDirection="row">
                          ...
                          <Box padding="18px">
                            <Typography variant="h4" align="left">
                              {GroupDetailsLocalState.offers?.length ?? 0}
                            </Typography>
                    ...
                    <Row>
                      <Col xs={12} sm={12} md={6} lg={6}>
                        <Box padding="18px" borderTop={`1px solid ${colors.grey['300']}`}>
                          <Typography variant="h4" align="left">
                            {GroupDetailsLocalState.admins?.length ?? 0}
                          </Typography>
                          <Box height="2px" />
                          <Typography variant="body1" align="left">
                            Lecturers / Admins
                      ...
                      {(GroupDetailsLocalState.group?.subGroups &&
                        GroupDetailsLocalState.group.subGroups.length > 0) ||
                      (GroupDetailsLocalState.group?.childGroups &&
                        GroupDetailsLocalState.group.childGroups.length > 0) ? (
                        <Col xs={12} sm={12} md={6} lg={6}>
                          <Box display="flex" flexDirection="row">
                            <Box>
                              <Divider orientation="vertical" />
                            ...
                            <Box padding="18px" borderTop={`1px solid ${colors.grey['300']}`}>
                              <Typography variant="h4" align="left">
                                {GroupDetailsLocalState.group?.childGroups?.length ??
                                  GroupDetailsLocalState.group?.subGroups?.length ??
                                  0}
                              </Typography>
        ...
        {GroupDetailsLocalState.group && (
          <EditGroupDetailsDialog
            ...
            groupUserName={
              this.isCourse()
                ? this.getParentGroupUserName()
                : GroupDetailsLocalState.group.groupUserName
            }
            courseUserName={
              this.isCourse() ? GroupDetailsLocalState.group.groupUserName : undefined
            }
