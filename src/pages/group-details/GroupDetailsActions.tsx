...
export const loadData: ActionCreator<any> = (groupUserName: string) => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { accessRequestsInstances } = getState().GroupDetailsLocalState;
    ...
    try {
      ...
      try {
        ...
        const courseAdmins = (await realtimeDBUtils.loadGroupAdminsBasedOnGroupID(
          group.anid
        )) as any[];
        ...
        if (snapshot.exists()) {
          ...
          adminsWithCourseId = allAdmins.filter(
            (admin: any) =>
              admin.courseIds &&
              Array.isArray(admin.courseIds) &&
              admin.courseIds.includes(group.anid)
          );
        }
        console.log(
          `    ✅ Found ${adminsWithCourseId.length} admins with courseIds containing ${group.anid}`
        );
        ...
        console.log(
          `    📊 Total unique admins for THIS COURSE ONLY (${group.displayName}): ${uniqueAdmins.length}`
        );
...
export const removeAccessRequest: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    try {
      const currentAccessRequestInstances: AccessRequestInstance[] | undefined =
        getState().GroupDetailsLocalState.accessRequestsInstances;
      if (!currentAccessRequestInstances) {
        return dispatch(completeAction);
      }
      const accessRequestIndex = currentAccessRequestInstances.findIndex(
        (accessRequestInstance) =>
          accessRequestInstance.group.anid === group.anid &&
          accessRequestInstance.user.id === currentUser.id
      );
      if (accessRequestIndex === -1) {
        return dispatch(completeAction);
      }
      const updatedAccessRequestInstances: AccessRequestInstance[] = [
        ...currentAccessRequestInstances,
      ];
