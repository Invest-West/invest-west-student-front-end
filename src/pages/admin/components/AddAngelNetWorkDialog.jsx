...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleInputChanged: (event) =>
      dispatch(addAngelNetworkDialogActions.handleAddAngelNetworkInputChanged(event)),
    addNewAngelNetwork: () => dispatch(addAngelNetworkDialogActions.addNewAngelNetwork()),

    handleImageFilesChanged: (mode, files) =>
      dispatch(addAngelNetworkDialogActions.handleImageFilesChanged(mode, files)),
    handleImageFileError: (mode, error, file) =>
      dispatch(addAngelNetworkDialogActions.handleImageFileError(mode, error, file)),
    ...
    resetImageFileErrorMessageWhenSnackbarExited: () =>
      dispatch(addAngelNetworkDialogActions.resetImageFileErrorMessageWhenSnackbarExited()),
  };
};
...
class AddAngelNetworkDialog extends Component {
  ...
  render() {
    ...
    return (
      <Dialog
        ref={forwardedRef}
        open={addAngelNetworkDialogOpen}
        fullWidth
        maxWidth="md"
        ...
        <DialogContent style={{ marginTop: 10 }}>
          ...
          <FormControl fullWidth>
            <TextField
              ...
              onChange={handleInputChanged}
            />
            {website.trim().length === 0 || utils.isValidWebURL(website) ? null : (
          ...
          <FormControl fullWidth style={{ marginTop: 25 }}>
            <TextField
              ...
              onChange={handleInputChanged}
            />
            {primaryColor.trim().length === 0 || utils.isValidHexColorCode(primaryColor) ? null : (
              <Typography variant="body2" color="error" align="left">
                Invalid color
              </Typography>
            )}
          ...
          <FlexView column>
            ...
            >
              <FlexView vAlignContent="center" hAlignContent="center">
                ...
                Upload plain logo (with no text)
              </FlexView>
            </Files>
            ...
            {!plainLogo ? null : (
              <FlexView marginTop={20} width={270} height={270}>
                <Image
                  src={window.URL.createObjectURL(plainLogo)}
                  roundedCircle
                  thumbnail
          ...
          <FlexView column marginTop={25}>
            ...
            {!logoWithText ? null : (
              <FlexView marginTop={20} width={540} height={270}>
                <Image
                  src={window.URL.createObjectURL(logoWithText)}
                  thumbnail
                  style={{ width: '100%', height: 'auto', objectFit: 'fill' }}
                />
              </FlexView>
   ...
   */
  renderRespondCode = () => {
    const { response, uploadMode, uploadProgress } = this.props;
    ...
    switch (response) {
      ...
      case ADD_STATUS_SUCCESSFULLY_ADDED:
        res.msg =
          'Added successfully. An email has been sent to the angel network for login information.';
        res.color = 'primary';
        break;
