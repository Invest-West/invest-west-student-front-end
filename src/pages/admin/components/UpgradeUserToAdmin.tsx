...
class UpgradeUserToAdmin extends Component<UpgradeUserToAdminProps, UpgradeUserToAdminState> {
  ...
  private getAvailableCoursesForUniversity = (
    universityId: string,
    systemGroups: GroupProperties[]
  ): GroupProperties[] => {
    ...
    const virtualCourses = availableCoursesStrings
      .filter(
        (courseName) =>
          !actualCourses.some(
            (course) => course.displayName.toLowerCase() === courseName.toLowerCase()
          )
      )
      .map((courseName) =>
        this.createVirtualCourseGroup(courseName, universityId, university.groupUserName)
      );
  ...
  private createVirtualCourseGroup = (
    courseName: string,
    universityId: string,
  ...
  constructor(props: UpgradeUserToAdminProps) {
    ...
    this.state = {
      ...
      searchEmail: '',
      selectedUniversity:
        props.currentAdmin?.superAdmin || props.currentAdmin?.superGroupAdmin
          ? ''
          : isUniversity(props.currentGroup)
            ? props.currentGroup?.anid || ''
            : props.currentGroup?.parentGroupId || '',
      selectedCourse:
        props.currentAdmin?.superAdmin || props.currentAdmin?.superGroupAdmin
          ? ''
          : !isUniversity(props.currentGroup)
  ...
  componentDidMount() {
    ...
    if (this.props.currentGroup && isUniversity(this.props.currentGroup)) {
      const availableCourses = this.getAvailableCoursesForUniversity(
        this.props.currentGroup.anid,
        this.props.systemGroups
      );
  ...
  componentDidUpdate(prevProps: UpgradeUserToAdminProps) {
    ...
    if (
      prevProps.systemGroups !== this.props.systemGroups ||
  ...
  searchUser = async () => {
    ...
    try {
      ...
      this.setState({
        ...
        searchResults,
        upgradeStatus:
          searchResults.length > 0 ? UPGRADE_USER_STATUS_NONE : UPGRADE_USER_STATUS_USER_NOT_FOUND,
  ...
  mockUserSearch = async (email: string): Promise<any[]> => {
    try {
      ...
      const groupFilter =
        currentAdmin?.superAdmin || currentAdmin?.superGroupAdmin ? null : currentAdmin?.anid;
      ...
      const matchingUsers = invitedUsers.filter((user: InvitedUser) =>
      ...
      for (const group of groupsToCheck) {
        try {
          const groupAdmins = (await realtimeDBUtils.loadGroupAdminsBasedOnGroupID(
            group.anid
          )) as any[];

          const adminMatches = groupAdmins.filter(
            (admin: any) => admin.email && admin.email.toLowerCase().includes(email.toLowerCase())
          );
      ...
      if (userGroup) {
        ...
        if (isCourse(userGroup)) {
          ...
          if (autoSelectedUniversity) {
            availableCourses = this.getAvailableCoursesForUniversity(
  ...
  upgradeUserToAdmin = async () => {
    ...
    if (
      !selectedUser ||
      !targetCourseId ||
      (!targetUniversityId && (currentAdmin?.superAdmin || currentAdmin?.superGroupAdmin))
    ) {
      const missingFields = [];
      if (!selectedUser) missingFields.push('user');
      if ((currentAdmin?.superAdmin || currentAdmin?.superGroupAdmin) && !targetUniversityId)
    ...
    if (!targetCourseProperties && targetUniversityId) {
      const allCoursesForUniversity = this.getAvailableCoursesForUniversity(
        targetUniversityId,
        systemGroups
      );
      targetCourseProperties = allCoursesForUniversity.find(
        (course) => course.anid === targetCourseId
      );
      isVirtualCourse = targetCourseProperties && (targetCourseProperties as any).isVirtual;
    }

    const targetUniversityProperties = systemGroups.find(
      (group) => group.anid === targetUniversityId
    );
    ...
    try {
      ...
      const isAlreadyAdminOfTargetCourse =
        selectedUser.isAdmin && selectedUser.adminOfGroup?.anid === targetCourseProperties.anid;

      const isAlreadyAdminOfTargetUniversity =
        selectedUser.isAdmin && selectedUser.adminOfGroup?.anid === targetUniversityId;
      ...
      try {
        ...
        if (isVirtualCourse) {
          ...
          const currentUniversityAdmins = (await realtimeDBUtils.loadGroupAdminsBasedOnGroupID(
            targetUniversityId
          )) as any[];
          existingAdmin = currentUniversityAdmins.find(
            (admin: any) =>
          ...
          const currentGroupAdmins = (await realtimeDBUtils.loadGroupAdminsBasedOnGroupID(
            targetCourseProperties.anid
          )) as any[];
          existingAdmin = currentGroupAdmins.find(
            (admin: any) =>
              admin.email && admin.email.toLowerCase() === selectedUser.email.toLowerCase()
          );
      ...
      if (isVirtualCourse) {
        ...
        await this.api.request('post', ApiRoutes.addGroupAdminRoute, {
        ...
        if (targetUniversityProperties && targetUniversityId !== targetCourseId) {
          try {
            ...
            console.warn(
              'Error adding user to university (might already be a member):',
              universityError
            );
      ...
      if (
        errorMessage &&
        (errorMessage.includes('already been used') || errorMessage.includes('already exists'))
      ) {
        this.setState({
          upgradeStatus: UPGRADE_USER_STATUS_ALREADY_ADMIN,
          statusMessage: `${selectedUser.email} is already an admin of ${targetCourseProperties.displayName}.`,
          ...
          <Table size="small">
            ...
            <TableBody>
              {searchResults.map((user, index) => (
                <TableRow key={index} selected={selectedUser?.id === user.id}>
                  <TableCell>
                    {user.isAdmin
  ...
  render() {
    ...
    const availableUniversities =
      currentAdmin?.superAdmin || currentAdmin?.superGroupAdmin
        ? getUniversities(systemGroups)
        : isUniversity(currentGroup)
          ? [currentGroup]
          : [];
        ...
        <Dialog open={dialogOpen} onClose={this.toggleDialog} maxWidth="md" fullWidth>
          ...
          <DialogContent>
            <Box display="flex" flexDirection="column" style={{ gap: '16px' }}>
                ...
                <Box>
                  ...
                  <Paper elevation={1} style={{ padding: 16 }}>
                    ...
                    <Typography variant="body2">
                      ...
                      {selectedUser.status === 1 ? 'Active' : 'Not Registered'}
                    </Typography>
                    <Typography variant="body2">
                    ...
                    {selectedUser.isAdmin && (
                      ...
                      >
                        <Typography variant="caption" style={{ color: '#1976d2' }}>
                          ℹ️ This user is currently an admin of{' '}
                          {selectedUser.adminOfGroup?.displayName}. They can be upgraded to admin of
                          additional courses.
                        </Typography>
                      </Box>
                    )}
                    ...
                    {!selectedUser.isAdmin &&
                      selectedUser.Invitor &&
                      selectedCourse &&
                      selectedUser.Invitor.anid === selectedCourse && (
                        <Box
                          marginTop={1}
                          padding={1}
                          style={{ backgroundColor: '#e3f2fd', borderRadius: 4 }}
                        >
                          <Typography variant="caption" style={{ color: '#1976d2' }}>
                            ℹ️ This user is already a member of the selected course and will be
                            upgraded to admin.
                          </Typography>
                        </Box>
                      )}
                    ...
                    {!selectedUser.isAdmin &&
                      ...
                      !selectedCourse &&
                      currentGroup &&
                      selectedUser.Invitor.anid === currentGroup.anid && (
                        <Box
                          marginTop={1}
                          padding={1}
                          style={{ backgroundColor: '#e3f2fd', borderRadius: 4 }}
                        >
                          <Typography variant="caption" style={{ color: '#1976d2' }}>
                            ℹ️ This user is already a member of your course and will be upgraded to
                            admin.
                          </Typography>
                        </Box>
                      )}
              ...
              {/* Status Message */}
              <Box>{this.renderStatusMessage()}</Box>
            </Box>
          </DialogContent>
