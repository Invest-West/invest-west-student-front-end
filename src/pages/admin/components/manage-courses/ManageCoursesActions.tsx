...
export const loadCoursesFromGroup: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const groupProperties: GroupProperties | null =
      getState().manageGroupFromParams.groupProperties;
    if (!groupProperties) {
    ...
    try {
      ...
      const coursesSnapshot = await firebase.database().ref(DB_CONST.COURSES_CHILD).once('value');
      ...
      if (coursesSnapshot.exists()) {
        ...
        Object.keys(coursesObject).forEach((key) => {
          ...
          if (
            course.parentGroupId === groupProperties.anid &&
        ...
        console.log(
          `[ManageCourses] Loaded ${courseNames.length} courses from Courses node for: ${groupProperties.displayName}`
        );
      } else {
...
export const addNewCourse: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    try {
      ...
      const courseId = firebase.database().ref(DB_CONST.GROUP_PROPERTIES_CHILD).push().key;
      ...
      const courseGroupProperties: GroupProperties = {
        ...
        settings: {
          ...
          makeInvestorsContactDetailsVisibleToIssuers:
            groupProperties.settings.makeInvestorsContactDetailsVisibleToIssuers,
        },
      ...
      dispatch(
        openFeedbackSnackbar(
          FeedbackSnackbarTypes.Success,
          `Course "${newCourse}" created successfully!`
        )
      );
  ...
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    if (coursesSnapshot.exists()) {
      coursesSnapshot.forEach((courseSnapshot) => {
        ...
        if (
          courseData.groupType === 'course' &&
          courseData.status === DB_CONST.GROUP_STATUS_ACTIVE
        ) {
    ...
    try {
      ...
      dispatch(
        openFeedbackSnackbar(FeedbackSnackbarTypes.Success, 'Courses updated successfully!')
      );
...
export const updateCourseStatistic: ActionCreator<any> = (
  courseName: string,
  studentCount: number,
  adminCount: number
) => {
...
export const loadCourseStatistics: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    try {
      ...
      const invitedUsers = (await realtimeDBUtils.loadInvitedUsers(groupProperties.anid)) as (
        | InvitedUser
        | InvitedUserWithProfile
      )[];

      // Load all admins for this group
      const groupAdmins = (await realtimeDBUtils.loadGroupAdminsBasedOnGroupID(
        groupProperties.anid
      )) as any[];
      ...
      const updatedStatistics: CourseStatistics[] = courses.map((courseName) => {
        ...
        const courseStudents = invitedUsers.filter((user) => {
          ...
          if (userWithProfile.profile?.course) {
            const profileCourseName = userWithProfile.profile.course
              .toLowerCase()
              .replace(/\s+/g, '-');
      ...
      dispatch(
        openFeedbackSnackbar(FeedbackSnackbarTypes.Error, 'Failed to load course statistics')
      );
