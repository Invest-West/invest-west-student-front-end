...
const CourseMembers: React.FC<CourseMembersProps> = ({
  ...
  const loadCourseMembers = async () => {
    try {
      ...
      const response = await Api.doGet(
        `/groups/${groupUserName}/courses/${courseUserName}/list-members`
      );
  ...
  const handleAddMembersToCourse = async () => {
    ...
    try {
      ...
      const response = await Api.doPut(
        `/groups/${groupUserName}/courses/${courseUserName}/add-members`,
        {
          emails: emailsToAdd,
        }
      );
  ...
  const handleRemoveMemberFromCourse = async (email: string) => {
    try {
      setError(null);
      await Api.doDelete(
        `/groups/${groupUserName}/courses/${courseUserName}/remove-member/${encodeURIComponent(email)}`
      );
        ...
        <Box mb={3}>
          ...
          {emailsToAdd.length > 0 && (
            ...
            >
              {addingMembers
                ? 'Adding Members...'
                : `Add ${emailsToAdd.length} Member(s) to Course`}
            </Button>
          )}
