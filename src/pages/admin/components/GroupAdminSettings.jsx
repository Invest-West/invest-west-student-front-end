...
const mapDispatchToProps = (dispatch) => {
  return {
    initializeGroupAttributesEdited: () =>
      dispatch(groupAdminSettingsActions.initializeGroupAttributesEdited()),
    handleInputChanged: (event) => dispatch(groupAdminSettingsActions.handleInputChanged(event)),

    saveGroupDetails: (field, value) =>
      dispatch(groupAdminSettingsActions.saveGroupDetails(field, value)),
    cancelEditingGroupDetails: (field) =>
      dispatch(groupAdminSettingsActions.cancelEditingGroupDetails(field)),
    ...
    handlePitchExpiryDateChanged: (date) =>
      dispatch(groupAdminSettingsActions.handlePitchExpiryDateChanged(date)),
    handleSavePitchExpiryDate: () =>
      dispatch(groupAdminSettingsActions.handleSavePitchExpiryDate()),
    handleCancelEditingPitchExpiryDate: () =>
      dispatch(groupAdminSettingsActions.handleCancelEditingPitchExpiryDate()),

    handleExpandPledgeFAQPanel: (FAQ, isExpanded) =>
      dispatch(groupAdminSettingsActions.handleExpandPledgeFAQPanel(FAQ, isExpanded)),
    ...
    toggleEditExpandedPledgeFAQ: () =>
      dispatch(groupAdminSettingsActions.toggleEditExpandedPledgeFAQ()),
    ...
    handleQuillEditorChanged: (fieldName, content, delta, source, editor) =>
      dispatch(
        groupAdminSettingsActions.handleQuillEditorChanged(
          fieldName,
          content,
          delta,
          source,
...
class GroupAdminSettings extends Component {
  ...
  componentDidMount() {
    const { initializeGroupAttributesEdited } = this.props;
          ...
          <Col xs={12} sm={12} md={12} lg={12}>
            ...
            <FlexView column marginTop={20}>
              ...
              <FlexView width="100%" hAlignContent="right" marginTop={15}>
                <Button
                  variant="outlined"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => cancelEditingGroupDetails('description')}
                  style={{ marginRight: 6 }}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  variant="contained"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => saveGroupDetails('description', groupDescription)}
                  disabled={groupDetails.description === groupDescription}
                  style={{ marginLeft: 6 }}
                >
                  Save
                </Button>
            ...
            <FlexView marginTop={35} column>
              <FormControl variant="standard" fullWidth>
                ...
                />
                {groupWebsite.trim().length === 0 || utils.isValidWebURL(groupWebsite) ? null : (
              ...
              <FlexView width="100%" hAlignContent="right" marginTop={15}>
                <Button
                  variant="outlined"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => cancelEditingGroupDetails('website')}
                  style={{ marginRight: 6 }}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  variant="contained"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => saveGroupDetails('website', groupWebsite)}
                  disabled={
            ...
            <FlexView marginTop={35} column>
              <FormControl variant="standard" fullWidth>
                <TextField
                  ...
                  margin="dense"
                  variant="outlined"
                  helperText="This link can be shared to unregistered issuers/students so that they can sign up and become members of your course without sending invitation emails manually."
                  value={
                    process.env.REACT_APP_PUBLIC_URL + Routes.constructSignUpRoute(groupUserName)
                  }
                />
              </FormControl>
            </FlexView>
            ...
            <FlexView marginTop={35} column>
              ...
              {utils.isValidHexColorCode(primaryColor) ? null : (
              ...
              <FlexView width="100%" marginTop={10}>
                <Button
                  variant="outlined"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => cancelEditingColor('primaryColor')}
                  style={{ marginRight: 6 }}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  disabled={
                    !utils.isValidHexColorCode(primaryColor) ||
                    groupDetails.settings.primaryColor === primaryColor
                  }
                  style={{ marginLeft: 6 }}
                >
                  Save
            ...
            <FlexView marginTop={35} column>
              ...
              {utils.isValidHexColorCode(secondaryColor) ? null : (
                <Typography variant="body2" color="error" align="left" style={{ marginTop: 4 }}>
                  Invalid color
                </Typography>
              )}
              ...
              <FlexView width="100%" marginTop={10}>
                <Button
                  variant="outlined"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => cancelEditingColor('secondaryColor')}
                  style={{ marginRight: 6 }}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  variant="contained"
                  className={css(sharedStyles.no_text_transform)}
                  onClick={() => saveColor('secondaryColor')}
                  disabled={
                    !utils.isValidHexColorCode(secondaryColor) ||
                    groupDetails.settings.secondaryColor === secondaryColor
                  }
                  style={{ marginLeft: 6 }}
                >
                  Save
                ...
                <FlexView column marginTop={30}>
                  ...
                  <FlexView width="100%" marginTop={15}>
                    <Button
                      variant="outlined"
                      className={css(sharedStyles.no_text_transform)}
                      onClick={handleCancelEditingPitchExpiryDate}
                      style={{ marginRight: 6 }}
                    >
                      Cancel
                    </Button>
                    <Button
                      ...
                      variant="contained"
                      className={css(sharedStyles.no_text_transform)}
                      onClick={handleSavePitchExpiryDate}
                      disabled={
                        groupAttributesEdited.hasOwnProperty('defaultPitchExpiryDate')
                          ? groupAttributesEdited.defaultPitchExpiryDate === null ||
                            isNaN(groupAttributesEdited.defaultPitchExpiryDate) ||
                            (groupDetails.settings.hasOwnProperty('defaultPitchExpiryDate') &&
                              groupAttributesEdited.hasOwnProperty('defaultPitchExpiryDate') &&
                              groupDetails.settings.defaultPitchExpiryDate ===
                                groupAttributesEdited.defaultPitchExpiryDate)
                          : false
                      }
                      style={{ marginLeft: 6 }}
              ...
              <FlexView marginTop={20}>
                <FormControl variant="standard">
                  ...
                  <RadioGroup
                    ...
                    value={groupAttributesEdited.projectVisibility.toString()}
                    onChange={handleInputChanged}
                  >
                    <FormControlLabel
                      value={DB_CONST.PROJECT_VISIBILITY_PRIVATE.toString()}
                      control={<Radio />}
                      label="Private"
                    />
                    <FormControlLabel
                      value={DB_CONST.PROJECT_VISIBILITY_RESTRICTED.toString()}
                      control={<Radio />}
                      label="Restricted"
                    />
                    <FormControlLabel
                      value={DB_CONST.PROJECT_VISIBILITY_PUBLIC.toString()}
                      control={<Radio />}
                      label="Public"
                    />
                  </RadioGroup>
