...
interface EditGroupImageDialogState {
  mode: 'upload' | 'url';
  imageUrl: string;
...
export default class EditGroupImageDialog extends Component<
  EditGroupImageDialogProps,
  EditGroupImageDialogState
> {
  constructor(props: EditGroupImageDialogProps) {
    super(props);
    this.state = {
      mode: 'upload',
  ...
  handleFileError = (error: any, file: File) => {
    ...
    this.setState({
      error: "Error uploading file. Please make sure it's a valid image under 30KB.",
      selectedFile: null,
    });
  };
  ...
  handleSave = async () => {
    ...
    const { groupUserName, onSuccess, onClose } = this.props;
    ...
    try {
      ...
      if (mode === 'upload') {
        ...
        formData.append('fileName', `${groupUserName}-logo-${Date.now()}`);
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            {isProcessing && (
              <Box display="flex" alignItems="center" marginTop="15px">
                ...
                <Typography variant="body2">
                  {uploading ? 'Uploading file...' : 'Updating logo...'}
                </Typography>
              </Box>
            )}
          </Box>
        ...
        <DialogActions style={{ margin: 0, padding: '20px' }}>
          ...
          <Button
            ...
            disabled={
              isProcessing || (mode === 'upload' && !selectedFile) || (mode === 'url' && !imageUrl)
            }
          >
            Save
          </Button>
