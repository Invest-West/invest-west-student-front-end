...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    startListeningForGroupAdminsChanged: () =>
      dispatch(groupAdminsTableActions.startListeningForGroupAdminsChanged()),
...
class GroupAdminsTable extends Component {
   ...
   */
  addListeners = () => {
    const { shouldLoadOtherData, groupAdminsLoaded, startListeningForGroupAdminsChanged } =
      this.props;
                        ...
                        <OverlayTrigger
                          ...
                          overlay={
                            <Tooltip id={`tooltip-request-admin`}>
                              Request to add a new admin to your course. A super admin will review
                              your request.
                            </Tooltip>
                          }
                        >
  ...
  getCourseNames = (admin) => {
    ...
    const adminCourse = systemGroups.find(
      (group) => group.anid === admin.anid && group.parentGroupId
    );
    ...
    const adminUniversity = systemGroups.find(
      (group) => group.anid === admin.anid && !group.parentGroupId
    );
    ...
    if (adminUniversity) {
      ...
      const courseNames = coursesUnderUniversity.map(
        (course) => course.displayName || course.groupUserName
      );
      return courseNames.join(', ');
    }
  ...
  getAvailableCourses = () => {
    ...
    return systemGroups.filter(
      (group) =>
        group.parentGroupId === tableGroup.anid ||
        (group.groupUserName && group.groupUserName.startsWith(tableGroup.groupUserName + '-'))
    );
  };
   ...
   */
  renderTableRows = () => {
    const { groupProperties, groupAdminsLoaded, page, rowsPerPage, inSearchMode } = this.props;
            ...
            <FlexView hAlignContent="center" vAlignContent="center" style={{ margin: 20 }}>
              {groupAdminsLoaded ? (
                <Typography variant="body1" align="center">
                  {inSearchMode ? 'No group admins found.' : 'No group admins.'}
                </Typography>
              ) : (
                <HashLoader
                  color={
    ...
    return this.getRenderedGroupAdmins()
      ...
      .map((groupAdmin) => (
        <TableRow key={groupAdmin.id} hover>
          ...
          <TableCell colSpan={1}>
            <Typography variant="body2" align="left">
              {groupAdmin.superGroupAdmin ? 'Super group admin' : 'Group admin'}
            </Typography>
          </TableCell>
          <TableCell colSpan={1}>
            <Typography variant="body2" align="left">
  ...
  getRenderedGroupAdmins = () => {
    const { searchText, inSearchMode, filterCourse, groupAdmins, systemGroups } = this.props;
    ...
    if (inSearchMode) {
      searchedGroupAdmins = searchedGroupAdmins.filter((groupAdmin) =>
        groupAdmin.email.includes(searchText.toLowerCase())
      );
    }
      ...
      searchedGroupAdmins = searchedGroupAdmins.filter((groupAdmin) => {
        ...
        if (systemGroups && systemGroups.length > 0) {
          ...
          if (selectedCourse) {
            ...
            const adminUniversity = systemGroups.find(
              (group) => group.anid === groupAdmin.anid && !group.parentGroupId
            );
            if (
              adminUniversity &&
              selectedCourse.groupUserName &&
