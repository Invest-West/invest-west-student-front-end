...
const EMAIL_TEMPLATE_INFO: Record<
  string,
  { name: string; variables: string[]; description: string }
> = {
  enquiry: {
    name: 'Enquiry Email',
...
class EmailTemplateManager extends Component<Props, State> {
  ...
  getSampleData = (slug: string): Record<string, string> => {
    ...
    realData['resetPasswordLink'] =
      `${baseUrl}/auth/action?mode=resetPassword&oobCode=sample-token`;
    ...
    realData['adderName'] = currentUser
      ? (currentUser as User).firstName
        ? `${(currentUser as User).firstName} ${(currentUser as User).lastName || ''}`.trim()
        : currentUser.email
      : 'Super Admin';
  ...
  handleSendTest = async () => {
    ...
    const result = await this.props.sendTestEmail(selectedTemplate.slug, testEmail, testData);
      ...
      <div style={{ padding: 20 }}>
                ...
                <Accordion>
                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}>
                      ...
                      <Chip
                        label={
                          template ? (template.isActive ? 'Active' : 'Inactive') : 'Not Created'
                        }
        ...
        >
                ...
                <div style={{ border: '1px solid #ccc', borderRadius: 4 }}>
                  <Editor
                    ...
                    onChange={(value) => {
                      ...
                      this.updatePreview(
                        value || '',
                        this.getSampleData(selectedTemplate?.slug || '')
                      );
                    }}
          ...
          <DialogActions>
            ...
            <Button onClick={this.handleSave} color="primary" variant="contained" disabled={saving}>
              {saving ? 'Saving...' : 'Save'}
            </Button>
          </DialogActions>
        </Dialog>
