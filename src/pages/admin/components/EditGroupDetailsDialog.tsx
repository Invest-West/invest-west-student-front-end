...
export default class EditGroupDetailsDialog extends Component<
  EditGroupDetailsDialogProps,
  EditGroupDetailsDialogState
> {
  ...
  handleSave = async () => {
    ...
    try {
      ...
      if (logoMode === 'upload' && selectedFile) {
        ...
        formData.append(
          'fileName',
          `${isCourse ? courseUserName : groupUserName}-logo-${Date.now()}`
        );
      ...
      if (Object.keys(updates).length > 0) {
        await firebase.database().ref(dbRef).child(groupAnid).update(updates);
      }
  ...
  hasChanges = () => {
    ...
    const logoChanged =
      (logoMode === 'upload' && selectedFile) || (logoMode === 'url' && logoUrl !== currentLogoUrl);
