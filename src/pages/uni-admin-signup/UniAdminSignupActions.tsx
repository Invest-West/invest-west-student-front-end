...
export const validateInviteToken: ActionCreator<any> = (token: string) => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    try {
      ...
      const response = await Api.doGet(`${UniAdminInviteApiRoutes.validateInviteRoute}/${token}`);
...
export const createAccount: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { token, title, firstName, lastName, password, confirmedPassword } =
    ...
    try {
      ...
      const response = await Api.doPost(UniAdminInviteApiRoutes.completeSignupRoute, {
        token,
        title,
