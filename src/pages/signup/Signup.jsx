...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    togglePreventValidatingUserWhenSigningUp: (shouldPrevent) =>
      dispatch(authActions.togglePreventValidatingUserWhenSigningUp(shouldPrevent)),
...
class Signup extends Component {
  ...
  componentDidMount() {
    ...
    setExpectedAndCurrentPathsForChecking(
      match.params.hasOwnProperty('groupUserName')
        ? ROUTES.SIGN_UP
        : ROUTES.SIGN_UP_INVEST_WEST_SUPER,
      match.path
    );
   ...
   */
  loadData = () => {
    const { dataLoaded, loadingData } = this.state;
    ...
    if (!dataLoaded && !loadingData) {
      if (invitedID === 'investor' || invitedID === 'issuer') {
        ...
        realtimeDBUtils
          .getInvitedUserBasedOnIDOrEmail(
            invitedID,
            realtimeDBUtils.GET_INVITED_USER_BASED_ON_INVITED_ID
          )
          .then((invitedUsersByInvitedID) => {
            ...
            realtimeDBUtils
              .getInvitedUserBasedOnIDOrEmail(
                invitedUsersByInvitedID[0].email,
                realtimeDBUtils.GET_INVITED_USER_BASED_ON_EMAIL
              )
  ...
  handleSignupForNewUserInputChanged = (event) => {
    const { publicRegistration } = this.state;
  ...
  handleSignupForNewUserClick = async () => {
    const { groupUserName, groupProperties } = this.props;
    ...
    if (
      ...
      if (publicRegistration) {
        if (
          ...
          publicRegistrationUser.confirmedEmail.trim().toLowerCase()
        ) {
          this.setState({
      ...
      try {
        await new Api().request('post', ApiRoutes.createUser, {
        ...
        this.firebaseAuth
          ...
          .then((auth) => {
            const id = this.firebaseDB.ref(DB_CONST.MARKETING_PREFERENCES_CHILD).push().key;
  ...
  handleDeclineToJoinTheAngelNetwork = () => {
    const { invitedUserWithSpecifiedInvitedID } = this.state;
  ...
  handleAgreeToJoinTheAngelNetwork = () => {
    const { invitedUserWithSpecifiedInvitedID, invitedUsersWithTheEmailObtained } = this.state;

    const invitedUsersByEmailThatHaveRegisteredIndex = invitedUsersWithTheEmailObtained.findIndex(
      (invitedUserByEmail) =>
        invitedUserByEmail.id !== invitedUserWithSpecifiedInvitedID.id &&
        invitedUserByEmail.hasOwnProperty('officialUserID')
    );

    // user has already registered (joined) before
    if (invitedUsersByEmailThatHaveRegisteredIndex !== -1) {
      const userOfficialID =
        invitedUsersWithTheEmailObtained[invitedUsersByEmailThatHaveRegisteredIndex].id;
  ...
  render() {
    const { groupUserName, groupProperties, groupPropertiesLoaded, shouldLoadOtherData } =
      this.props;
    ...
    if (
      ...
      return <UserNotAbleToRegisterOrJoin invitedUser={invitedUserWithSpecifiedInvitedID} />;
    }
          ...
          >
            ...
            >
              ...
              {!publicRegistration ? (
                ...
                >
                  You have been invited to join the {invitedUser.Invitor.displayName} members area.
                </Typography>
              ) : null}
              ...
              <FlexView>
                <FlexView grow marginRight={10}>
                  <TextField
                    ...
                    name="firstName"
                    value={
                      !publicRegistration ? invitedUser.firstName : publicRegistrationUser.firstName
                    }
                    fullWidth
                    variant="outlined"
                    margin="dense"
                ...
                <FlexView grow marginLeft={10}>
                  <TextField
                    ...
                    name="lastName"
                    value={
                      !publicRegistration ? invitedUser.lastName : publicRegistrationUser.lastName
                    }
                    fullWidth
                    variant="outlined"
                    margin="dense"
              ...
              <FormControl fullWidth>
                <TextField
                  ...
                  label="Email"
                  name="email"
                  value={!publicRegistration ? invitedUser.email : publicRegistrationUser.email}
                  fullWidth
                  variant="outlined"
                  margin="dense"
                  error={
              ...
              <FlexView vAlignContent="center" marginTop={8}>
                <Checkbox
              ...
              {!processingSubmission ? null : (
                <FlexView hAlignContent="center" marginTop={15}>
                  <HashLoader
                    color={
      ...
      <Row noGutters>
        ...
        >
          ...
          >
            ...
            >
              <Typography variant="h5">Join {invitedUser.Invitor.displayName}</Typography>
            </FlexView>
            ...
            >
              ...
              <FlexView width="100%" marginTop={25} marginBottom={30}>
                <Button
                  variant="outlined"
                  color="secondary"
                  fullWidth
