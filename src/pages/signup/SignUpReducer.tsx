...
export const hasSuccessfullyLoadedInvitedUser = (state: SignUpState) => {
  return (
    state.invitedUserLoaded &&
    !state.loadingInvitedUser &&
    state.invitedUser !== undefined &&
    state.errorLoadingInvitedUser === undefined
  );
};
...
export const notFoundInvitedUser = (state: SignUpState) => {
  return (
    hasErrorLoadingInvitedUser(state) &&
    state.errorLoadingInvitedUser &&
    state.errorLoadingInvitedUser.detail.includes('404')
  );
};
...
const signUpReducer = (state = initialState, action: SignUpAction) => {
  switch (action.type) {
    ...
    case SignUpEvents.CompleteLoadingInvitedUser:
      const completeLoadingInvitedUserAction: CompleteLoadingInvitedUserAction =
        action as CompleteLoadingInvitedUserAction;
      return {
        ...state,
        invitedUser: completeLoadingInvitedUserAction.invitedUser,
        invitedUserLoaded: true,
        loadingInvitedUser: false,
        errorLoadingInvitedUser:
          completeLoadingInvitedUserAction.error !== undefined
            ? { detail: completeLoadingInvitedUserAction.error }
            : state.errorLoadingInvitedUser,
        userType: completeLoadingInvitedUserAction.invitedUser
          ? completeLoadingInvitedUserAction.invitedUser.type
          : state.userType,
        title: completeLoadingInvitedUserAction.invitedUser
          ? completeLoadingInvitedUserAction.invitedUser.title
          : state.title,
        firstName: completeLoadingInvitedUserAction.invitedUser
          ? completeLoadingInvitedUserAction.invitedUser.firstName
          : state.firstName,
        lastName: completeLoadingInvitedUserAction.invitedUser
          ? completeLoadingInvitedUserAction.invitedUser.lastName
          : state.lastName,
        email: completeLoadingInvitedUserAction.invitedUser
          ? completeLoadingInvitedUserAction.invitedUser.email
          : state.email,
      };
    ...
    case SignUpEvents.CompleteCreatingAccount:
      const completeCreatingAccountAction: CompleteCreatingAccountAction =
        action as CompleteCreatingAccountAction;
      return {
        ...state,
        creatingAccount: false,
        errorCreatingAccount: completeCreatingAccountAction.error
          ? { detail: completeCreatingAccountAction.error }
          : state.errorCreatingAccount,
      };
    default:
      return state;
  }
