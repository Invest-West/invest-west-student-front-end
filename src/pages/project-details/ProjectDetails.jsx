...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    pledgesTable_stopListeningForPledgesChanged: () =>
      dispatch(pledgesTableActions.stopListeningForPledgesChanged()),

    createPledgeDialog_toggleDialog: () =>
      dispatch(createPledgeDialogActions.toggleCreatePledgeDialog()),
    createPledgeDialog_setProject: (project) =>
      dispatch(createPledgeDialogActions.setProject(project)),

    selectProjectVisibility_setProject: (project) =>
      dispatch(selectProjectVisibilityActions.setProject(project)),

    setFeedbackSnackbarContent: (message, color, position) =>
      dispatch(feedbackSnackbarActions.setFeedbackSnackbarContent(message, color, position)),

    toggleContactPitchOwnerDialog: (projectName, projectOwnerEmail) =>
      dispatch(toggleContactPitchOwnerDialog(projectName, projectOwnerEmail)),
  };
};
...
class ProjectDetailsMain extends Component {
  ...
  componentDidMount() {
    ...
    const { dataLoaded, dataBeingLoaded } = this.state;
    ...
    setExpectedAndCurrentPathsForChecking(
      match.params.hasOwnProperty('groupUserName')
        ...
        : ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER,
      match.path
    );
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const { project, projectLoaded } = this.state.projectDetail;
  ...
  componentWillUnmount() {
    const { pledgesTable_stopListeningForPledgesChanged } = this.props;
  ...
  processProjectData = (project) => {
    ...
    if (user?.type === DB_CONST.TYPE_INVESTOR) {
      realtimeDBUtils.trackActivity({
        ...
        activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_VIEWED_PROJECT_DETAILS.replace(
          '%project%',
          project.projectName
        ),
    ...
    const shouldLoadRejectFeedbacks =
      user && (user.type === DB_CONST.TYPE_ADMIN || user.id === project.issuerID);
    ...
    Promise.all(promises)
      .then((results) => {
        ...
        if (user?.type === DB_CONST.TYPE_INVESTOR) {
          const currentUserPledgeIndex = pledges.findIndex(
            (pledge) => pledge.investorID === user?.id && pledge.amount !== ''
          );
  ...
  loadDataWhenUIElementsChange = () => {
    ...
    const { mainBody, commentsLoaded } = this.state;
    ...
    if (votes && votesLoaded && project && projectLoaded) {
      ...
      if (project.status === DB_CONST.PROJECT_STATUS_PITCH_PHASE) {
        if (!this.votesListener) {
          ...
          // vote added
          this.votesListener.on('child_added', (snapshot) => {
            const vote = snapshot.val();

            const votes = this.state.projectDetail.votes;
            const voteIndex = votes.findIndex((existingVote) => existingVote.id === vote.id);
            if (voteIndex === -1) {
              realtimeDBUtils.getUserBasedOnID(vote.investorID).then((investor) => {
          ...
          // vote changed
          this.votesListener.on('child_changed', (snapshot) => {
            const vote = snapshot.val();
          ...
          // vote removed
          this.votesListener.on('child_removed', (snapshot) => {
            const voteRemovedID = snapshot.key;

            const votes = this.state.projectDetail.votes;
            const voteIndex = votes.findIndex((existingVote) => existingVote.id === voteRemovedID);
    ...
    if (pledges && pledgesLoaded && projectLoaded && project) {
      ...
      if (
        ...
        if (!this.pledgesListener) {
          ...
          // new pledge added
          this.pledgesListener.on('child_added', (snapshot) => {
            const pledge = snapshot.val();

            const pledges = [...this.state.projectDetail.pledges];
            const pledgeIndex = pledges.findIndex(
              (existingPledge) => existingPledge.id === pledge.id
            );
            if (pledgeIndex === -1) {
              realtimeDBUtils.getUserBasedOnID(pledge.investorID).then((investor) => {
          ...
          // pledge changed
          this.pledgesListener.on('child_changed', (snapshot) => {
            const pledge = snapshot.val();
          ...
          // pledge removed
          this.pledgesListener.on('child_removed', (snapshot) => {
            const pledgeRemovedID = snapshot.key;

            const pledges = [...this.state.projectDetail.pledges];
            const pledgeIndex = pledges.findIndex(
              (existingPledge) => existingPledge.id === pledgeRemovedID
            );
            ...
            realtimeDBUtils
              ...
              .then((commentsWithReplies) => {
                ...
                if (!this.commentsListener) {
                  ...
                  // new comment added
                  this.commentsListener.on('child_added', (snapshot) => {
                    ...
                    if (commentIndex === -1) {
                      realtimeDBUtils.getUserBasedOnID(comment.commentedBy).then((user) => {
                        comment.author = user;
                        this.setState({
                  ...
                  // comment changed
                  this.commentsListener.on('child_changed', (snapshot) => {
                    const comment = snapshot.val();
                ...
                if (!this.commentsRepliesListener) {
                  ...
                  this.commentsRepliesListener.on('child_added', (snapshot) => {
                    const reply = snapshot.val();

                    const comments = [...this.state.comments];

                    const correspondingCommentIndex = comments.findIndex(
                      (comment) => comment.id === reply.commentID
                    );

                    // ensure the comment exists locally
                    if (correspondingCommentIndex !== -1) {
                      const comment = comments[correspondingCommentIndex];
                      ...
                      if (replyIndex === -1) {
                        ...
                        realtimeDBUtils.getUserBasedOnID(reply.repliedBy).then((user) => {
                          reply.author = user;
                  ...
                  this.commentsRepliesListener.on('child_changed', (snapshot) => {
                    const reply = snapshot.val();
  ...
  handleVotePitch = (voteVal) => {
    const { groupProperties, user } = this.props;

    const { project, votes } = this.state.projectDetail;

    const currentVoteObj = getInvestorVote(votes, user);
    if (currentVoteObj) {
      ...
      this.database
        ...
        .then(() => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary:
              realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_VOTE_FOR_PROJECT.replace(
                '%project%',
                project.projectName
              ),
            action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
    ...
    // investor has not voted before
      ...
      const id = this.database.ref(DB_CONST.VOTES_CHILD).push().key;
      ...
      this.database
        ...
        .then(() => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary:
              realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_MADE_A_VOTE_FOR_PROJECT.replace(
                '%project%',
                project.projectName
              ),
            action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
  ...
  attachProjectChangedListener = () => {
    const { project } = this.state.projectDetail;

    if (this.projectListener === null) {
      this.projectListener = this.database.ref(DB_CONST.PROJECTS_CHILD).child(project.id);

      this.projectListener.on('value', (snapshot) => {
        ...
        const newMainBody =
          isDraft || !canSeeAdminTab ? MAIN_BODY_CAMPAIGN : MAIN_BODY_ADMIN_OFFER_STATES;
        ...
        this.setState({
          ...
          adminOfferStatesActiveStep:
            project.status === DB_CONST.PROJECT_STATUS_BEING_CHECKED
              ...
              : project.status === DB_CONST.PROJECT_STATUS_PITCH_PHASE ||
                  project.status ===
                    DB_CONST.PROJECT_STATUS_PITCH_PHASE_EXPIRED_WAITING_TO_BE_CHECKED
                ? ADMIN_OFFER_STATES_MOVE_TO_PLEDGE
                : project.status ===
                      DB_CONST.PROJECT_STATUS_PRIMARY_OFFER_CREATED_WAITING_TO_BE_CHECKED ||
                    project.status === DB_CONST.PROJECT_STATUS_PRIMARY_OFFER_PHASE
  ...
  handleDateChanged = (date) => {
    ...
    this.setState({
      changedPitchExpiryDate: !date ? null : date.getTime(),
    });
  };
  ...
  handleSubmitCommentClick = () => {
    ...
    const id = this.database.ref(DB_CONST.COMMENTS_CHILD).push().key;
    ...
    this.database
      ...
      .then(() => {
        ...
        realtimeDBUtils.trackActivity({
          ...
          interactedObjectID: id,
          activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_COMMENTED_IN_PROJECT.replace(
            '%project%',
            project.projectName
          ),
          action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
        ...
        setFeedbackSnackbarContent('Comment added.', 'primary', 'bottom');
      })
      .catch((error) => {
        this.handleCloseCommentDialog();

        setFeedbackSnackbarContent('Error happened. Could not add comment.', 'error', 'bottom');
      });
  };
  ...
  handleSubmitUpdateCurrentComment = () => {
    const { currentComment, currentCommentText } = this.state;

    const { project } = this.state.projectDetail;

    const { setFeedbackSnackbarContent } = this.props;

    if (currentCommentText.trim().length === 0) {
      return;
    }

    const newComment = JSON.parse(JSON.stringify(currentComment));
    ...
    this.database
      ...
      .then(() => {
        ...
        realtimeDBUtils.trackActivity({
          ...
          activitySummary:
            realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_COMMENT_IN_PROJECT.replace(
              '%project%',
              project.projectName
            ),
          action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
        ...
        setFeedbackSnackbarContent('Comment updated.', 'primary', 'bottom');
      });
  };
  ...
  handleSubmitCommentReply = () => {
    const { replyingToComment, replyText, replyEdited } = this.state;

    const { project } = this.state.projectDetail;

    const { user, setFeedbackSnackbarContent } = this.props;
      ...
      realtimeDBUtils
        ...
        .then((replyID) => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_REPLIED_TO_A_COMMENT.replace(
              '%projectName%',
              project.projectName
            ),
            action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
          ...
          setFeedbackSnackbarContent('Reply added.', 'primary', 'bottom');
        })
        .catch((error) => {
          // handle error
          setFeedbackSnackbarContent('Error happened. Could not add reply.', 'error', 'bottom');
        });
    } else {
      const replyBeforeUpdating = JSON.parse(JSON.stringify(replyEdited));
      replyBeforeUpdating.author = null;

      const replyAfterUpdating = JSON.parse(JSON.stringify(replyBeforeUpdating));
      ...
      realtimeDBUtils
        ...
        .then(() => {
          ...
          realtimeDBUtils.trackActivity({
            ...
            activitySummary:
              realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_A_REPLY_OF_A_COMMENT.replace(
                '%projectName%',
                project.projectName
              ),
            action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
          ...
          setFeedbackSnackbarContent('Reply updated.', 'primary', 'bottom');
        })
        .catch((error) => {
          // handle error
          setFeedbackSnackbarContent('Error happened. Could not update reply.', 'error', 'bottom');
        });
    }
  };
    ...
    realtimeDBUtils
      ...
      .then(() => {
        ...
        realtimeDBUtils.trackActivity({
          ...
          activitySummary:
            realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_DELETED_A_REPLY_OF_A_COMMENT.replace(
              '%projectName%',
              project.projectName
            ),
          action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(':projectID', project.id),
        ...
        setFeedbackSnackbarContent('Reply deleted.', 'primary', 'bottom');
      })
      .catch((error) => {
        // handle error
        setFeedbackSnackbarContent('Error happened. Could not delete reply.', 'error', 'bottom');
      });
  };
  ...
  bringPitchBackToLive = async () => {
    ...
    const { changedPitchExpiryDate } = this.state;
    ...
    try {
      await realtimeDBUtils.bringPitchBackToLive({
        project: JSON.parse(JSON.stringify(project)),
        newPitchExpiryDate: changedPitchExpiryDate,
      });

      setFeedbackSnackbarContent('Project is now live again.', 'primary', 'bottom');
    } catch (error) {
      // handle error
      setFeedbackSnackbarContent("Error happened. Couldn't update project.", 'error', 'bottom');
    }
  };
  ...
  sendProjectBackToIssuer = async () => {
    const { projectDetail, rejectFeedback, sendingProjectBack } = this.state;
    ...
    try {
      ...
      await new Api().request('post', ApiRoutes.sendProjectBackToIssuerRoute, {
      ...
      setFeedbackSnackbarContent('Project has been sent back.', 'primary', 'bottom');
    } catch (error) {
      setFeedbackSnackbarContent("Error happened. Couldn't send offer back.", 'error', 'bottom');
    }
  };
  ...
  render() {
    ...
    if (
      authenticating ||
      (authIsNotInitialized(AuthenticationState) && isAuthenticating(AuthenticationState))
    ) {
      return (
        <FlexView marginTop={30} hAlignContent="center">
          <HashLoader
      ...
      <div>
        <ProjectDetails
          ...
          toggleContactPitchOwnerDialog={() =>
            this.props.toggleContactPitchOwnerDialog(project.projectName, projectIssuer.email)
          }
        />
...
class ProjectDetails extends Component {
  ...
  renderPageContent = (props) => {
    ...
    // display loading
    if (!pledgesLoaded || !votesLoaded || !investorPledgeLoaded || !projectIssuerLoaded) {
          ...
          <Col xs={12} md={{ span: 10, offset: 1 }} lg={{ span: 8, offset: 2 }}>
            <FlexView marginTop={40} hAlignContent="center">
              <Typography variant="h5" align="center">
                This offer has been closed temporarily.
              </Typography>
            </FlexView>
          </Col>
        </Row>
        ...
        <Row noGutters style={{ marginLeft: 10, marginRight: 10 }}>
          <Col xs={12} md={{ span: 10, offset: 1 }} lg={{ span: 8, offset: 2 }}>
            <FlexView marginTop={40} hAlignContent="center">
              <Typography variant="h5" align="center">
                {' '}
                You can only edit projects from your course.
              </Typography>
            </FlexView>
          </Col>
        </Row>
    ...
    if (sortedComments) {
      ...
      const currentCommentIndex = sortedComments.findIndex(
        (comment) => comment.commentedBy === user?.id
      );
    ...
    return (
      <Container
        fluid
        style={{ paddingLeft: 0, paddingRight: 0, paddingTop: 0, paddingBottom: 100 }}
      >
        <Row
          noGutters
          style={{
            paddingTop: 25,
            paddingBottom: 25,
            backgroundColor: colors.kick_starter_background_color,
          }}
        >
          {/** Banner to announce if a project is temporarily closed */}
          {!utils.isProjectTemporarilyClosed(project) ? null : (
            <Col
              xs={{ span: 12, offset: 0, order: 0 }}
              sm={{ span: 12, offset: 0, order: 0 }}
              md={{ span: 12, offset: 0, order: 0 }}
              lg={{ span: 12, offset: 0, order: 0 }}
            >
              <FlexView
                marginBottom={20}
                hAlignContent="center"
                style={{ backgroundColor: colors.errorColor, padding: 15 }}
              >
                <Typography variant="body2" align="center" className={css(sharedStyles.white_text)}>
                  <b>This offer has been temporarily closed.</b>
                </Typography>
              </FlexView>
            </Col>
          )}
          ...
          <Col
            ...
            sm={{ span: 12, offset: 0, order: 3 }}
            md={{ span: 10, offset: 1, order: 3 }}
            lg={{ span: 8, offset: 2, order: 2 }}
            style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 20, paddingBottom: 40 }}
          >
            <FlexView column width="100%" hAlignContent="center">
              <Typography align="center" variant="h5" style={{ marginBottom: 12 }}>
                {this.shouldHideInformation() ? 'This is an offer' : project.projectName}
              </Typography>
              <Typography
                className={css(styles.gray_text)}
                align="center"
                variant="body1"
                style={{ whiteSpace: 'pre-line' }}
              >
                {this.shouldHideInformation() ? `in ${project.sector} sector` : project.description}
              </Typography>
              {
                          ...
                          <NavLink
                            to={
                              groupUserName
                                ...
                                : ROUTES.EDIT_OFFER_INVEST_WEST_SUPER.replace(
                                    ':projectID',
                                    project.id
                                  )
                            }
              ...
              {user?.id === project.issuerID ? null : (
                <FlexView column hAlignContent="center">
                  <NavLink
                    to={
                      this.shouldHideInformation() ||
                      (project.hasOwnProperty('createdByGroupAdmin') && project.createdByGroupAdmin)
                        ? groupUserName
                          ? ROUTES.VIEW_GROUP_DETAILS.replace(
                              ':groupUserName',
                              groupUserName
                            ).replace(':groupID', project.group.groupUserName)
                          : ROUTES.VIEW_GROUP_DETAILS_INVEST_WEST_SUPER.replace(
                              ':groupID',
                              project.group.groupUserName
                            )
                        : groupUserName
                  ...
                  {project.hasOwnProperty('createdByGroupAdmin') &&
                  project.createdByGroupAdmin ? null : projectIssuer.hasOwnProperty('linkedin') &&
                    projectIssuer.linkedin.trim().length > 0 ? (
                    <a href={projectIssuer.linkedin} target="_blank" rel="noopener noreferrer">
                      <img
                        alt="linkedin_logo"
                        src={require('../../img/linkedin_logo.png').default}
          ...
          >
            {this.shouldHideInformation() ? (
              <Image
                src={utils.getLogoFromGroup(utils.GET_PLAIN_LOGO, project.group)}
                style={{
                  maxHeight: isMobile
                    ? MAX_COVER_HEIGHT_IN_MOBILE_MODE
                    : MAX_COVER_HEIGHT_IN_BIG_SCREEN_MODE,
              ...
              />
            ) : project.Pitch.cover && Array.isArray(project.Pitch.cover) ? (
              project.Pitch.cover.map((coverItem, index) =>
                coverItem.hasOwnProperty('removed') ? null : coverItem.fileType ===
                  DB_CONST.FILE_TYPE_VIDEO ? (
                  <ReactPlayer
                    key={index}
                    url={coverItem.url}
                    playsInline
            ...
            <Row noGutters>
              ...
              >
                <FlexView
                  ...
                  height={4}
                  style={{
                    backgroundColor: !groupProperties
                      ? colors.primaryColor
              ...
              <Col
                xs={12}
                sm={12}
                md={{ span: 5, offset: 0, order: 4 }}
                lg={{ span: 12, offset: 0, order: 4 }}
              >
                {this.renderProjectStatus()}
              </Col>

              {/** Contact us */}
              {isProjectOwner(AuthenticationState.currentUser, project) ? null : (
                <Col
                  xs={12}
                  sm={12}
                  md={{ span: 10, offset: 1, order: 5 }}
                  lg={{ span: 12, offset: 0, order: 7 }}
                  style={{ marginTop: 15 }}
                >
                  <FlexView column hAlignContent="left">
                    <FlexView hAlignContent="center" vAlignContent="center">
                      <Button
                        ...
                        variant="contained"
                        className={css(sharedStyles.no_text_transform)}
                        disabled={this.shouldHideInformation()}
                        onClick={() => this.toggleContactPitchOwnerDialog()}
                      >
                        Contact us
                      </Button>
                      {/*<Button*/}
                    ...
                    user?.type === DB_CONST.TYPE_ADMIN ? (
                      <FlexView column marginTop={20}>
                        <Typography align="left" variant="body2" color="textSecondary">
                          You cannot see detailed information of this restricted offer.
                        </Typography>
                      </FlexView>
                    ) : (
                      <FlexView column marginTop={20}>
                        <Typography align="left" variant="body2" color="textSecondary">
                          Restricted to {project.group.displayName} members.
                        </Typography>
                        <NavLink
                          to={
                            groupUserName
              ...
              {!utils.isProjectInLivePledgePhase(project) ? null : user?.type !==
                DB_CONST.TYPE_INVESTOR ? (
                <Col
                  xs={12}
                  sm={12}
                  md={{ span: 10, offset: 1, order: 6 }}
                  lg={{ span: 12, offset: 0, order: 6 }}
                  style={{ marginTop: 30 }}
                >
                  <Typography align="left" variant="body2">
                    <u>
                      Closes on {utils.dateTimeInReadableFormat(project.PrimaryOffer.expiredDate)}.
                    </u>
                  </Typography>
                </Col>
              ) : investorPledge ? (
              ...
              >
                {
                  ...
                  !utils.isDraftProject(project) ? (
                    <Tab
                      value={MAIN_BODY_ADMIN_OFFER_STATES}
                      className={css(sharedStyles.tab_title)}
                      fullWidth
                      label="Control phases"
                      style={{
                ...
                {/*}*/}
                <Tab
                  value={MAIN_BODY_CAMPAIGN}
                  className={css(sharedStyles.tab_title)}
                  fullWidth
                  label="Project"
                />
                <Tab
                  value={MAIN_BODY_DOCUMENTS}
                  className={css(sharedStyles.tab_title)}
                  fullWidth
                  label="Documents"
                />
        ...
        <Row noGutters className={css(styles.main_body_section)}>
          ...
          {mainBody !== MAIN_BODY_ADMIN_OFFER_STATES ? null : (
                ...
                <Row>
                  ...
                  >
                    <Stepper
                      ...
                      activeStep={adminOfferStatesActiveStep}
                      style={{ marginTop: 15 }}
                    >
                      {/** Publish project step */}
                      <Step
                        key={ADMIN_OFFER_STATES_PUBLISH_PITCH}
                        completed={!utils.isProjectWaitingToGoLive(project)}
                      >
                        <StepButton
                          onClick={() =>
                            this.onAdminOfferStatesStepClick(ADMIN_OFFER_STATES_PUBLISH_PITCH)
                          }
                        >
                        ...
                        <StepContent>
                          {utils.isProjectWaitingToGoLive(project) ? (
                                  ...
                                  <FlexView width="100%" hAlignContent="right" marginTop={10}>
                                    <FlexView marginRight={6}>
                                      <Button
                                        ...
                                        variant="contained"
                                        className={css(sharedStyles.no_text_transform)}
                                        disabled={rejectFeedback.trim().length === 0}
                                        onClick={() => this.sendProjectBackToIssuer()}
                                      >
                                        {sendingProjectBack ? 'Sending...' : 'Send'}
                                      </Button>
                                    </FlexView>
                            ...
                            <FlexView column width="100%" marginTop={20}>
                              <Typography
                                color={isProjectRejectedToGoLive(project) ? 'secondary' : 'primary'}
                                align="left"
                                variant="body1"
                              >
                      ...
                      <Step
                        ...
                        disabled={
                          isProjectWaitingToGoLive(project) || isProjectRejectedToGoLive(project)
                        }
                        completed={
                      ...
                      >
                        <StepButton
                          onClick={() =>
                            this.onAdminOfferStatesStepClick(ADMIN_OFFER_STATES_MOVE_TO_PLEDGE)
                          }
                        >
                          {/*<StepLabel*/}
                          ...
                          {/*</StepLabel>*/}
                          <StepLabel error={utils.isProjectRejectedToGoLive(project)}>
                            Make decision for expired project
                          </StepLabel>
                        </StepButton>
                        <StepContent>
                          ...
                          !isProjectWaitingForPledgeToBeChecked(project) ? (
                            <FlexView column>
                              {!isProjectPitchExpiredWaitingForAdminToCheck(project) ? null : (
                                <FlexView
                                  column
                                  width="100%"
                                  marginTop={20}
                                ...
                                >
                                  ...
                                  >
                                    This project has expired. You can bring it back to live by
                                  ...
                                  <Button
                                    ...
                                    style={{ marginTop: 25 }}
                                    onClick={this.bringPitchBackToLive}
                                  >
                                    Make project live again
                                  </Button>
                                  ...
                                  <FlexView column marginTop={100}>
                                    ...
                                    >
                                      <b>
                                        <u>
                                          Please do it carefully as you cannot reverse your action.
                                        </u>
                                      </b>
                                    </Typography>
                                  </FlexView>
                            ...
                            <FlexView column width="100%" marginTop={20}>
                              <Typography
                                color={utils.isProjectFailed(project) ? 'secondary' : 'primary'}
                                align="left"
                                variant="body1"
                              >
                                {utils.isProjectFailed(project)
                                  ...
                                  : project.Pitch.status ===
                                      DB_CONST.PITCH_STATUS_ACCEPTED_CREATE_PRIMARY_OFFER
                                    ? project.hasOwnProperty('createdByGroupAdmin') &&
                                      project.createdByGroupAdmin &&
                                      groupProperties &&
                                      project.anid === groupProperties.anid
                    ...
                    {!isProjectLive(project) ? null : (
                      <FlexView column marginTop={30}>
                        <Button
                          ...
                          className={css(sharedStyles.no_text_transform)}
                          onClick={() =>
                            realtimeDBUtils.toggleProjectLivelinessTemporarily(
                              user,
          ...
          {mainBody !== MAIN_BODY_CAMPAIGN ? null : this.shouldHideInformation() ? (
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }}>
              {user?.type === DB_CONST.TYPE_ADMIN ? (
            ...
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }}>
              ...
              {
                // Project deck
                !project.Pitch.presentationDocument ? null : (
                  <FlexView column marginTop={30}>
                    <Typography variant="h5">Project deck</Typography>
                    <InlinePdfViewer
                      documents={project.Pitch.presentationDocument}
                      shouldShowRiskWarningOnDownload={true}
                    />
...

              {
                ...
                !project.Pitch.presentationText ? null : (
                  <FlexView
                    column
                    dangerouslySetInnerHTML={{
                      __html: utils.convertQuillDeltaToHTML(project.Pitch.presentationText.ops),
                    }}
                    marginTop={30}
                  />
                )
          ...
          {/** Comments */}
          {mainBody !== MAIN_BODY_COMMENTS ? null : this.shouldHideInformation() ? (
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }}>
              {user?.type === DB_CONST.TYPE_ADMIN ? (
                <FlexView column marginTop={30} hAlignContent="center">
                  <Typography align="center" variant="h5">
            ...
            <Col xs={12} sm={12} md={12} lg={{ span: 8, offset: 2 }}>
              <Row>
                <Col
                  ...
                  style={{ marginTop: 40 }}
                >
                  {user?.type === DB_CONST.TYPE_INVESTOR ? null : (
                    <Typography
                      align="center"
                      style={{
                        padding: 15,
                  ...
                  {!commentsLoaded ? (
                    ...
                    <FlexView
                      ...
                      style={{ padding: 15, backgroundColor: colors.kick_starter_background_color }}
                      column
                    >
                      {sortedComments.map((comment) =>
                        ...
                        comment.commentedBy === user?.id ? (
                          ...
                          >
                            ...
                            <FlexView marginTop={25} width="100%" hAlignContent="right">
                              ...
                              <Button
                                ...
                                disabled={
                                  (currentComment &&
                                    currentComment.comment === currentCommentText) ||
                                  currentCommentText.trim().length === 0
                                }
                                onClick={this.onSubmitUpdateCurrentComment}
                              >
                                Update
                              </Button>
                            </FlexView>
                            ...
                            {comment.replies.map((reply) => (
                              ...
                              >
                                <Divider
                                  style={{
                                    backgroundColor: styles.gray_text,
                                    height: 'auto',
                                    width: 6,
                                  }}
                                />
                                      ...
                                      reply.author.type === DB_CONST.TYPE_ADMIN
                                        ? groupUserName
                                          ? ROUTES.VIEW_GROUP_DETAILS.replace(
                                              ':groupUserName',
                                              groupUserName
                                            ).replace(':groupID', reply.author.anid)
                                          : ROUTES.VIEW_GROUP_DETAILS_INVEST_WEST_SUPER.replace(
                                              ':groupID',
                                              reply.author.anid
                                            )
                          ...
                          >
                            ...
                            {/** Comment text */}
                            <Typography style={{ marginTop: 12 }} variant="body1" align="left">
                              {comment.comment}
                            </Typography>
                            ...
                            replyEdited === null ? (
                              <CommentReplyInputArea
                                replyText={replyText}
                                replyEdited={replyEdited}
                                onToggleReplyToComment={this.onToggleReplyToComment}
                                    ...
                                    (user?.superAdmin ||
                                      ...
                                      <FlexView marginTop={16}>
                                        <Button
                                          ...
                                          size="small"
                                          color="secondary"
                                          onClick={() => this.onDeleteCommentReply(reply)}
                                        ...
                                        <Button
                                          ...
                                          color="primary"
                                          onClick={() =>
                                            this.onToggleReplyToComment(comment, reply)
                                          }
                                        >
                                          Edit
                    ...
                    <FlexView
                      ...
                      vAlignContent="center"
                      column
                      style={{ padding: 30, backgroundColor: colors.kick_starter_background_color }}
                    >
                    ...
                    >
                      ...
                      <FlexView marginTop={25} column hAlignContent="center">
                        <div>
                          <Button
                            size="small"
                            onClick={this.onPostACommentClick}
                            variant="outlined"
                            color="inherit"
                            fullWidth={false}
                            disabled={
                              utils.isProjectLive(project) && user?.type === DB_CONST.TYPE_INVESTOR
                            }
                          >
                            Post a comment
                          </Button>
                ...
                >
                  {user?.type === DB_CONST.TYPE_INVESTOR ? (
                    comments.filter((comment) => comment.commentedBy === user?.id).length === 0 ? (
                      <FlexView column>
                        <Typography variant="subtitle1" align="left" style={{ marginBottom: 25 }}>
                          This is your space to offer support and feedback to the issuer. Your
                      ...
                      <Typography variant="subtitle1" align="left">
                        You have already commented on this project opportunity. You can only edit
                        your comment.
                      </Typography>
                    )
                  ) : (
                    <Typography variant="subtitle1" align="left">
                      All the comments from students are displayed anonymously here.
                    </Typography>
                  )}
                </Col>
              </Row>
          ...
          {/** Documents */}
          {mainBody !== MAIN_BODY_DOCUMENTS ? null : this.shouldHideInformation() ? (
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }}>
              {user?.type === DB_CONST.TYPE_ADMIN ? (
                <FlexView column marginTop={30} hAlignContent="center">
                  <Typography align="center" variant="h5">
            ...
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }} style={{ marginTop: 30 }}>
              ...
              {project.Pitch.supportingDocuments &&
              project.Pitch.supportingDocuments.findIndex(
                (document) => !document.hasOwnProperty('removed')
              ) !== -1 ? (
                <InlinePdfViewer
                  documents={project.Pitch.supportingDocuments}
                  shouldShowRiskWarningOnDownload={true}
                />
              ) : (
                <Typography variant="body1" color="textSecondary" style={{ marginTop: 35 }}>
          ...
          {/** Notes */}
          {mainBody !== MAIN_BODY_NOTES ? null : this.shouldHideInformation() ? (
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }}>
              {user?.type === DB_CONST.TYPE_ADMIN ? (
                <FlexView column marginTop={30} hAlignContent="center">
                  <Typography align="center" variant="h5">
            ...
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }} style={{ marginTop: 30 }}>
              ...
              projectIssuer.BusinessProfile.university ? (
                <FlexView
                  className={css(styles.border_box)}
                  style={{ backgroundColor: colors.kick_starter_background_color }}
                  column
                ...
                >
                  <Typography variant="body1" align="left">
                    University: <b>{projectIssuer.BusinessProfile.university}</b>
                  </Typography>
                </FlexView>
              ) : null}
              ...
              {!project.Pitch.hasOwnProperty('detailsAboutEarlierFundraisingRounds') ? null : (
                <FlexView
                  className={css(styles.border_box)}
                  style={{ backgroundColor: colors.kick_starter_background_color }}
                  column
                ...
                >
                  <Typography variant="body1" align="left">
                    Details about earlier fundraising rounds:
                    <br />
                    <br /> {project.Pitch.detailsAboutEarlierFundraisingRounds}
                  </Typography>
                </FlexView>
              )}
            </Col>
  ...
  renderProjectStatus = () => {
          ...
          return (
            <FlexView column marginTop={30} marginBottom={30}>
              <Typography variant="body2" color="primary" align="left">
                Offer is in pledge phase
              </Typography>
              <Typography variant="h5" color="textPrimary" align="left">
                {utils.dateDiff(project.PrimaryOffer.expiredDate)}
              </Typography>
              <Typography variant="body2" color="textSecondary" align="left">
                days remaining
              </Typography>
        ...
        case DB_CONST.PITCH_STATUS_ON_GOING:
          return (
            <FlexView column marginTop={30} marginBottom={30}>
              ...
              <Typography variant="h5" color="textPrimary" align="left">
                {utils.dateDiff(project.Pitch.expiredDate)}
              </Typography>
              <Typography variant="body2" color="textSecondary" align="left">
                days remaining
              </Typography>
    ...
    return (
      <FlexView marginTop={30} marginBottom={30}>
        <Typography align="left" variant="body1" color={stt.color}>
          {stt.text}
        </Typography>
      </FlexView>
    );
  ...
  shouldVoteDisabled = () => {
    ...
    if (
      project.visibility !== DB_CONST.PROJECT_VISIBILITY_PUBLIC &&
      groupsUserIsIn.findIndex((group) => group.anid === project.anid) === -1
    ) {
      return true;
    }
...
const getInvestorVote = (votes, user) => {
  ...
  const investorVoteIndex = votes.findIndex((vote) => vote.investorID === user?.id);
  // investor has voted
...
class CommentDialog extends Component {
  ...
  render() {
    const { open, project, commentText, commentSubmitClick } = this.props;
    ...
    return (
      <Dialog fullWidth open={open} onClose={this.onClose}>
        ...
        <DialogContent>
          ...
          <InputBase
            error={commentText.trim().length === 0 && commentSubmitClick}
            name="commentText"
            value={commentText}
        ...
        <DialogActions style={{ paddingTop: 20, paddingBottom: 20, paddingRight: 24 }}>
          ...
          <Button
            disabled={commentText.trim().length > 0 && commentSubmitClick}
            style={{ marginLeft: 15 }}
            size="small"
            color="inherit"
            variant="outlined"
