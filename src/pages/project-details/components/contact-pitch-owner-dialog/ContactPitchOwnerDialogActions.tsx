...
export const toggleContactPitchOwnerDialog: ActionCreator<any> = (
  projectName?: string,
...
export const sendContactEmail: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { projectName, projectOwnerEmail, sendingContactEmail } =
    ...
    const { senderName, companyName, companyPosition, companyEmail, message } =
      getState().ContactPitchOwnerDialogLocalState;
    ...
    try {
      if (!clubAttributes || !clubAttributes.serverURL) {
        console.error(
          'Server URL not available in clubAttributes. State:',
          state.manageClubAttributes
        );
      ...
      return dispatch(
        openFeedbackSnackbar(FeedbackSnackbarTypes.Success, 'Email sent successfully.')
      );
    } catch (error) {
