...
class PledgePage extends Component {
  ...
  componentDidMount() {
    ...
    const { loadingData, dataLoaded } = this.state;
      ...
      .then((project) => {
        ...
        realtimeDBUtils
          ...
          .then((pledges) => {
            ...
            const userPledgeIndex = pledges.findIndex(
              (pledge) =>
                pledge.investorID === user.id &&
                project.id === pledge.projectID &&
                pledge.amount !== ''
            );
  ...
  handleCancelPledge = () => {
    ...
    const currentPledgeObject = JSON.parse(JSON.stringify(this.state.currentPledge));
    ...
    const newPledgeObject = JSON.parse(JSON.stringify(currentPledgeObject));
    ...
    this.firebaseDB
      ...
      .then(() => {
        ...
        realtimeDBUtils.trackActivity({
          ...
          activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_CANCELLED_PLEDGE.replace(
            '%project%',
            project.projectName
          ),
  ...
  handlePledgeConfirmClick = () => {
    ...
    const { project, pledgeAmount } = this.state;
    ...
    if (!currentPledgeObject) {
      ...
      this.firebaseDB
        ...
        .once('value', (snapshots) => {
          ...
          // add a new pledge
          if (addNewPledge) {
            const newPledgeID = this.firebaseDB.ref(DB_CONST.PLEDGES_CHILD).push().key;
              ...
              .then(() => {
                ...
                realtimeDBUtils.trackActivity({
                  ...
                  activitySummary:
                    existingPledgeWithBlankAmount !== null
                      ? realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_PLEDGE.replace(
                          '%project%',
                          project.projectName
                        )
                      : realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_MADE_NEW_PLEDGE.replace(
                          '%project%',
                          project.projectName
                        ),
                ...
                realtimeDBUtils
                  .sendNotification({
                    ...
                    message:
                      'Congratulations! Your investment opportunity has been pledged by an investor. Go and check your progress.',
            ...
            this.firebaseDB
              ...
              .then(() => {
                ...
                realtimeDBUtils.trackActivity({
                  ...
                  interactedObjectLocation: DB_CONST.PLEDGES_CHILD,
                  interactedObjectID: newPledgeObject.id,
                  activitySummary: realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_EDITED_PLEDGE.replace(
                    '%project%',
                    project.projectName
                  ),
                  action: ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(
              ...
              <Col xs={12} sm={12} md={{ span: 10, offset: 1 }} lg={{ span: 8, offset: 2 }}>
                <FlexView column hAlignContent="center" marginTop={40}>
                  <Typography variant="h5" align="center" paragraph>
                    You are not a member of {project.group.displayName}, so you cannot pledge this
                    offer. Join {project.group.displayName} to pledge.
                  </Typography>
                  <NavLink
                    to={
                      groupUserName
                        ? ROUTES.VIEW_GROUP_DETAILS.replace(
                            ':groupUserName',
                            groupUserName
                          ).replace(':groupID', project.group.anid)
    ...
    return (
      <Container fluid style={{ padding: 0 }}>
        ...
        <Row noGutters>
          ...
          >
            ...
            <FlexView column hAlignContent="center">
              <NavLink
                to={
                  project.hasOwnProperty('createdByGroupAdmin') && project.createdByGroupAdmin
                    ? groupUserName
                      ? ROUTES.VIEW_GROUP_DETAILS.replace(':groupUserName', groupUserName).replace(
                          ':groupID',
                          project.anid
                        )
                      : ROUTES.VIEW_GROUP_DETAILS_INVEST_WEST_SUPER.replace(
                    ...
                    : groupUserName
                      ...
                      : ROUTES.USER_PROFILE_INVEST_WEST_SUPER.replace(':userID', project.issuer.id)
                }
                target="_blank"
              ...
              project.createdByGroupAdmin ? null : project.issuer.hasOwnProperty('linkedin') &&
                project.issuer.linkedin.trim().length > 0 ? (
          ...
          <Row noGutters style={{ padding: 20, marginBottom: 30 }}>
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }} style={{ marginTop: 50 }}>
              <Typography variant="h5">
                {!currentPledge ? 'Support this project' : 'Update your pledge'}
              </Typography>
                ...
                <FlexView column>
                  ...
                  <Row>
                    ...
                    >
                      <TextField
                        error={
                          (pledgeButtonClick && pledgeAmount.length === 0) ||
                    ...
                    >
                      <FlexView vAlignContent="center" height="100%">
                        <Button
                          ...
                          variant="outlined"
                          color="primary"
                          onClick={this.handlePledgeClick}
                        >
                          {' '}
                          Pledge
                        </Button>
            ...
            <Col xs={12} sm={12} md={12} lg={{ span: 6, offset: 3 }} style={{ marginTop: 50 }}>
              <FlexView column>
                <Typography variant="body1" align="left" paragraph>
                  ...
                  details with the issuer. The issuer will then contact you with regards to next
                  steps. Making a pledge is simply an instruction to pass on your contact details.
                  Making a pledge does not constitute a contractual or investment obligation of any
                  sort.
                  <br />
                  <br />
                  Student Showcase is not regulated or authorised by the FCA. Student Showcase is
                  for self certified investors only. If you are in any doubt please seek advice from
                  an investment professional.
                  <br />
                  <br />
                  Neither Student Showcase nor the Groups offer any guarantees or assurances about
                  the suitability or viability of any investment opportunity.
                  <br />
                  <br />
                  ...
                  opportunity. Please refer to our full&nbsp;
                  <NavLink
                ...
                {FAQs.length === 0
                  ...
                  : FAQs.map((question, index) => (
                      <Accordion
                        key={index}
                        elevation={0}
                        className={css(styles.frequently_asked_question_box)}
                      >
          ...
          <Row noGutters>
            <Col
              ...
              style={{
                marginTop: 100,
                position:
                  MediaQueryState.isMobile || !MediaQueryState.minHeightScreen
                    ? 'relative'
                    : 'fixed',
                bottom: 0,
                zIndex: 10,
        ...
        <ConfirmationDialog
          ...
          project={project}
          onClose={this.handleClosePledgeConfirmationDialog}
          onConfirmClick={this.handlePledgeConfirmClick}
        />
      </Container>
    );
  }
...
class ConfirmationDialog extends Component {
  ...
  render() {
    const { open, pledgeAmount, project } = this.props;
      ...
      <Dialog open={open} onClose={this.onClose}>
        ...
        <DialogContent>
          ...
          <DialogContentText>
            ...
            {utils.dateInReadableFormat(project.PrimaryOffer.expiredDate)}.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          ...
          <Button color="primary" onClick={this.onConfirmClick}>
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
...
class ThankYouComponent extends Component {
  render() {
    const { groupUserName, user, project, currentPledge } = this.props;
    ...
    return (
      <Row
        noGutters
        style={{
          backgroundColor: colors.kick_starter_gray_background,
          paddingTop: 20,
          paddingBottom: 20,
          paddingLeft: 10,
          paddingRight: 10,
          ...
          <Row noGutters>
            <Col xs={12} sm={12} md={12} lg={12}>
              <FlexView column hAlignContent="center">
                ...
                (project.issuer.profilePicture &&
                  project.issuer.profilePicture.findIndex(
                    (profilePicture) => !profilePicture.hasOwnProperty('removed')
                  ) === -1) ? (
                  <LetterAvatar
                    firstName={project.issuer.firstName}
                    lastName={project.issuer.lastName}
                    width={120}
                    height={120}
                    textVariant="h5"
                  />
                ...
                <Typography
                  ...
                  align="left"
                  style={{ marginTop: 6 }}
                >{`${project.issuer.firstName} ${project.issuer.lastName}`}</Typography>
              </FlexView>
            </Col>
            <Col
              xs={12}
              ...
              md={12}
              lg={12}
              style={{ backgroundColor: colors.white, padding: 18, marginTop: 30 }}
            >
              ...
              <Row noGutters style={{ marginTop: 45 }}>
                <Col xs={12} sm={12} md={6} lg={6}>
                  <FlexView hAlignContent="center">
                    <Button
                      color="primary"
                      className={css(sharedStyles.no_text_transform, sharedStyles.black_text)}
                      size="large"
                      onClick={() => window.location.reload()}
                    >
                      <u>Edit your pledge</u>
                    </Button>
                  </FlexView>
                </Col>
                <Col xs={12} sm={12} md={6} lg={6}>
                    ...
                    <Button
                      ...
                      href={
                        groupUserName
                          ...
                          : ROUTES.PROJECT_DETAILS_INVEST_WEST_SUPER.replace(
                              ':projectID',
                              project.id
                            )
                      }
                    >
                      <u>Go back to this offer</u>
                    </Button>
                  </FlexView>
                </Col>
              </Row>
