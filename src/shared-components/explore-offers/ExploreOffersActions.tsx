...
export const fetchOffers: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { searchFilter, visibilityFilter, sectorFilter, phaseFilter, groupFilter } =
    ...
    if (groupFilter === 'all') {
      orderBy =
        phaseFilter === FetchProjectsPhaseOptions.ExpiredPitch
          ? FetchProjectsOrderByOptions.Group
          : FetchProjectsOrderByOptions.Phase;
    } else {
    ...
    if (effectiveGroupFilter && effectiveGroupFilter !== 'all') {
      console.log(
        '[ExploreOffersActions] WARNING: Group filter is set to:',
  ...
  return (dispatch: Dispatch, getState: () => AppState) => {
    ...
    switch (name) {
      ...
      case 'visibilityFilter':
        if (
          value === 'all' ||
          value.toString() === PROJECT_VISIBILITY_PUBLIC.toString() ||
          value.toString() === PROJECT_VISIBILITY_RESTRICTED.toString()
        ) {
      ...
      case 'groupFilter':
        return dispatch(
          fetchOffers(value === 'all' ? undefined : FetchProjectsOrderByOptions.Group)
        );
      default:
        return;
    }
...
export const paginationChanged: ActionCreator<any> = (
