...
const mapDispatchToProps = (dispatch: ThunkDispatch<any, any, AnyAction>) => {
  return {
    ...
    handleSliderChanged: (sliderName: string, value: number) =>
      dispatch(handleSliderChanged(sliderName, value)),
...
class EditImageDialog extends Component<EditImageDialogProps, any> {
  ...
  render() {
    const { ProfileLocalState, toggleDialog } = this.props;
  ...
  renderEditPhotoContent = () => {
    const { ProfileLocalState, setEditor } = this.props;
  ...
  renderDisplayPhotoContent = () => {
    const { ProfileLocalState } = this.props;
  ...
  renderAddPhotoActions = () => {
    const { handleImageChanged, handleImageError } = this.props;
  ...
  renderEditPhotoActions = () => {
    const { ProfileLocalState, handleImageChanged, handleImageError, saveImage } = this.props;

    const EditImageLocalState: EditImageDialogState = ProfileLocalState.EditImageDialogState;

    return (
      <Box display="flex" flexDirection="row" justifyContent="flex-end" padding="20px">
        <Files
          onChange={handleImageChanged}
          onError={handleImageError}
          accepts={['image/png', 'image/jpg', 'image/jpeg']}
          ...
          clickable
        >
          <Button className={css(sharedStyles.no_text_transform)} variant="outlined">
            Change photo
          </Button>
        </Files>
        <Box width="20px" />
        <Button
          ...
          onClick={() => saveImage()}
        >
          {isSavingProfilePicture(EditImageLocalState) ? 'Saving photo ...' : 'Save photo'}
        </Button>
      </Box>
    );
  };
  ...
  renderDisplayPhotoActions = () => {
    const { ProfileLocalState, changeMode, deleteImage } = this.props;
    ...
    return (
      <Box display="flex" flexDirection="row" justifyContent="flex-end" padding="20px">
        ...
        >
          <Delete />
          <Box width="6px" />
          {isDeletingProfilePicture(EditImageLocalState) ? 'Deleting photo ...' : 'Delete'}
        </Button>
      </Box>
    );
  };
