...
const mapDispatchToProps = (dispatch: ThunkDispatch<any, any, AnyAction>) => {
  return {
    handleInputFieldChanged: (
      inputCategory: InputCategories,
      event: React.ChangeEvent<HTMLInputElement>
    ) => dispatch(handleInputFieldChanged(inputCategory, event)),
...
class PersonalDetails extends Component<PersonalDetailsProps, any> {
  onInputFieldChanged =
    (inputCategory: InputCategories) => (event: React.ChangeEvent<HTMLInputElement>) => {
      this.props.handleInputFieldChanged(inputCategory, event);
    };

  render() {
    const { AuthenticationState, ProfileLocalState, toggleUpdateProfilePhotoDialog } = this.props;
                ...
                {currentAdmin ? null : (
                  <Box marginY="20px">
                    <Button
                      ...
                      color="primary"
                      onClick={() =>
                        toggleUpdateProfilePhotoDialog(getProfilePicture(copiedUser) ?? undefined)
                      }
                    >
