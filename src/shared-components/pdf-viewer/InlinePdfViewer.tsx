...
const InlinePdfViewer: React.FC<InlinePdfViewerProps> = ({
  ...
  const handleIframeError = (index: number) => {
    ...
    setErrorStates((prev) => ({
      ...prev,
      [index]: 'Failed to load PDF. Your browser may not support inline PDF viewing.',
    }));
  };
          ...
          <Box key={index} marginBottom="30px">
            ...
            <Paper elevation={1} style={{ padding: '12px', marginBottom: '16px' }}>
              ...
              >
                <Box display="flex" alignItems="center">
                  <IconComponent
                    style={{ color: fileInfo.color, fontSize: 24, marginRight: '8px' }}
                  />
                  <Box>
            ...
            {error ? (
              ...
              <Box display="flex" flexDirection="column" alignItems="center" padding="40px">
                <IconComponent
                  style={{ fontSize: 64, color: fileInfo.color, marginBottom: '16px' }}
                />
                <Typography variant="h6" style={{ marginBottom: '8px' }}>
                  {fileInfo.label}
                </Typography>
                <Typography variant="body2" color="textSecondary" style={{ marginBottom: '24px' }}>
                  This file type cannot be previewed in the browser. Use the buttons above to open
                  or download.
                </Typography>
              </Box>
            )}
