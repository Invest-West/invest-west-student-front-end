...
export const fetchGroups: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { accessRequestsInstances, nameFilter } = getState().ExploreGroupsLocalState;
...
const filterGroupsByGroupFilter: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { groupFilter, groups } = getState().ExploreGroupsLocalState;
      ...
      switch (groupFilter) {
        ...
        case 'groupsOfMembership':
          ...
          if (!group.parentGroupId) {
            ...
            const isMemberOfUniversity =
              AuthenticationState.groupsOfMembership.findIndex(
  ...
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    try {
      ...
      const accessRequestIndex = currentAccessRequestInstances.findIndex(
        (accessRequestInstance) =>
          accessRequestInstance.group.anid === groupID &&
