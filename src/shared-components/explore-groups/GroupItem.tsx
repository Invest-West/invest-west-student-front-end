...
class GroupItem extends Component<GroupItemFullProps, any> {
  render() {
    ...
    const groupMember: GroupOfMembership | undefined = AuthenticationState.groupsOfMembership.find(
    ...
    if (hasAccessRequestsBeenSatisfied(ExploreGroupsLocalState)) {
      hasRequestedToAccessGroup =
        ExploreGroupsLocalState.accessRequestsInstances?.findIndex(
          (accessRequestInstance) => accessRequestInstance.group.anid === group.anid
        ) !== -1;
    }

    return (
      <Box marginY={isSubGroup ? '8px' : '18px'}>
        <Card
          elevation={isSubGroup ? 1 : undefined}
          style={isSubGroup ? { backgroundColor: '#fafafa' } : undefined}
        >
          {isVirtualCourse ? (
            ...
            <CustomLink
              url={Routes.constructGroupDetailRoute(
                  ...
                  onClick={() => {
                    if (!isAdmin(currentUser)) {
                      realtimeDBUtils.trackActivity({
                        ...
                        activitySummary:
                          realtimeDBUtils.ACTIVITY_SUMMARY_TEMPLATE_CLICKED_ON_GROUP_ITEM.replace(
                            '%group%',
                            group.displayName
                          ),
          ...
          {isVirtualCourse || isAdmin(currentUser) ? null : groupMember ? null : (
