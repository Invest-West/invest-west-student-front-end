...
const mapDispatchToProps = (dispatch: ThunkDispatch<any, any, AnyAction>) => {
  return {
    ...
    paginationChanged: (event: React.ChangeEvent<unknown>, page: number) =>
      dispatch(paginationChanged(event, page)),
...
class ExploreGroups extends Component<ExploreGroupsProps, any> {
      ...
      >
            ...
            <Col xs={8} sm={8} md={3} lg={2}>
              <Box display="flex" flexDirection="row" marginTop="48px">
                ...
                >
                  ...
                  <MenuItem key="groupsOfMembership" value="groupsOfMembership">
                    My universities
                  </MenuItem>
                </Select>
        ...
        {!successfullyFetchedGroups(ExploreGroupsLocalState) ? null : !hasGroupsForCurrentFilters(
          ...
          <Box marginTop="30px">
            <Row noGutters>
              <Col xs={12} sm={12} md={12} lg={12}>
                {coursesOnlyMode ? this.renderCoursesOnly() : this.renderHierarchicalGroups()}
              </Col>
            </Row>
          </Box>
        )}
  ...
  renderHierarchicalGroups() {
    const { ExploreGroupsLocalState } = this.props;
    ...
    const paginationIndicesLocal = calculatePaginationIndices(ExploreGroupsLocalState);
    const paginatedGroups = hierarchicalGroups.slice(
      paginationIndicesLocal.startIndex,
      paginationIndicesLocal.endIndex + 1
    );

    return paginatedGroups.map((university) => (
      <UniversityGroupItem key={university.anid} university={university} />
    ));
  }
  ...
  renderCoursesOnly() {
    ...
    return filteredCourses.map((course: GroupProperties) => (
      <GroupItem key={course.anid} group={course} />
    ));
  }
}
