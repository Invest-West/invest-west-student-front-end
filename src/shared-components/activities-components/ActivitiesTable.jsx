...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    startListeningForActivitiesChanged: () =>
      dispatch(activitiesTableActions.startListeningForActivitiesChanged()),
    stopListeningForActivitiesChanged: () =>
      dispatch(activitiesTableActions.stopListeningForActivitiesChanged()),

    jsonCompareDialog_setData: (jsonBefore, jsonAfter) =>
      dispatch(manageJSONCompareChangesActions.setData(jsonBefore, jsonAfter)),
  };
};
          ...
          <TableCell colSpan={2}>
            {activity.hasOwnProperty('action') ? (
              <FlexView column>
                ...
                {!activity.hasOwnProperty('userProfile') ? null : (
                  <Typography
                    variant="body2"
                    align="left"
                    style={{
                  ...
                  >
                    <i>
                      _ done by <b>{activity.userProfile.email}</b> _
                    </i>
                  </Typography>
                )}
                {currentUser.type !==
                DB_CONST.TYPE_ADMIN ? null : !currentUser.superAdmin ? null : !activity.hasOwnProperty(
                    'value'
                  ) ? null : (
