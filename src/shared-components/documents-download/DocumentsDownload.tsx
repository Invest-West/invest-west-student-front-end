...
const mapDispatchToProps = (dispatch: ThunkDispatch<any, any, AnyAction>) => {
  return {
    onDocumentClick: (document: PitchDocument, shouldShowRiskWarning: boolean) =>
      dispatch(onDocumentClick(document, shouldShowRiskWarning)),
...
class DocumentsDownload extends Component<DocumentsDownloadProps, any> {
  ...
  render() {
    ...
    riskWarningFooter = riskWarningFooter
      .split('%groupName%')
      .join(ManageGroupUrlState.group ? ManageGroupUrlState.group.displayName : 'Student Showcase');

    const riskWarningTextSplits = riskWarningFooter.split('%URL%');

    return (
      <Box marginTop="20px">
        <List>
          {documents.map((document, index) =>
            document.removed !== undefined && document.removed ? null : (
              <ListItem
                key={index}
                button
            ...
            <Box marginTop="18px" whiteSpace="pre-line">
              <Typography variant="body1" align="justify">
                ...
                <CustomLink
                  ...
                  component="nav-link"
                  childComponent={riskWarningTextSplits[1]}
                />
        ...
        {DocumentsDownloadLocalState.selectedDocument && (
          <PdfViewerModal
            ...
            onDownload={() =>
              window.open(DocumentsDownloadLocalState.selectedDocument!.downloadURL, '_blank')
            }
          />
        )}
      </Box>
