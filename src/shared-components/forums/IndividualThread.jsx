...
const mapDispatchToProps = (dispatch) => {
  return {
    toggleThreadReply: (open, threadReplyEdited) =>
      dispatch(forumsActions.toggleThreadReply(open, threadReplyEdited)),
  ...
  renderReplies = () => {
    ...
    return forumThreadReplies.map((reply) => (
      <Paper key={reply.id} className={css(styles.thread_reply_style)}>
        {reply.hasOwnProperty('deleted') && reply.deleted === true ? (
          <FlexView>
            ...
            <FlexView vAlignContent="center" marginBottom={10} width="100%">
              ...
              <FlexView hAlignContent="left">
                {reply.author.type === DB_CONST.TYPE_ADMIN ? (
                  reply.author.superAdmin ? (
                    ...
                    <FlexView width={AVATAR_MAX_HEIGHT} height={AVATAR_MAX_HEIGHT}>
                      <Image
                        roundedCircle
                        thumbnail
                        src={utils.getLogoFromGroup(
                          utils.GET_PLAIN_LOGO,
                          reply.author.groupDetails
                        )}
                        style={{
                          width: '100%',
                          maxHeight: AVATAR_MAX_HEIGHT,
                  ...
                  (reply.author.profilePicture &&
                    reply.author.profilePicture.findIndex(
                      (pictureItem) => !pictureItem.hasOwnProperty('removed')
                    ) === -1) ? (
            ...
            {!reply.message ? null : (
              <FlexView
                ...
                dangerouslySetInnerHTML={{
                  __html: utils.convertQuillDeltaToHTML(reply.message.ops),
                }}
              />
            )}

            {threadInReplyMode && threadReplyEdited && threadReplyEdited.id === reply.id ? (
              <CreateReply />
            ) : null}
          </FlexView>
        )}
  ...
  render() {
    ...
    return (
      <FlexView column>
          ...
          <FlexView column>
            <FlexView vAlignContent="center" marginBottom={10}>
              {forumThreadSelected.author.type === DB_CONST.TYPE_ADMIN ? (
                forumThreadSelected.author.superAdmin ? (
                  ...
                  <FlexView width={AVATAR_MAX_HEIGHT} height={AVATAR_MAX_HEIGHT}>
                    <Image
                      ...
                      src={utils.getLogoFromGroup(
                        utils.GET_PLAIN_LOGO,
                        forumThreadSelected.author.groupDetails
                      )}
                      style={{ width: '100%', maxHeight: AVATAR_MAX_HEIGHT, objectFit: 'contain' }}
                    />
                  </FlexView>
                ...
                (forumThreadSelected.author.profilePicture &&
                  forumThreadSelected.author.profilePicture.findIndex(
                    (pictureItem) => !pictureItem.hasOwnProperty('removed')
                  ) === -1) ? (
                // normal users with no profile picture
                <LetterAvatar
            ...
            <Typography
              variant="body1"
              align="left"
              style={{ whiteSpace: 'pre-line', marginTop: 6, marginBottom: 15 }}
            >
              {forumThreadSelected.description}
            </Typography>
            {!forumThreadSelected.message ? null : (
              <FlexView
                column
                dangerouslySetInnerHTML={{
                  __html: utils.convertQuillDeltaToHTML(forumThreadSelected.message.ops),
                }}
                marginTop={25}
              />
            )}
            {threadInReplyMode ? null : (
        ...
        {!threadInReplyMode ? null : threadReplyEdited ? null : <CreateReply />}
