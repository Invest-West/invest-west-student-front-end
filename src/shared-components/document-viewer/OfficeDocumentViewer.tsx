...
const OfficeDocumentViewer: React.FC<OfficeDocumentViewerProps> = ({
    ...
    return (
      <Box display="flex" flexDirection="column" alignItems="center" padding="40px">
        <Typography variant="body1" color="textSecondary" style={{ marginBottom: '16px' }}>
          This document could not be previewed. Use the buttons below to open or download.
        </Typography>
  ...
  return (
    ...
    >
      {docType.type === 'word' ? (
        <WordViewer fileUrl={fileUrl} fileName={fileName} onDownload={onDownload} />
      ) : docType.type === 'excel' ? (
        <ExcelViewer fileUrl={fileUrl} fileName={fileName} onDownload={onDownload} />
      ) : null}
    </Suspense>
  );
};
