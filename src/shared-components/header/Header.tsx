  ...
  render() {
    ...
    return (
      <Container fluid style={{ padding: 0 }}>
        <Row
          noGutters
          style={{ paddingTop: 10, paddingBottom: 10, backgroundColor: appColors.white }}
        >
          {/** Home/Dashboard */}
          <Col
            xs={{ span: 6, order: 3 }}
          ...
          >
            <Box
              ...
              marginLeft="30px"
            >
              {successfullyAuthenticated(AuthenticationState) ? null : (
                <CustomLink
                  color="black"
                  activeColor={
                  ...
                  url={homUrl}
                  component="nav-link"
                  childComponent={<Typography variant="body1">Home</Typography>}
                />
              )}

              {isAuthenticating(AuthenticationState) ? null : !successfullyAuthenticated(
                  AuthenticationState
                ) ? null : (
                <CustomLink
                  color="black"
                  ...
                  url={dashboardUrl}
                  component="nav-link"
                  childComponent={<Typography variant="body1">Dashboard</Typography>}
                />
              )}
            </Box>
          </Col>
                    ...
                    <img
                      ...
                      src={(() => {
                        ...
                        if (!successfullyAuthenticated(AuthenticationState)) {
                          console.log(
                            '[HEADER] Using universal logo (not authenticated):',
                            defaultLogo
                          );
                          return defaultLogo;
                        }
                        ...
                        if (isError || isSystemPublic) {
                          ...
                          console.log(
                            '[HEADER] Using default logo (no group or no logo):',
                            defaultLogo
                          );
                          return defaultLogo;
                        }
          ...
          >
            {isAuthenticating(AuthenticationState) ? null : !successfullyAuthenticated(
              ...
              Routes.isSignInRoute(routePath) ? null : (
                <Box
                  display="flex"
                  flexDirection="row"
                  height="100%"
