...
class OffersTable extends Component<OffersTableProps, any> {
  componentDidMount() {
    const { directTableUser, AuthenticationState, setUser, fetchDraftProjectsWithFeedbackCount } =
      this.props;
  ...
  componentDidUpdate(prevProps: OffersTableProps) {
    ...
    if (
      prevProps.OffersTableLocalState.offersFetched !== OffersTableLocalState.offersFetched &&
      OffersTableLocalState.offersFetched
    ) {
      fetchDraftProjectsWithFeedbackCount();
    }
  }
      ...
      <TableContainer component={Paper}>
        <Table color="black">
          ...
          <TableHead>
            ...
            {!(currentAdmin && tableAdmin && currentAdmin.id === tableAdmin.id) ? null : (
              <TableRow>
                <TableCell colSpan={5}>
                  <Box>
                    <Button
                    ...
                    >
                      ...
                      <Box width="10px" />
                      {isExportingCsv(OffersTableLocalState) ? 'Exporting ...' : 'Export csv'}
                    </Button>

                    {!hasErrorExportingCsv(OffersTableLocalState) ? null : (
                      <Box marginTop="12px">
            ...
            {isInvestor(currentUser) || (currentAdmin && currentAdmin.superAdmin) ? null : (
              <TableRow>
                <TableCell colSpan={5}>
                  <CustomLink
                    url={
                  ...
                  <Col xs={12} sm={12} md={6} lg={4}>
                    <Box paddingY="4px">
                      ...
                      >
                        ...
                        <MenuItem key={PROJECT_VISIBILITY_PUBLIC} value={PROJECT_VISIBILITY_PUBLIC}>
                          Public
                        </MenuItem>
                        <MenuItem
                  ...
                  <Col xs={12} sm={12} md={6} lg={4}>
                    <Box paddingY="4px">
                      ...
                      >
                        ...
                        <MenuItem key={PROJECT_STATUS_DRAFT} value={PROJECT_STATUS_DRAFT}>
                          Draft
                        </MenuItem>
                        <MenuItem
                          key={PROJECT_STATUS_PITCH_PHASE}
                          value={PROJECT_STATUS_PITCH_PHASE}
                        >
                          Approved
                        </MenuItem>
                        <MenuItem key={PROJECT_STATUS_REJECTED} value={PROJECT_STATUS_REJECTED}>
                          Rejected
                        </MenuItem>
                      </Select>
                    </Box>
          ...
          <TableBody>
            {
              ...
              !successfullyFetchedOffers(
                  OffersTableLocalState
                ) ? null : !hasOffersForCurrentFilters(OffersTableLocalState) ? ( // No offers available for current filters
                <TableRow>
                  <TableCell colSpan={5}>
                    <Box display="flex" justifyContent="center" alignItems="center" height="120px">
                      <Typography variant="h6" align="center">
                        There are no projects available using your current filter criteria.
                      </Typography>
                    </Box>
                  </TableCell>
                </TableRow>
                ...
                OffersTableLocalState.offerInstancesFilteredByName
                  .slice(
                    OffersTableLocalState.currentPage * OffersTableLocalState.rowsPerPage,
                    OffersTableLocalState.currentPage * OffersTableLocalState.rowsPerPage +
                      OffersTableLocalState.rowsPerPage
                  )
                  .map((offerInstance) => {
                    return (
                      <TableRow key={offerInstance.projectDetail.id} hover>
                        {/** Offer name */}
                        <TableCell colSpan={2}>
                          <Box display="flex" flexDirection="column" justifyContent="center">
                            ...
                            <Box display="flex" flexDirection="row">
                              ...
                              <Box width="15px" />
                              <CustomLink
                                url={Routes.constructProjectDetailRoute(
                                  ManageGroupUrlState.groupNameFromUrl ?? null,
                                  ManageGroupUrlState.courseNameFromUrl ?? null,
                            ...
                            currentUser.id === OffersTableLocalState.tableUser.id ? null : (
                              <Box marginTop="10px">
                            ...
                            {isInvestor(currentUser) ? null : !isDraftProject(
                                offerInstance.projectDetail
                              ) ? null : !(
                                // Only show edit if current user is an issuer AND they created this project
                                (
                                  isIssuer(currentUser) &&
                                  offerInstance.issuer &&
                                  currentUser.id === offerInstance.issuer.id
                                )
                              ) ? null : (
                              <Box marginTop="18px">
                                <CustomLink
                                  url={Routes.constructCreateProjectRoute(
                                    ManageGroupUrlState.groupNameFromUrl ?? null,
                                    ManageGroupUrlState.courseNameFromUrl ?? null,
                                    { edit: offerInstance.projectDetail.id }
                                  )}
                                  target="_blank"
                                  color="none"
                                  activeColor="none"
                        ...
                        <TableCell colSpan={1}>
                          <Box display="flex" flexDirection="column">
                            ...
                            </Typography>
                            {
                              ...
                              !currentAdmin &&
                              offerInstance.rejectFeedbacks &&
                              offerInstance.rejectFeedbacks.length > 0 ? (
                                <Box marginTop="12px">
                                  <Chip
                                    icon={<Warning />}
