...
export const setUser: ActionCreator<any> = (user?: User | Admin) => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    if (shouldSetTableUser) {
      ...
      try {
        if (user) {
          const groupsOfMembershipResponse = await new Api().request(
            'get',
          ...
          if (isTableAdminUser && isTableAdminUser.superAdmin) {
            ...
            action.groupsSelect = groupsOfMembership.map(
              (groupOfMembership) => groupOfMembership.group
            );
          }
        }
        dispatch(action);
...
export const fetchOffers: ActionCreator<any> = () => {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const { tableUser, tableUserGroupsOfMembership, visibilityFilter, groupFilter, phaseFilter } =
    ...
    // currentUser != tableUser --> admin is viewing issuer/investor/admin offers table
      ...
      // group admin is viewing issuer/investor offers table
        ...
        if (
          tableUserGroupsOfMembership !== undefined &&
          tableUserGroupsOfMembership.findIndex(
            (groupOfMembership) => groupOfMembership.group.anid === currentAdmin.anid
          ) !== -1 &&
          !isAdmin(tableUser)
        ) {
          shouldFetchOffers = true;
...
export const filterOffersByName: ActionCreator<any> = () => {
  return (dispatch: Dispatch, getState: () => AppState) => {
    ...
    if (nameFilter.trim().length > 0) {
      offerInstances = offerInstances.filter((offerInstance) =>
        offerInstance.projectDetail.projectName?.toLowerCase().includes(nameFilter.toLowerCase())
      );
  ...
  return async (dispatch: Dispatch, getState: () => AppState) => {
    ...
    try {
      ...
      const draftProjectsWithFeedback = response.data.filter((projectInstance: ProjectInstance) => {
        ...
        const hasFeedback =
          projectInstance.rejectFeedbacks && projectInstance.rejectFeedbacks.length > 0;
