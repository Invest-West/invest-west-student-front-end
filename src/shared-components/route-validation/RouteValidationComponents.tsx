...
export const RouteLoadingIndicator: React.FC<RouteLoadingIndicatorProps> = ({
  ...
  return (
    <Box>
      <BarLoader color={color} width="100%" height={4} />
      <Box
        display="flex"
...
export const RouteErrorDisplay: React.FC<RouteErrorDisplayProps> = ({
  ...
  const getSuggestions = () => {
    switch (errorCode) {
      ...
      default:
        return ['Check that the URL is correct', 'Go back to the home page'];
    }
  };
...
export const ValidatedRouteWrapper: React.FC<ValidatedRouteWrapperProps> = ({
  ...
  if (isLoading) {
    return <RouteLoadingIndicator message={loadingMessage} color={primaryColor} />;
  }
