...
const mapStateToProps = (state) => {
  return {
    ...
    allowVideoUpload:
      state.manageClubAttributes.clubAttributes &&
      state.manageClubAttributes.clubAttributes.hasOwnProperty('allowVideoUpload')
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleVideoFileError: (error, file) =>
      dispatch(editVideoActions.handleVideoFileError(error, file)),
    cancelEditingCurrentVideo: () => dispatch(editVideoActions.cancelEditingCurrentVideo()),
    handlePreviousVideoItemClick: (index) =>
      dispatch(editVideoActions.handlePreviousVideoItemClick(index)),
...
class EditVideoDialog extends Component {
                ...
                <FlexView column width="100%" hAlignContent="center">
                  ...
                  {allowVideoUpload ? (
                    <FlexView>
                      ...
                      <Button
                        variant={
                          videoTypeSelected === VIDEO_UPLOAD_URL_SELECTED ? 'contained' : 'outlined'
                        }
                        color="primary"
                        className={css(sharedStyles.no_text_transform)}
                        onClick={() =>
                  ...
                  {!videoTypeSelected ? null : videoTypeSelected === VIDEO_UPLOAD_FILE_SELECTED ? (
                    ...
                    <TextField
                      ...
                      variant="outlined"
                      value={!videoChosen ? '' : typeof videoChosen === 'string' ? videoChosen : ''}
                      fullWidth
  ...
  renderSelectedPreviousVideoStatus = () => {
    const { user, videoPreIndex } = this.props;
    ...
    // user has previous videos
      ...
      if (
        videoPreIndex !==
        user.BusinessProfile.video.findIndex((video) => !video.hasOwnProperty('removed'))
      ) {
    ...
    return (
      <Typography align="center" variant="body2" color="textSecondary" style={{ marginTop: 12 }}>
        {msg}
      </Typography>
    );
  };
