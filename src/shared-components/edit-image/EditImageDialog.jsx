...
import { UPLOAD_PROFILE_PICTURE_MODE } from '../uploading-dialog/UploadingDialog';
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleImageFileError: (error, file) =>
      dispatch(editImageActions.handleImageFileError(error, file)),
    handleSliderChanged: (event, newValue) =>
      dispatch(editImageActions.handleZoomSliderChanged(event, newValue)),
    onPreviousImageItemClick: (index) =>
      dispatch(editImageActions.handlePreviousImageItemClick(index)),
...
class EditImageDialog extends Component {
              ...
              <Col xs={12} sm={12} md={12} lg={12} style={{ padding: 18 }}>
                <FlexView column width="100%" hAlignContent="center">
                  ...
                  <FlexView
                    ...
                    style={{ position: 'relative', width: '100%', height: 325 }}
                  >
                    <AvatarEditor
  ...
  renderSelectedPreviousPhotoStatus = () => {
    const { user, imgPreIndex, mode } = this.props;
    ...
    if (mode === UPLOAD_PROFILE_PICTURE_MODE) {
      ...
      // user has previous profile pictures
        ...
        if (
          imgPreIndex !==
          user.profilePicture.findIndex((profilePic) => !profilePic.hasOwnProperty('removed'))
        ) {
    ...
    // upload logo
      ...
      // user has previous logos
        ...
        if (
          imgPreIndex !==
          user.BusinessProfile.logo.findIndex((logo) => !logo.hasOwnProperty('removed'))
        ) {
