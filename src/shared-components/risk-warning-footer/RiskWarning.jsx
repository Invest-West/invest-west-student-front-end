...
class RiskWarning extends Component {
  render() {
    const { clubAttributesLoaded } = this.props;
      ...
      >
        <Row noGutters>
          <Col
            ...
            md={12}
            lg={12}
            style={{ paddingTop: 45, paddingBottom: 45, paddingLeft: 28, paddingRight: 28 }}
          >
            <FlexView column>
              <Typography variant="body1" align="left" className={css(sharedStyles.white_text)}>
                {' '}
                <b>Risk warning</b>{' '}
              </Typography>
  ...
  reformattedRiskWarningFooter = () => {
    const { clubAttributes, groupProperties } = this.props;

    let riskWarningFooter = clubAttributes.riskWarningFooter;
    riskWarningFooter = riskWarningFooter
      .split('%groupName%')
      .join(groupProperties ? groupProperties.displayName : 'Student Showcase');

    const splits = riskWarningFooter.split('%URL%');
    ...
    return (
      ...
      >
        {splits[0]}
        <NavLink to={ROUTES.RISK_WARNING} target="_blank" className={css(styles.hyper_link)}>
          {' '}
          {urlText}{' '}
        </NavLink>
        {splits[2]}
