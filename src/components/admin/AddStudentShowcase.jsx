...
const AddStudentShowcase = () => {
  ...
  const addStudentShowcase = async () => {
    ...
    try {
      ...
      setMessage(
        `✅ Successfully added "Student Showcase" as the default course! Total courses: ${updatedCourses.length}`
      );
    } catch (error) {
  ...
  return (
    <Card className="m-3">
      ...
      <Card.Body>
        <p>
          This will add "Student Showcase" as the default course to your invest-west group's
        ...
        <div className="mb-3">
          ...
          <Button variant="primary" onClick={addStudentShowcase} disabled={isLoading}>
            {isLoading && <Spinner animation="border" size="sm" className="me-2" />}
        ...
        <div className="mt-3">
          <small className="text-muted">
            <strong>Note:</strong> "Student Showcase" will be added as the first course in the list,
            making it the default course for routing.
          </small>
        </div>
      </Card.Body>
    </Card>
