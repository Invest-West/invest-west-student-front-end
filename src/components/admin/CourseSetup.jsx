...
const CourseSetup = () => {
  ...
  const setupCourseStructure = async () => {
    ...
    try {
      ...
      const courseList = Object.entries(structuredCourses)
        .map(
          ([courseUserName, course]) =>
            `${course.displayName} (${courseUserName})${course.isDefault ? ' - DEFAULT' : ''}`
        )
        .join(', ');
  ...
  return (
    <Card className="m-3">
      ...
      <Card.Body>
        <p>This will add the course structure to your invest-west group, including:</p>
        <ul>
          <li>
            <strong>Student Showcase</strong> (student-showcase) - Default course
          </li>
          <li>
            <strong>History MSc</strong> (history-msc) - From existing availableCourses
          </li>
        </ul>
        ...
        <Button variant="primary" onClick={setupCourseStructure} disabled={isLoading}>
        ...
        <div className="mt-3">
          <small className="text-muted">
            <strong>Note:</strong> This is safe to run multiple times. If the structure already
